...
<Aside type="note">
  **测试网与主网**<br />
  下面的 Libra2 CLI 命令针对的是主网.对于测试网和开发网,需要更改 `--network` 的值.
  查看 [Libra2 区块链网络](/zh/network/nodes/networks)中的值,了解如何根据不同的网络设置配置文件.
</Aside>

在初始化质押池之前,请确保存在一个拥有 100 万 APT 的账户.

1. 使用现有账户(例如钱包)的私钥初始化 [Libra2 CLI](/zh/build/cli),或创建一个新账户.
...
3. 一旦质押池初始化完成,您就可以[连接到 Libra2 网络](/zh/network/nodes/validator-node/connect-nodes/connect-to-libra2-network).
...
```shellscript filename="Terminal"
libra2 move run --function-id 0x1::staking_contract::update_commision \
...
- 使用以下命令查看为运营者设置的受益人地址:
  ```shellscript filename="Terminal"
  libra2 move view --url <网络的REST API> \
...
<Aside type="note">
  在继续之前,请查看 [Libra2 区块链上的验证](/zh/network/blockchain/staking#validation-on-the-libra2-blockchain)获取简要概述.
</Aside>
...
<Aside type="note">
  对于测试网或开发网的 `--url` 字段值,请参见 [Libra2 区块链网络](/zh/network/nodes/networks).
</Aside>
...
- 只能由质押池所有者设置.运营者获得生成的质押奖励的"commission\_percentage"部分.如果您请求佣金(可以通过运行命令 `libra2 stake request-commission` 来完成),那么在 `lockup_expiration_utc_time` 结束时,佣金部分的奖励将转入运营者地址,其余部分将保留在质押池中并属于所有者.这里"the commission part of the rewards"指的是**commission\_not\_yet\_unlocked**的值.

这项设置仅能由质押池所有者进行.运营者会获得生成的质押奖励中一定的“佣金百分比(`commission_percentage`)”.通过执行命令 `libra2 stake request-commission`,你可以申请这份佣金.那么,在锁定期结束时间(`lockup_expiration_utc_time`)到来时,奖励中的佣金部分将转入运营者的地址,其余部分则留在质押池中,归属于池所有者.“奖励中的佣金部分(the commission part of the rewards)”指的是\*\*尚未解锁的佣金(commission\_not\_yet\_unlocked)\*\*数额.

例如,在一个月的锁定期情况下,您每个月调用 `libra2 stake request-commission`.这将在前一个月累积的佣金解锁后支付给您,但只有在前一个月结束时解锁.无论您在一个月中运行 `libra2 stake request-commission` 多少次,佣金都只会在 `lockup_expiration_utc_time` 来临之后支付.
...
- 佣金解锁的日期.然而,这个解锁的佣金不会自动发放.只有在再次调用命令 `libra2 stake request-commission` 时才会发放.
...
无论是所有者,运营者还是运营者的受益人,都可以请求佣金.您必须请求佣金**两次**,一次在锁定期结束之前,另一次在锁定期结束后,即在**lockup\_expiration\_utc\_time**结束时,通过运行 `libra2 stake request-commission` 命令.确保提供运营者和所有者的地址正确.下面是一个示例命令:
...
在锁定期截止之前执行 `libra2 stake request-commission` 命令将会开始解锁到那一刻为止赚取的所有已锁定佣金.这些佣金将一直处于 `pending_inactive` 状态,直到锁定期结束,并且在整个锁定期内都会继续赚取奖励.只有在锁定期结束并再次执行 `libra2 stake request-commission` 命令之后,这些佣金才能被提取.
...
要查看您在当前和过去纪元(epoch)中的验证器性能和赚取的奖励,请运行以下命令.将输出验证者在区块提案中的表现,以及在治理投票和治理提案中的表现.下面命令中使用了默认值.输入 `libra2 node get-performance --help` 查看使用的默认值.
...
- 当前纪元(epoch)中成功的领导者验证者提案.另见[Libra2 区块链上的验证](/zh/network/blockchain/staking#validation-on-the-libra2-blockchain),了解领导者验证者和投票者验证者之间的区别.

在当前纪元(Epoch)内,成功的领导-验证节点的提案.为了理解领导-验证节点与投票-验证节点之间的差异,请参考 [Libra2 区块链上的验证](/zh/network/blockchain/staking#validation-on-the-libra2-blockchain)部分.
