...
这是一份通过 Google Cloud Platform (GCP) 逐步部署 Libra2 验证器及其全节点(VFN)的指南.遵循此指南,你将会在不同的服务器上分别部署验证器和 VFN.

<Aside type="caution">
  **先决条件** <br />
  在开始之前,请确保阅读并理解了 [节点要求](/zh/network/nodes/validator-node/node-requirements).同样,请确保您已经安装了 [Libra2 CLI](/zh/build/cli),
...
8. `terraform apply` 完成后,您可以通过运行以下命令来检查资源是否已正确创建:
   ...
   - `gcloud container clusters get-credentials libra2-$WORKSPACE --zone <region/zone> --project <project>`:此命令将为您的 k8s 集群配置访问权限.
...
11. 接下来,您需要设置验证器的配置.这包括设置验证器和 VFN 的主机名,这些可能是 IP 地址或 DNS 地址.您可以通过运行以下命令来完成此操作:
    ...
    配置验证器将在 `~/$WORKSPACE/$USERNAME` 目录中创建两个 YAML 文件:`owner.yaml` 和 `operator.yaml`.这些(稍后)将有助于将您的节点连接到 Libra2 网络.
...
<Aside type="caution">
  **下一步** <br />
  您现在已经使用 GCP 完成了验证器和 VFN 的设置.然而,您的节点还不能连接到 Libra2 网络.
</Aside>
