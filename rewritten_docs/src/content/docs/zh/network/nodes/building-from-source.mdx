...
[使用 CLI 的二进制版本构建](/zh/build/cli/install-cli),但如果你想从源代码构建 Libra2 节点或 CLI,可以使用以下方法.

## 支持的操作系统

Libra2 可以在包括 Linux,macOS 和 Windows 在内的各种操作系统上构建.Libra2 在 Linux 和 macOS 上经过了广泛的测试,而在 Windows 上的测试较少.这是我们使用的版本:
...
1. 安装[Git](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git).
   Git 是克隆 libra2-core 仓库的必需工具,需要在克隆仓库之前安装.你可以按照官方[Git 网站](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git)上的说明进行安装.

2. 克隆 Libra2 仓库.要克隆 Libra2 仓库,你首先需要打开命令行提示符(macOS 或 Linux 的终端,Windows 的 PowerShell).然后运行以下命令从 GitHub 克隆 Git 仓库.

   ```shellscript filename="Terminal"
   git clone https://github.com/libra2org/libra2-core.git
   ```

3. 现在让我们 `cd` 进入 `libra2-core` 目录:
   ```shellscript filename="Terminal"
   cd libra2-core
   ```
...
这是可选的:切换到 `release` 分支以安装 Libra2 节点.
...
通过安装构建(build),测试(test)和检查(inspect) Libra2 Core 所需的依赖项来准备你的开发环境.
