...
<Aside type="caution">
  **不想连接到 devnet?**<br />
  本教程默认在 Libra2 `devnet` 网络上部署 PFN .要连接到其他网络(例如,`mainnet` 和 `testnet`),请将所有 `devnet` 实例替换为适当的网络名称.
</Aside>
...
<Aside type="note">
  **请先做好准备**<br />
  从这里开始,本指南假设你已经设置好了你的 GCP 账户,并为部署 Libra2 PFN 创建了一个新的项目.
</Aside>

你可以通过使用 Libra2 Terraform 模块在 GCP 上部署 PFN.
...
你可以在 [Libra2 Docker Hub](https://hub.docker.com/r/libra2labs/validator/tags?page=1\&ordering=last_updated\&name=devnet) 上找到 `devnet` 的 Docker 镜像标签.
...
module "fullnode" {
  # 从 libra2-labs/libra2-core 仓库下载 Terraform 模块
...
5. 要验证你的 PFN 是否正确,你需要:
   ...
   - 在一个 SSH 终端中直接设置到 Libra2 pod 的端口转发机制,
   - 在另一个 SSH 终端中进行测试.

   你可以按照以下步骤操作:

   - 设置到 libra2-fullnode pod 的端口转发.使用 `kubectl get pods -n libra2` 获取 pod 的名称:

     ```shellscript filename="Terminal"
     kubectlport-forward -n libra2 <pod-name> 9101:9101
     # 例如：
...
```terraform filename="main.tf"
module "fullnode" {
  # 从libra2-labs/libra2-core仓库下载Terraform模块
  source        = "github.com/libra2org/libra2-core.git//terraform/fullnode/gcp?ref=main"
  region        = "us-central1"  # 指定区域
  zone          = "c"            # 指定区域后缀
...
`devnet0-libra2-fullnode` 是运行 libra2 fullnode 容器的 pod.点击 pod 查看详细信息.你将看到一些指标和其他关于 pod 的详细信息.
...
Google Cloud 能够从集群中收集众多指标,并且可以在控制台中轻松地进行查看.您可以在控制台菜单中选择 `Kubernetes Engine`,然后点击您部署了 libra2 应用的集群.接着点击右上角的 `Operations` 链接.最后点击 `Metrics` 子标签,即可查阅具体的集群指标.
