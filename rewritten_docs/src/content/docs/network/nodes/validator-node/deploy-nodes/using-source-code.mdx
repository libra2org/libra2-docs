...
This is a step-by-step guide to deploy an Libra2 validator and validator fullnode (VFN) using source code. Using this guide, the validator
...
<Aside type="caution">
  **Prerequisites**<br />
  ...
  installed the [Libra2 CLI](/build/cli), as you will need it to setup your nodes.
</Aside>
...
<Aside type="note">
  **Default connection to mainnet**<br />
  ...
  To connect to a different Libra2 network, such as testnet, make sure you select the appropriate source code branch
  when you build the binary, and download the correct genesis and waypoint files for the network you want to connect to.
</Aside>

1. Follow the steps in [Building Libra2 From Source](/network/nodes/building-from-source) to download the `libra2-core` repository and source code.
...
3. Create a working directory for your Libra2 nodes, and pick a username for your nodes, e.g.,
...
5. Next, you will need to set your validator configuration. This includes setting the validator and VFN host names,
   ...
   You can set your validator configuration by running the following command with the Libra2 CLI:
   ...
   libra2 genesis set-validator-configuration \
       ...
       --username $USERNAME \
       --owner-public-identity-file ~/$WORKSPACE/keys/public-keys.yaml \
       --validator-host bot.libra2dev.com:6180 \
       --full-node-host fn.bot.libra2dev.com:6182 \
       --stake-amount 100000000000000
   ```
...
10. Now that you have set up your configuration files, you can start your validator and VFN.
    To start your validator, run the following commands, with the paths assuming you are in the root of the `libra2-core` directory:

    ```shellscript filename="Terminal"
    cargo clean
    cargo build -p libra2-node --release
    sudo mv target/release/libra2-node /usr/local/bin
    libra2-node -f ~/$WORKSPACE/config/validator.yaml
    ```

    To start your VFN, run the following commands on a separate, dedicated VFN machine. You will need to download the
    `libra2-core` source code and build the binary on the VFN machine. Likewise, you will need to copy across
    ...
    Start your VFN by running the following commands, with the paths assuming you are in the root of the `libra2-core` directory:

    ```shellscript filename="Terminal"
    cargo clean
    cargo build -p libra2-node --release
    sudo mv target/release/libra2-node /usr/local/bin
    libra2-node -f ~/$WORKSPACE/config/fullnode.yaml
    ```
...
If you want to run `libra2-node` as a service, you can set it up to run as a service controlled by `systemctl`.
...
```shellscript filename="/etc/systemd/system/libra2-node.service"
[Unit]
Description=Libra2 Node Service
...
WorkingDirectory=/home/nodeuser/libra2-core
ExecStart=/usr/local/bin/libra2-node -f /home/nodeuser/libra2-mainnet/config/validator.yaml
