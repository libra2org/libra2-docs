...
This is a step-by-step guide to deploy an Libra2 validator and validator fullnode (VFN) using Google Cloud Platform Services (GCP).
...
2. Create a storage bucket for storing the Terraform state on Google Cloud Storage.
   ...
   gsutil mb gs://<project-name>-libra2-terraform-dev
   ```
...
4. Modify the `main.tf` file to configure Terraform and create the Terraform module. See the example below:
   ...
   module "libra2-node" {
     ...
     source        = "github.com/libra2org/libra2-core.git//terraform/libra2-node/gcp"
   ...
   - The Terraform variables: [https://github.com/libra2org/libra2-core/blob/main/terraform/libra2-node/gcp/variables.tf](https://github.com/libra2org/libra2-core/blob/main/terraform/libra2-node/gcp/variables.tf)
...
8. After `terraform apply` finishes, you can check if the resources have been created correctly, by running the following commands:
   ...
   - `gcloud container clusters get-credentials libra2-$WORKSPACE --zone <region/zone> --project <project>`: This command will configure access for your k8s cluster.
