...
When running an Libra2 validator and VFN, we strongly recommend that the nodes run on two separate and
...
For running an Libra2 validator and VFN in mainnet, we recommend that your hardware be performant enough to maintain
...
2. Run the performance benchmarking tool provided by Libra2.
...
**Specifications for Running an Libra2 Validator and VFN on Mainnet**
...
tool. First, clone the `libra2-core` repository and install the required dependencies (see the
[**Cloning libra2-core**](/network/nodes/building-from-source#clone-the-libra2-core-repo) section).
...
Hardware requirements for Libra2 nodes depend on: (i) the transaction workload being executed; and (ii) the size of the
...
nodes (i.e., peers) to connect to you. There are different Libra2 network types, and each network type uses a different port (see below).

### Network types

There are three types of Libra2 networks:
...
configuration file](https://github.com/libra2org/libra2-core/blob/4ce85456853c7b19b0a751fb645abd2971cc4c0c/docker/compose/libra2-node/fullnode.yaml#L10) for a VFN node
