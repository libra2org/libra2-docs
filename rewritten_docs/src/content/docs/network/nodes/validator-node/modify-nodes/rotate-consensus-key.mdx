...
Using the following [Libra2 CLI](/build/cli) command to generate a new consensus identity.
In this example, identity files are saved to directory `/new/key/root`.

```shellscript filename="Terminal"
libra2 genesis generate-keys --output-dir /new/key/root
```
...
Libra2 CLI is needed again, and your operator account is assumed to have been set up as a CLI profile `profile1`.
...
Run a transaction with libra2 CLI to update the on-chain consensus public key for the next epoch.
