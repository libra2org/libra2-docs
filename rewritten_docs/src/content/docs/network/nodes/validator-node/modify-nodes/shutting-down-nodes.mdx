...
run the following command using the Libra2 CLI. You will need to set the `profile` and `owner-address` flags.

```shellscript filename="Terminal"
libra2 node leave-validator-set --profile <operator-profile> --pool-address <owner-address>
```
...
1. Stop your node by killing the `libra2-node` process. This is sufficient to shut down your node.
