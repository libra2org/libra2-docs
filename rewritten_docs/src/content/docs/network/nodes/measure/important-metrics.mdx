...
<Aside type="note">
  **Metric instability**<br />
  As Libra2 continues to grow and develop the blockchain software, many metrics will come and go.
...
1. `libra2_consensus_proposals_count`: Counts the number of times the node sent a block
...
2. `libra2_consensus_last_committed_round`: Counts the last committed round of the node.
...
3. `libra2_consensus_timeout_count`: Counts the number of times the node locally timed out while trying
...
4. `libra2_state_sync_executing_component_counters{label="consensus"`: This counter increases
...
1. `libra2_state_sync_version{type="synced"}`: This metric displays the current synced version of the node,
...
2. `libra2_data_client_highest_advertised_data{data_type="transactions"}`: This metric displays the highest
   version synced and advertised by the peers that your node is connected to. As a result, when this metric is
   higher than `libra2_state_sync_version{type="synced"}` (above) it means your node can see new blockchain data and
   will sync the data from its peers.
3. `libra2_state_sync_version{type="synced_states"}`: This metric counts the number of states that have been
   downloaded while a node is [fast syncing](/network/nodes/configure/state-sync#fast-syncing). If this metric doesn't increase,
   and `libra2_state_sync_version{type="synced"}` doesn't increase (from above), it means the node is not syncing
   at all and an issue has likely occurred.
4. `libra2_state_sync_bootstrapper_errors` and `libra2_state_sync_continuous_syncer_errors`: If your node is
...
<Aside type="note">
  Compare the synced version shown by `libra2_state_sync_version{type="synced"}` with the highest version shown on the [Libra2 Explorer](https://explorer.libra2labs.com/?network=mainnet) to see how far behind the latest blockchain version your node is. Remember to select the correct network that your node is syncing to (e.g., `mainnet`).
</Aside>
...
1. `libra2_connections{direction="inbound"` and `libra2_connections{direction="outbound"`: These metrics count
...
1. `libra2_api_requests_count{method="GET"` and `libra2_api_requests_count{method="POST"`: These metrics count
...
2. `libra2_api_response_status_count`: This metric counts the number of response types that were sent for
   the REST API. For example, `libra2_api_response_status_count{status="200"}` counts the number of requests
