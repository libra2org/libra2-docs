...
This document describes how to bootstrap an Libra2 node using a backup. This can be done on all node types, including
...
To achieve these goals, the Libra2 database restore tool lets you use existing [public backup files](#public-backup-files) to restore
...
Libra2 Foundation maintains a few publicly accessible database backups by continuously querying a PFN and storing the backup
...
| ------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Testnet | Discontinued                                                                          | https://github.com/libra2org/libra2-networks/blob/main/testnet/backups/gcs.yaml |
| Mainnet | https://github.com/libra2org/libra2-networks/blob/main/mainnet/backups/s3-public.yaml | https://github.com/libra2org/libra2-networks/blob/main/mainnet/backups/gcs.yaml |
...
![libra2-db-restore.png](~/images/libra2-db-restore.png)

## Restore an Libra2 DB

The [Libra2 CLI](/build/cli) supports two kinds of restore operations for Libra2 nodes:
...
<Aside type="note">
  Libra2 CLI 1.0.14 or newer is needed to perform these operations. Additionally, depending on whether you use AWS or
  Google Cloud, install [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) or
  [gsutil](https://cloud.google.com/storage/docs/gsutil_install).
</Aside>

The sections below provide examples of how to use the Libra2 CLI to restore a database from a backup.

### Bootstrap to the latest version

The `libra2 node bootstrap-db` command can quickly restore a database from the latest snapshot back to a target version,
...
The `libra2 node bootstrap-db` command can restore the transaction history within a specified period, along with the state Merkle tree at the target version.
...
To restore an Libra2 node with the full history from genesis, set `ledger-history-start-version` to 0 and
