---
title: "Running a Local Network via Libra2 CLI"
---

import { Aside, Steps } from '@astrojs/starlight/components';

Local networks can be helpful when testing your code. They are not connected to any production Libra2 networks like mainnet, but they are useful for three main reasons:
...
3. **High availability**: The Libra2 devnet and testnet networks are periodically upgraded, during which time they can be unavailable. Local development networks are also always available even if you have no internet access.
...
<Steps>
  1. Ensure you have the  installed.
     ...
     You can verify if the Libra2 CLI is installed by running `libra2 --version`.

  2. Ensure you have  installed.

     You can check whether Docker is installed by running `docker --version`.

     1. This is exclusively needed for making a production-like environment by running the Indexer API. Many downstream tools such as the Libra2 SDK depend on the Indexer API.
  ...
  4. Run the following command in a new terminal to start the private network:
     ...
     libra2 node run-local-testnet --with-indexer-api
     ```

     <Aside type="caution">
       Note: Despite the name (`local-testnet`), this has nothing to do with the Libra2 testnet, it will run a network entirely local to your machine.
     </Aside>
     ...
     Completed generating configuration:
             Log file: "/Users/dport/.libra2/testnet/validator.log"
             Test dir: "/Users/dport/.libra2/testnet"
             Libra2 root key path: "/Users/dport/.libra2/testnet/mint.key"
             Waypoint: 0:397412c0f96b10fa3daa24bfda962671c3c3ae484e2d67ed60534750e2311f3d
             ChainId: 4
             REST API endpoint: http://0.0.0.0:8080
             Metrics endpoint: http://0.0.0.0:9101/metrics
             Libra2net fullnode network endpoint: /ip4/0.0.0.0/tcp/6181
  ...
  5. Wait for the network to start
     ...
     <details>
       ...
       ```shellscript filename="Terminal"
       panicked at crates/libra2/src/node/local_testnet/logging.rs:64:10:
     ...
     - [Node API](/network/nodes/libra2-api-spec): This is a REST API that runs directly on the node. It enables core write functionality such as transaction submission and a limited set of read functionality, such as reading account resources or Move module information.
     ...
     ```shellscript filename="Terminal"
     libra2 node run-local-testnet --help
     ```
</Steps>
...
```shellscript filename="Terminal"
libra2 init --profile <your-profile-name> --network local
```

You can then use that profile for any commands you want to use going forward. For example, if you wanted to publish a Move module like the [`hello_blockchain`](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/hello_blockchain) package to your local network you could run:

```shellscript filename="Terminal"
libra2 move publish --profile <your-profile-name> --package-dir /opt/git/libra2-core/libra2-move/move-examples/hello_blockchain --named-addresses HelloBlockchain=local
```

### Configuring the TypeScript SDK

If you want to use the local network with the TypeScript SDK, you can use local network URLs when initializing the client object (`Libra2`):

```tsx
import { Libra2, Libra2Config, Network } from "@libra2-labs/ts-sdk";

const network = Network.LOCAL;
const config = new Libra2Config({ network });
const client = new Libra2(config);
```
...
```shellscript filename="Terminal"
libra2 node run-local-testnet --force-restart
```
...
```shellscript filename="Terminal"
libra2 node run-local-testnet --force-restart --assume-yes
```
...
docker run \
   ...
   libra2labs/tools:nightly \
   libra2 node run-local-testnet \
   --test-dir /testnet \
   --with-indexer-api
```

Instead of `nightly` you can use any tag from [here](https://hub.docker.com/r/libra2labs/tools/tags).
