...
<Aside type="note">
  We strongly recommend that you read the consensus section of [Libra2 Blockchain Deep Dive](/network/blockchain/blockchain-deep-dive#consensus) before proceeding further.
</Aside>
...
Anyone can participate in the Libra2 consensus process, if they stake sufficient utility coin, i.e., place their utility coin into escrow. To encourage validators to participate in the consensus process, each validator's vote weight is proportional to the amount of validator's stake. In exchange, the validator is rewarded proportionally to the amount staked. Hence, the performance of the blockchain is aligned with the validator's interest, i.e., rewards.

<Aside type="note">
  Currently, slashing is not implemented.
</Aside>

The current on-chain data can be found in [`staking_config::StakingConfig`](https://api.mainnet.libra2labs.com/v1/accounts/0x1/resource/0x1::staking_config::StakingConfig). The configuration set is defined in [`staking_config.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/configs/staking_config.move).

The rest of this document presents how staking works on the Libra2 blockchain. See [Supporting documentation](#supporting-documentation) at the bottom for related resources.

## Staking on the Libra2 blockchain

Below is a summary flow diagram of how staking on the Libra2 blockchain works. The sections following the summary describe it in detail.
...
The Libra2 staking module defines a capability that represents ownership.

<Aside type="note">
  See the `OwnerCapability` defined in [stake.move](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/stake.move).
</Aside>
...
Using this owner-operator-voter model, a custodian can assume the owner persona and stake on the Libra2 blockchain and participate in the Libra2 governance. This model allows delegations and staking services to be built as it separates the account that is in control of the funds from the other accounts (operator, voter), hence allows secure delegations of responsibilities.

This section describes how this works, using Bob and Alice in the example.

### Owner

The owner is the owner of the funds. For example, Bob creates an account on the Libra2 blockchain. Now Bob has the `OwnerCapability` resource. Bob can assign his account’s operator address to the account of Alice, a trusted node operator, to appoint Alice as a validator.
...
<Aside type="note">
  This document describes staking. See [Governance](/network/blockchain/governance) for how to participate in the Libra2 on-chain governance using the owner-voter model.
</Aside>

## Validation on the Libra2 blockchain
...
2. Libra2 governance can also directly remove validators from the active set. **Note that governance proposals will always trigger an epoch change.**
...
1. Owner initializes the stake pool with `libra2 stake create-staking-contract`.
2. When the owner is ready to deposit the stake (or have funds assigned by a staking service in exchange for ownership capability), owner calls `libra2 stake add-stake`.
3. When the validator node is ready, the operator can call `libra2 node join-validator-set` to join the active validator set. Changes will be effective in the next epoch.
...
7. After exiting, the validator can either explicitly leave the validator set by calling `libra2 node leave-validator-set` or if their stake drops below the min required, they would get removed at the end of the epoch.
8. Validator can always rejoin the validator set by going through steps 2-3 again.
9. An owner can always switch operators by calling `libra2 stake set-operator`.
10. An owner can always switch designated voter by calling `libra2 stake set-delegated-voter`.

## Joining the validator set

Participating as a validator node on the Libra2 network works like this:

1. Operator runs a validator node and configures the on-chain validator network addresses and rotates the consensus key.
2. Owner deposits her Libra2 coins funds as stake, or have funds assigned by a staking service. The stake must be at least the minimum amount required.
3. **The validator node cannot sync until the stake pool becomes active.**
4. Operator validates and gains rewards.
5. The staked pool is automatically be locked up for a fixed duration (set by the Libra2 governance) and will be automatically renewed at expiration. You cannot withdraw any of your staked amount until your lockup period expires. See [stake.move#L728](https://github.com/libra2org/libra2-core/blob/00a234cc233b01f1a7e1680f81b72214a7af91a9/libra2-move/framework/libra2-framework/sources/stake.move#L728).
6. Operator must wait until the new epoch starts before their validator becomes active.

<Aside type="note">
  For step-by-step instructions on how to join the validator set, see: [Joining Validator Set](/network/nodes/validator-node/connect-nodes/connect-to-libra2-network#join-the-validator-set).
</Aside>
...
When you join the validator set, your stake will automatically be locked up for a fixed duration that is set by the Libra2 governance.
...
<Aside type="note">
  The lockup duration is decided by the Libra2 governance, i.e., by the covenants that the Libra2 community members vote on, and not by any special entity like the Libra2 Foundation.
</Aside>

## Epoch

An epoch in the Libra2 blockchain is defined as a duration of time, in seconds, during which a number of blocks are voted on by the validators, the validator set is updated, and the rewards are distributed to the validators.

<Aside type="note">
  The Libra2 mainnet epoch is set as 7200 seconds (two hours).
</Aside>

### Triggers at the epoch start

<Aside type="note">
  See the [Triggers at epoch boundary section of `stake.move`](https://github.com/libra2org/libra2-core/blob/256618470f2ad7d89757263fbdbae38ac7085317/libra2-move/framework/libra2-framework/sources/stake.move#L1036) for the full code.
</Aside>
...
3. Your proposer performance in the Libra2 governance.

<Aside type="note">
  The `rewards_rate` is set by the Libra2 governance. Also see [Validation on the Libra2 blockchain](#validation-on-the-libra2-blockchain).
</Aside>
...
Note that rewards are given only to the **leader-validators**, i.e., validators who propose the new block, and not to the **voter-validators** who vote on the leader's proposal for the new block. See [Validation on the Libra2 blockchain](#validation-on-the-libra2-blockchain).
...
<Aside type="note">
  See the Libra2 Stake module in the Move language at [stake.move](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/stake.move).
</Aside>
...
- [Current on-chain data](https://api.mainnet.libra2labs.com/v1/accounts/0x1/resource/0x1::staking_config::StakingConfig)
...
- [Configuration file `staking_config.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/configs/staking_config.move)
- [Contract file `staking_contract.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/staking_contract.move) covering requesting commissions
- [All staking-related \`.move files](https://github.com/libra2org/libra2-core/tree/main/libra2-move/framework/libra2-framework/sources)
