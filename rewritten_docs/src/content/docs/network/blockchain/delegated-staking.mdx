...
## Delegated Staking on the Libra2 Blockchain
...
Using this model, the owner does not have to stake on the Libra2 blockchain in order to run a validator.

[How Validation on the Libra2 blockchain works](/network/blockchain/staking#validation-on-the-libra2-blockchain)
...
<Aside type="note">
  ...
  participate in the Libra2 on-chain governance using the owner-voter model.
</Aside>
...
Participating as a delegation validator node on the Libra2 network works like this:
...
When the operator joins the validator set, the delegation pool's stake will automatically be locked up for a fixed duration that is set by the Libra2 governance. Delegators will follow the delegation pool's lockup cycle.
