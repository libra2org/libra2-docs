...
An Libra2 node is an entity of the Libra2 ecosystem that tracks the [state](/network/glossary#state) of the Libra2 blockchain. Clients interact with the blockchain via Libra2 nodes. There are two types of nodes:

- [Validator nodes](/network/blockchain/validator-nodes)
- Fullnodes

Each Libra2 node comprises several logical components:
...
The [Libra2-core](/network/glossary#libra2-core) software can be configured to run as a validator node or as a fullnode.

## Overview

Fullnodes can be run by anyone. Fullnodes verify blockchain history by either re-executing all transactions in the history of the Libra2 blockchain or replaying each transaction's output. Fullnodes replicate the entire state of the blockchain by synchronizing with upstream participants, e.g., other fullnodes or validator nodes. To verify blockchain state, fullnodes receive the set of transactions and the [accumulator root hash](/network/glossary#accumulator-root-hash) of the ledger signed by the validators. In addition, fullnodes accept transactions submitted by Libra2 clients and forward them directly (or indirectly) to validator nodes. While fullnodes and validators share the same code, fullnodes do not participate in consensus.
...
- Get a consistent view of the Libra2 ledger.
