...
Libra2 transactions by default charge a base gas fee, regardless of market conditions.
...
As explained in the [optimization principles](#optimization-principles) section, storage gas has by far the largest effect on base gas. For background on the Libra2 gas model, see [The Making of the Libra2 Gas Schedule](https://libra2labs.medium.com/the-making-of-the-libra2-gas-schedule-508d5686a350).
...
Additional storage gas parameters are defined in [`table.rs`], [`move_stdlib.rs`], and other assorted source files in [`libra2-gas-schedule/src/`].
...
The following storage fee parameters are applied (i.e., charged in absolute APT values) to represent the disk space and structural costs associated with using the [Libra2 authenticated data structure](/network/glossary#merkle-trees) for storing items on the blockchain.

| Parameter                         | Meaning                                                                                                                                                                                                                                             |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| storage\_fee\_per\_state\_slot\_create | allocating a state slot, by `move_to()`, `table::add()`, etc                                                                                                                                                                                        |
| storage\_fee\_per\_state\_byte        | Notice this is charged every time the slot grows in size, not only at allocation time. (However, for simplicity, refunding is only at deletion time. See [AIP-65](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-65.md#specification)) |
...
Payload gas is defined in [`transaction.rs`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-schedule/src/gas_schedule/transaction.rs), which incorporates storage gas with several payload- and pricing-associated parameters:
...
As of the time of this writing, `min_price_per_gas_unit` in [`transaction.rs`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-schedule/src/gas_schedule/transaction.rs) is defined as [`libra2_global_constants`]`::GAS_UNIT_PRICE` (which is itself defined as 100), with other noteworthy [`transaction.rs`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-schedule/src/gas_schedule/transaction.rs) constants as follows:
...
As of the time of this writing, [`transaction/mod.rs`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-schedule/src/gas_schedule/transaction.rs) defines the minimum amount of internal gas per transaction as 1,500,000 internal units (15,000 [octas](/network/glossary#octa) at minimum), an amount that increases by 2,000 internal gas units (20 octas minimum) per byte for payloads larger than 600 bytes, with the maximum number of bytes permitted in a transaction set at 65536.
Hence, in practice, payload gas is unlikely to be a concern.

[#4540]: https://github.com/libra2org/libra2-core/pull/4540/files

[`libra2-gas-schedule/src/`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-schedule/src

[`libra2_global_constants`]: https://github.com/libra2org/libra2-core/blob/main/config/global-constants/src/lib.rs

[`base_8192_exponential_curve()`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/doc/storage_gas.md#0x1_storage_gas_base_8192_exponential_curve

[BCS sequence specification]: https://github.com/diem/bcs#fixed-and-variable-length-sequences

[`meter.rs`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-meter/src/meter.rs

[`initialize()`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/doc/storage_gas.md#0x1_storage_gas_initialize

[`instr.rs`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-schedule/src/gas_schedule/instr.rs

[`move_stdlib.rs`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-schedule/src/gas_schedule/move_stdlib.rs

[`on_reconfig()`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/doc/storage_gas.md#@Specification_16_on_reconfig

[`storage_gas.md`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/doc/storage_gas.md

[`storage_gas.move`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/storage_gas.move

[`StorageGas`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/doc/storage_gas.md#resource-storagegas

[`table.rs`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-schedule/src/gas_schedule/table.rs

[`transaction.rs`]: https://github.com/libra2org/libra2-core/blob/main/libra2-move/libra2-gas-schedule/src/gas_schedule/transaction.rs
