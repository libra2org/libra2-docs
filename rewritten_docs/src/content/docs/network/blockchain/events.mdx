...
Events are emitted during the execution of a transaction. Each Move module can define its own events and choose when to emit the events upon execution of the module. Libra2 Move supports two form of events: module events and EventHandle events. Module events are the modern event mechanism and shipped in the framework release 1.7. EventHandle events are deprecated and shipped with the original framework. Because of how blockchains work, EventHandle events will likely never be fully removed from Libra2.
...
Example module events are available [here](https://explorer.libra2labs.com/txn/682252266/events?network=testnet). Indices 0, 1, 2 are three module events of
...
As part of our legacy, Libra2 inherited the Libra/Diem event streams derived from EventHandles. Where each EventHandle is identified by a globally unique value, GUID, and a per-event sequence number and stored within a resource. Each event within a stream has a unique sequence number derived from the EventHandle sequence number.

For example, during a [coin transfer](/build/guides/first-transaction), both the sender and receiver's accounts will emit `SentEvent` and `ReceivedEvent`, respectively. This data is stored within the ledger and can be queried via the REST interface's [Get events by event handle](https://api.devnet.libra2labs.com/v1/spec#/operations/get_events_by_event_handle).

Assuming that an account `0xc40f1c9b9fdc204cf77f68c9bb7029b0abbe8ad9e5561f7794964076a4fbdcfd` had sent coins to another account, the following query could be made to the REST interface: `https://api.devnet.libra2labs.com/v1/accounts/c40f1c9b9fdc204cf77f68c9bb7029b0abbe8ad9e5561f7794964076a4fbdcfd/events/0x1::coin::CoinStore<0x1::libra2_coin::LibraCoin>/withdraw_events`. The output would be all `WithdrawEvent`s stored on that account, it would look like
...
Each registered event has a unique `key`. The key `0x0000000000000000caa60eb4a01756955ab9b2d1caca52ed` maps to the event `0x1::coin::CoinStore<0x1::libra2_coin::LibraCoin>/sent_events` registered on account `0xc40f1c9b9fdc204cf77f68c9bb7029b0abbe8ad9e5561f7794964076a4fbdcfd`. This key can then be used to directly make event queries, e.g., `https://api.devnet.libra2labs.com/v1/events/0x0000000000000000caa60eb4a01756955ab9b2d1caca52ed`.
...
[coin_transfer]: https://github.com/libra2org/libra2-core/blob/bdd0a7fe82cd6aab4b47250e5eb6298986777cf7/libra2-move/framework/libra2-framework/sources/coin.move#L412

[get_events]: https://api.devnet.libra2labs.com/v1/spec#/operations/get_events_by_event_handle
