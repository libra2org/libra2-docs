...
On Libra2, execution refers to the process where validators run and apply smart contract transactions
...
throughput. The impact of this can be felt in the block time. As of December 2024, Libra2 blocks close within 250ms.
...
Libra2 uses a highly efficient, multi-threaded, in-memory parallel execution engine called Block-STM.
...
Block-STM is the state-of-the-art dynamic parallelism execution engine developed by the Libra2 Foundation
team.
...
- [Block-STM blog post](https://medium.com/libra2labs/block-stm-how-we-execute-over-160k-transactions-per-second-on-the-libra2-blockchain-3b003657e4ba)
