...
|                             | Libra2 / Move                                                  | Solana / SeaLevel                                                | EVM                                                        | Sui / Move                            |
| --------------------------- | ------------------------------------------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------- |
...
| Parallelization             | Capable of inferring parallelization at runtime within Libra2  | Requires specifying all data accessed                            | Currently serial nothing in production                     | Requires specifying all data accessed |
...
The Libra2 Move adapter features include:

- [Move Objects](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-10.md) that offer an extensible programming model for globally access to heterogeneous set of resources stored at a single address on-chain.
...
- [Tables](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-stdlib/sources/table.move) for storing key, value data within an account at scale.
- Parallelism via [Block-STM](https://medium.com/libra2labs/block-stm-how-we-execute-over-160k-transactions-per-second-on-the-libra2-blockchain-3b003657e4ba) that enables concurrent execution of transactions without any input from the user.
- Multi-agent framework that enables a single transaction to be submitted with multiple distinct `signer` entities.

The Libra2 framework ships with many useful libraries:

- An [Libra2 Token Objects](https://github.com/libra2org/libra2-core/tree/main/libra2-move/framework/libra2-token-objects/sources) standard as defined in [AIP-11](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-11.md) and [AIP-22](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-22.md) that makes it possible to create interoperable NFTs with either lightweight smart contract development or none at all.
- A [Coin standard](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/coin.move) that makes it possible to create type-safe Coins by publishing a trivial module.
- A [Fungible asset standard](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/fungible_asset.move) as defined in [AIP-21](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-21.md) to modernize the coin concept with better programmability and controls.
- A [staking](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/staking_contract.move) and [delegation](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/delegation_pool.move) framework.
- A [`type_of`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-stdlib/sources/type_info.move) service to identify at run-time the address, module, and struct name of a given type.
- A [timestamp service](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/timestamp.move) that provides a monotonically increasing clock that maps to the actual current Unix time.
