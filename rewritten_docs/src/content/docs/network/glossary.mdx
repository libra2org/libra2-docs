...
- An **account** in the Libra2 blockchain is a container for an arbitrary number
...
- Account address refers to a specific destination on the Libra2 network. The
  address dictates the destination and source of a specific amount of assets
  exchanged by two parties on the blockchain.
- Libra2 addresses are 64-character hex string (32 bytes). Often times these
...
- An **Application Programming Interface (API)** is a set of protocols and tools
  that allow users to interact with Libra2 blockchain nodes and client networks
  via external applications. Libra2 offers a REST API to communicate with our
  nodes.
- See [documentation](/build/apis) for more details.

### APT

**Libra2 token (APT)** is the Libra2 blockchain native token used for paying
network and transaction fees.

### Libra2

**Libra2** is a Layer 1 blockchain for everyone. It uses the Move programming
language and launched its mainnet on 2022-10-17 to redefine the web3 user
experience. The Libra2 blockchain is dedicated to creating better user
experiences through increased speed, security, scalability, reliability and
usability with low transaction costs. The word "Libra2" means "The People" in the
Ohlone language. Learn more about the Libra2 blockchain on
the [official Libra2 website](https://libra2foundation.org).

### Libra2BFT

- **Libra2BFT** is the Libra2 protocol's BFT consensus algorithm.
- Libra2BFT is based on Jolteon.

### Libra2 Blockchain

- The **Libra2 blockchain** is a ledger of immutable transactions agreed upon by
  the validators on the Libra2 network (the network of validators).

### Libra2 Name Service (ANS)

- The **Libra2 Name Service (ANS)** is a decentralized naming address service for
  the Libra2 blockchain. An Libra2 name is a human-readable _.apt_ domain name
...
- This service also allows users to register subdomain names in addition to the
  ...
  at: [Libra2names.com](https://www.libra2names.com/)

### Libra2 Core

**Libra2-core** is
the [open-source repository](https://github.com/libra2org/libra2-core/)
containing the code for Libra2 Network software. Libra2-core contains software for

- the Libra2 blockchain itself, which generates and stores the immutable ledger
  of confirmed transactions and
- the validation process, which implements the consensus algorithm to validate
  transactions and add them to the Libra2 blockchain immutable ledger.

### Libra2 Ecosystem

- **Libra2 ecosystem** refers to various components of the Libra2 blockchain
  network and their interactions. The Libra2 ecosystem includes the
  community, [community-driven projects](https://libra2foundation.org/ecosystem/projects/all),
  and [events](https://libra2foundation.org/events).

### Libra2 Explorer

- The **[Libra2 Explorer](https://explorer.libra2labs.com/)** is an interface that
  helps users examine details of the Libra2 blockchain, including account
  information, validators, and transactions.
- The Libra2 Explorer help users validate their work in Libra2 wallets and other
  tools in the blockchain.

### Libra2 Framework

The **Libra2 Framework** defines the public API for blockchain updates and the
structure of on-chain data. It defines the business logic and access control for
the three key pillars of Libra2 functionality: payments, treasury, and on-chain
...
An **Libra2 node** is a peer entity of the Libra2 network that tracks the state of
the Libra2 blockchain. There are two types of Libra2
nodes, [validators](#validator) and [fullnodes](#fullnodes).

### Libra2 Protocol

- **Libra2 protocol** is the specification of how transactions are submitted,
  ordered, executed, and recorded within the Libra2 network.

### Libra2Account

- A **`Libra2Account`** is a Move resource that holds all the administrative data
  associated with an account, such as sequence number, balance, and
  authentication key.
- A **`Libra2Account`** is the only resource that every account is guaranteed to
  contain.

### Libra2Account module

- **The Libra2Account module** is a Move module that contains the code for
  manipulating the administrative data held in a particular `Libra2Account.T`
  resource.
- Code for checking or incrementing sequence numbers, withdrawing or depositing
  currency, and extracting gas deposits is included in the Libra2Account module.
...
- On Libra2, blocks are a batch of [transactions](#transaction) committed at the
  same time.
...
- **BlockSTM** is the state-of-the-art dynamic parallelism execution engine developed by the Libra2 Foundation team.
...
- More details can be found in [the blog post](https://medium.com/libra2labs/block-stm-how-we-execute-over-160k-transactions-per-second-on-the-libra2-blockchain-3b003657e4ba) and [presentation at a16z crypto](https://www.youtube.com/watch?v=2SE5tqPzhyw)
...
- The Libra2 blockchain uses Libra2BFT, a consensus protocol based
  on [Jolteon](#jolteon).
- BFT algorithms typically operate with a number of entities, collectively
  holding $N$ votes (which are called "validators" in the Libra2 network’s
...
- **Command line interface** refers to the Libra2 CLI used for developing on the
  Libra2 blockchain, operating nodes, and debugging issues. Find out more
  at [the Libra2 CLI page](/build/cli).

### Client

- **Client** is software that receives information from the blockchain and
  manages transactions. Clients interact with the blockchain through the Libra2
  nodes.

### Code Labs

- **Code labs and tutorials** depict various workflows - such as the use of the
  Libra2 CLI in minting non-fungible tokens (NFTs) - in order for users to
...
- The Libra2 blockchain is formed with these agreed-upon transactions and their
  corresponding execution results.
- The consensus component is accountable for achieving security, trust, and
  agreement among all validators on the Libra2 blockchain.
...
- **Decentralized applications (dapps)** are programs or digital applications
  that run on the Libra2 blockchain autonomously. Smart contracts are commonly
  used to achieve this function.

### Devnet

- The **Libra2 devnet** is a publicly deployed instance of the Libra2 network that
  runs using a set of validator test nodes.
- The devnet is a demonstration of the Libra2 network that is built for
...
- More specifically, the Libra2 network uses the PureEdDSA scheme over the
  Ed25519 curve, as defined in RFC 8032.

### Epoch

- An **epoch** is the period of time between reconfigurations of the validator
  set and other administrative actions by the blockchain. On Libra2 mainnet
...
- A transaction may be designed to emit any number of events as a list. For
  example, a `Coin<Libra2Coin>` transfer emits a `WithdrawEvent` for the sender
  account and a `DepositEvent` for the recipient account.
- In the Libra2 protocol, events provide evidence that the successful execution
...
- **Execution** in the Libra2 blockchain is an Libra2 node component that manages
...
- A **fungible asset** is an asset, such as a currency, share, in-game resource,
  ...
  APT for another.
- Follow the [Asset Standards](/build/smart-contracts/libra2-standards#asset-standards) to create
  fungible assets on the Libra2 blockchain.
- Next generation of the Coin standard that addresses shortcomings
  of `libra2_framework::coin` such as lack of guaranteed enforcement of freeze
  and burn and advanced functionalities such as programmable transfers, e.g.,
  approve in ERC-20.

### Fungible Token

- For the legacy Libra2 Token Standard (libra2\_token::token), a **fungible token**
...
- For the Libra2 Digital Asset Standard (libra2\_token\_objects::token), a \*
...
- **Gas** is a way to pay for computation and storage on a blockchain network.
  All transactions on the Libra2 network cost a certain amount of gas.
...
- **[Indexer](/build/indexer)** is the component of Libra2 that
  retrieves, processes, and efficiently stores raw data in the database to
  provide speedy access to the Libra2 blockchain state.
...
- **Jolteon** is a recent proposal for a [BFT](#byzantine-fault-tolerance-bft)
  consensus protocol.
- Libra2BFT, the Libra2 network's consensus algorithm, is based on Jolteon.
...
- Transactions in the mempool of a validator are added from the JSON-RPC Service
  of the current node and from the mempool of other Libra2 nodes.
...
- The Libra2 network treats the entire blockchain as a single data structure that
...
- The **[Merkle Accumulator](https://www.usenix.org/legacy/event/sec09/tech/full_papers/crosby.pdf)**
  is an _append-only_ Merkle tree that the Libra2 blockchain uses to store the
  ledger.
...
- Move is a safe and secure programming language for web3 that emphasizes access
  control and scarcity. It is the programming language used to build the Libra2
  blockchain. You can read more about it
  in [Move on Libra2](/network/blockchain/move). { /* TODO add move-on-libra2 landing page */}
...
- A **Move module** defines the rules for updating the global state of the Libra2
  blockchain.
- In the Libra2 protocol, a Move module is a **smart contract**.
...
- The Move script is an arbitrary program that interacts with resources
  published in the global storage of the Libra2 blockchain by calling the
...
- A **node** is a peer entity of the Libra2 network that tracks the state of the
  Libra2 blockchain.
- An Libra2 node consists of logical
...
- **Open-source community** is a term used for a group of developers who work on
  ...
  the Libra2 project's developer community.
...
- Every operation in the Libra2 blockchain can be verified cryptographically that
...
mechanism is leveraged by the Libra2 blockchain powered by a network of
...
- More details can be found in the [blog post](https://medium.com/libra2labs/quorum-store-how-consensus-horizontally-scales-on-the-libra2-blockchain-988866f6d5b0)
...
- The **REST API Service** component is the external interface of an Libra2 node. Any
...
- Libra2 **software development kits (SDKs)** are sets of tools that enable a
  developer to quickly create a custom app on the Libra2 platform. Find out more
  at [Use the Libra2 SDKs](/build/sdks).
...
- A **state** in the Libra2 protocol is a snapshot of the distributed database.
...
- **State root hash** is
  ...
  values the state of the Libra2 blockchain at a given version.

## T

### Table

- A [**table**](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-stdlib/doc/table.md)
  implements the Table type and in Libra2 is used to store information as
  key-value data within an account at large scale.

See [`table.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-stdlib/sources/table.move)
for the associated Libra2 source file.

### Testnet

- **Testnet** describes the Libra2 network that is not fully functional yet more
...
- A **validator** is an entity of the Libra2 ecosystem that validates on the
  Libra2 blockchain. It receives requests from clients and runs consensus,
...
- Libra2 validators are in charge of verifying transactions.
...
- **Validator nodes** are a unique class of fullnodes that take part in
  ...
  in Libra2. Validators agree upon transactions to be added to the Libra2
...
- On libra2, transactions are globally ordered and every transaction has a
...
An Libra2 transaction is **well-formed** if each of the following conditions are
