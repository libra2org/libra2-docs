...
- **LibraBFT** is the Libra2 protocol's BFT consensus algorithm.
- LibraBFT is based on Jolteon.
...
- This service also allows users to register subdomain names in addition to the
  ...
  at: [Libranames.com](https://www.libra2names.com/)
...
- A **`LibraAccount`** is a Move resource that holds all the administrative data
  associated with an account, such as sequence number, balance, and
  authentication key.
- A **`LibraAccount`** is the only resource that every account is guaranteed to
  contain.

### LibraAccount module

- **The LibraAccount module** is a Move module that contains the code for
  manipulating the administrative data held in a particular `LibraAccount.T`
  resource.
- Code for checking or incrementing sequence numbers, withdrawing or depositing
  currency, and extracting gas deposits is included in the LibraAccount module.
...
- The Libra2 blockchain uses LibraBFT, a consensus protocol based
  on [Jolteon](#jolteon).
...
- A transaction may be designed to emit any number of events as a list. For
  example, a `Coin<LibraCoin>` transfer emits a `WithdrawEvent` for the sender
...
- **Jolteon** is a recent proposal for a [BFT](#byzantine-fault-tolerance-bft)
  consensus protocol.
- LibraBFT, the Libra2 network's consensus algorithm, is based on Jolteon.
