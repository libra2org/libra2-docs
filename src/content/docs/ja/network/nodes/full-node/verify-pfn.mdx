...
<Aside type="note">
  You can learn more about Aptos metrics in the [Node Inspection Service](/ja/network/nodes/measure/node-inspection-service) documents.
</Aside>

### Verify synchronization

During the initial synchronization of your PFN, there may be a lot of data to transfer (read more about how state
sync works in the [state sync](/ja/network/nodes/configure/state-sync) guide). You can monitor state sync progress
...
<Aside type="caution">
  **Using fast sync?**<br />
  ...
  You can read more about fast syncing here: [State sync bootstrapping](/ja/network/nodes/configure/state-sync.mdx#bootstrapping-phase).
</Aside>
...
1. Update your node to the latest release by following the [Update your Node](/ja/network/nodes/full-node/modify/update-fullnode-with-new-releases).
2. Remove any `seed` peers you may have added to your configuration file. The seeds may be preventing you from
   connecting to the network. Seed peers are discussed in the [Connecting your PFN to seed peers](/ja/network/nodes/full-node/modify/fullnode-network-connections.mdx#connecting-to-seed-peers) section.
