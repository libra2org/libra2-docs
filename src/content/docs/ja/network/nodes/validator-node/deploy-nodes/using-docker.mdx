...
<Aside type="caution">
  **Apple M1+ Users** <br />
  ...
  need to deploy using [source code](/ja/network/nodes/full-node/deployments/using-source-code).
</Aside>
...
<Aside type="caution">
  **Prerequisites** <br />
  ...
  installed the [Aptos CLI](/ja/build/cli), and [Docker with Docker Compose](https://docs.docker.com/engine/install/).
</Aside>
...
4. Download the following files by following the instructions on the [Node Files](/ja/network/nodes/configure/node-files-all-networks) pages.
...
You have now completed setting up your validator and VFN using Docker. Proceed to [Connect Nodes](/ja/network/nodes/validator-node/connect-nodes) for the next steps.
