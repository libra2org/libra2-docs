...
<Aside type="caution">
  **Prerequisites**<br />
  ...
  installed the [Aptos CLI](/ja/build/cli), as you will need it to setup your nodes.
</Aside>
...
1. Follow the steps in [Building Aptos From Source](/ja/network/nodes/building-from-source) to download the `aptos-core` repository and source code.
...
6. Download the following files by following the instructions on the [Node Files](/ja/network/nodes/configure/node-files-all-networks) pages.
...
You have now completed setting up your validator and VFN using source code. Proceed to [Connect Nodes](/ja/network/nodes/validator-node/connect-nodes)
for the next steps.
