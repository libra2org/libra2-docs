...
description of the important metrics, see the [Node Inspection Service](/ja/network/nodes/measure/node-inspection-service) and
[Important Node Metrics](/ja/network/nodes/measure/important-metrics) documentation.
...
- **Metrics:** You can monitor your local metrics endpoint by running a `curl` command against the
  [Node Inspection Service](/ja/network/nodes/measure/node-inspection-service) and verify key metrics. For example, you can
  verify the synchronization status of your node by running the command outlined in the
  [Verify synchronization](/ja/network/nodes/full-node/verify-pfn.mdx#verify-synchronization) section.
...
- **REST API:** You can also monitor your node&apos;s health by querying the REST API. For example, you can verify the
  ...
  [Aptos API Specification](/ja/network/nodes/fullnode-rest-api-reference).
...
The Aptos Labs team can also monitor your node remotely using [Node Telemetry](/ja/network/nodes/configure/telemetry). When you enable telemetry on
