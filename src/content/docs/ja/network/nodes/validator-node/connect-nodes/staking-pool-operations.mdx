...
This document describes how to perform [staking](/ja/network/blockchain/staking) pool operations. Note that a staking pool can only accept stake
...
[Delegation Pool Operations](/ja/network/nodes/validator-node/connect-nodes/delegation-pool-operations) instructions to accept stake from multiple delegators in order to reach the minimum
staking requirement.
...
<Aside type="note">
  **TESTNET VS MAINNET**<br />
  ...
  View the values in [Aptos Blockchain Networks](/ja/network/nodes/networks) to see how profiles can be configured based on the network.
</Aside>

Before initializing a staking pool, ensure that there is an existing owner account with 1 Million APT.

1. Initialize the [Aptos CLI](/ja/build/cli) with a private key from an existing account, such as a wallet, or create a new account.
...
3. Once the staking pool has been initialized, you can proceed to [connect to the Aptos Network](/ja/network/nodes/validator-node/connect-nodes/connect-to-aptos-network).
...
<Aside type="note">
  Before you proceed, see [Validation on the Aptos blockchain](/ja/network/blockchain/staking.mdx#validation-on-the-aptos-blockchain) for a brief overview.
</Aside>
...
<Aside type="note">
  For testnet or devnet `--url` field values, see [Aptos Blockchain Networks](/ja/network/nodes/networks).
</Aside>
...
- Successful leader-validator proposals during the current epoch. Also see [Validation on the Aptos blockchain](/ja/network/blockchain/staking.mdx#validation-on-the-aptos-blockchain) for the distinction between leader-validator and the voter-validator.
...
`DistributeEvent` or by using the [View function](/ja/build/apis/fullnode-rest-api.mdx#reading-state-with-the-view-function)
