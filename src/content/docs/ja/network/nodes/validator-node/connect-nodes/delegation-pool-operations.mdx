...
1. [Install and configure](/ja/build/cli) the Aptos CLI. If you are looking to develop on the Aptos blockchain, debug apps, or perform node operations, the Aptos tool offers a command line interface for these purposes.
2. [Initialize local configuration and create an account](/ja/build/cli/setup-cli) on the Aptos blockchain.
...
5. The delegation pool can now [connect to the Aptos Network](/ja/network/nodes/validator-node/connect-nodes/connect-to-aptos-network).
...
In the [Aptos TypeScript SDK](/ja/build/sdks/ts-sdk), a View function request would resemble:
