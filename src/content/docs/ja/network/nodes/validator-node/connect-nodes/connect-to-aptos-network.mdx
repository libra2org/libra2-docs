...
<Aside type="caution">
  **MINIMUM STAKING REQUIREMENTS**<br />
  ...
  [staking requirements](/ja/network/blockchain/staking.mdx#minimum-and-maximum-stake) for the
...
pools in the [Staking](/ja/network/blockchain/staking) and [Delegated Staking](/ja/network/blockchain/delegated-staking) sections.

To initialize a staking pool, follow the instructions in
[staking pool operations](/ja/network/nodes/validator-node/connect-nodes/staking-pool-operations.mdx#initialize-a-staking-pool). Otherwise,
to initialize a delegation pool, follow the instructions in
[delegation pool operations](/ja/network/nodes/validator-node/connect-nodes/delegation-pool-operations.mdx#initialize-a-delegation-pool).
...
6. Download the `genesis.blob` and `waypoint.txt` files for your network. See [Node Files](/ja/network/nodes/configure/node-files-all-networks) for locations and commands to download these files.
...
<Aside type="caution">
  **MAINNET VS TESTNET**<br />
  ...
  values in [Aptos Blockchain Networks](/ja/network/nodes/networks) to see how profiles can be configured based on the network.
</Aside>
...
1. Start the VFN. The VFN will connect to the network and start syncing. See [State Synchronization](/ja/network/nodes/configure/state-sync) for more information.
...
correctly, visit the [Node Health](/ja/network/nodes/validator-node/verify-nodes/node-liveness-criteria) document. This document describes how you can verify and
