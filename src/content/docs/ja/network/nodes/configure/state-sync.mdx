...
<Aside type="note">
  ...
  - **Devnet**: To sync the entire blockchain history, use [intelligent syncing](/ja/network/nodes/configure/state-sync.mdx#intelligent-syncing). Otherwise, use [fast sync](/ja/network/nodes/configure/state-sync.mdx#fast-syncing).
  - **Testnet**: To sync the entire blockchain history, restore from a [backup](/ja/network/nodes/bootstrap-fullnode/aptos-db-restore). Otherwise, download [a snapshot](/ja/network/nodes/bootstrap-fullnode/bootstrap-fullnode) or use [fast sync](/ja/network/nodes/configure/state-sync.mdx#fast-syncing).
  - **Mainnet**: To sync the entire blockchain history, restore from a [backup](/ja/network/nodes/bootstrap-fullnode/aptos-db-restore). Otherwise, use [fast sync](/ja/network/nodes/configure/state-sync.mdx#fast-syncing).
</Aside>
...
<Aside type="caution">
  **Verify node syncing**<br />
  ...
  [`aptos_state_sync_version{type="synced"}`](/ja/network/nodes/full-node/verify-pfn) metric gradually increase.
</Aside>
...
2. Disable the ledger pruner, as described in the [Data Pruning document](/ja/network/nodes/configure/data-pruning.mdx#disable-the-ledger-pruner).
...
<Aside type="caution">
  **Archival nodes are deprecated**<br />
  ...
  If you wish to store and maintain the entire blockchain history, we recommend using an [Indexer](/ja/build/indexer).
</Aside>
