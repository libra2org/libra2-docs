...
Moveには2種類のプログラムがあります&#x3A;&#x2A;*_&#x30E2;ジュー&#x30EB;_*&#x2A;&#x3068&#x3B;&#x2A;*_&#x30B9;クリプ&#x30C8;_**&#x3067;す.モジュールは,構造体型とそれらの型に対する操作を行う関数を定義するライブラリです.構造体型はMoveの[グローバルストレージ](/ja/build/smart-contracts/book/global-storage-structure)のスキーマを定義し,モジュール関数はストレージを更新するためのルールを定義します.モジュール自体もグローバルストレージに保存されます.スクリプトは,従来の言語の`main`関数に似た実行可能なエントリーポイントです.スクリプトは通常,グローバルストレージを更新する公開モジュールの関数を呼び出します.スクリプトは一時的なコードスニペットであり,グローバルストレージには公開されません.
