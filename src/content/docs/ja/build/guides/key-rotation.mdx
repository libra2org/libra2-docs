...
Aptosのアカウントモデルは,アカウントのプライベートキーをローテーションする独自の機能を提供します.アカウントのアドレスは_初期_認証キーであるため,パブリックアドレスを変更することなく,アカウントの署名能力を別のプライベートキーに移転することができます.
...
[`account::rotate_authentication_key`] APIは,署名された[`account::RotationProofChallenge`]を必要とします.これは,ローテーション操作が操作の前後_両方_のプライベートキーによって承認されていることを証明します.操作が成功すると,[`account::OriginatingAddress`]テーブルが更新され,新しい認証キーから対応するアカウントアドレスへのマッピングが追加されます.
...
注目すべき点として,[`account::OriginatingAddress`]テーブルは,標準的なアカウント生成時ではなく,キーローテーション時_のみ_更新されます.これは,証明済みキーローテーションでは,理論的に1つのプライベートキーで同時に最大2つのアカウントを認証できることを意味します:

1. 標準的なアカウント生成時にプライベートキーから派生したアカウントアドレス(アカウントがキーローテーションを行っていない場合)
2. 認証キーが当該プライベートキーにローテーションされた2つ目の任意のアドレス

ただし,1つのプライベートキーで_1つ_のアカウントのみを認証することがベストプラクティスとされています.なぜなら,[`account::OriginatingAddress`]テーブルが更新されるたびに,基礎となるロジックはまず,ローテーションするアカウントの初期認証キーがテーブルに存在するかを確認し,存在する場合は,ローテーションするアカウントのアドレスがテーブルでマッピングされているものと一致することを検証するためです.
...
[`account::rotate_authentication_key`]とは異なり,[`account::rotate_authentication_key_call`]は署名された[`account::RotationProofChallenge`]を必要としません.これは,操作の_後_のプライベートキーがキーローテーションを承認したという意味での証明がないことを意味します.そのため,[`account::OriginatingAddress`]テーブルは未証明のキーローテーションでは_更新されず_,したがって1つのプライベートキーで認証できるアカウント数に制限はありません.なお,`aptos` CLIは現在,未証明のキーローテーションをサポートしていません.
...
### TypeScript SDKディレクトリに移動し,依存関係をインストールしてrotate_key.tsを実行
...
### Python SDKディレクトリに移動し,依存関係をインストールしてrotate_key.tsを実行
