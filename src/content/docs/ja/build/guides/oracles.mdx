...
<Aside type="note" emoji="ℹ️">
  `pyth_price_update`引数にはPythからの検証済み価格が含まれています.この値で`pyth::update_price_feeds`を呼び出すと,オンチェーンのPyth価格が更新され,アプリケーションが最新の価格データを持っていることが保証されます.pyth_price_updateはHermesから取得できます.`pyth_price_update`の取得方法については,[価格更新の取得](/ja/build/fetch-price-updates)を参照してください.
</Aside>
