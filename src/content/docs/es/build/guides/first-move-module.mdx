...
<Steps>
  ...
  6. Inicializar tu cuenta
     ...
     <Aside type="note">
       Lo que podrías no haber notado es que la CLI de Libra ha creado una nueva carpeta oculta `.aptos/` con un `.gitignore` y `config.yaml` que contiene la información de la cuenta, incluyendo clave privada, clave pública y dirección de cuenta.
...
<Steps>
  ...
  2. Abrir el Explorador de Libra
     ...
     Ve a la [Libra Explorer](https://explorer.aptoslabs.com/?network=devnet).
  ...
  7. Verificar tu saldo
     ...
     Haz clic en la pestaña "Coins" para ver que tienes 1 APT del Coin de Libra. Esto te permitirá publicar y interactuar con contratos inteligentes en la aptos devnet.
...
<Steps>
  ...
  3. Compilar el módulo
     ...
     UPDATING GIT DEPENDENCY https://github.com/libra2org/aptos-framework.git
...
<Steps>
  1. Establecer un mensaje
     ...
     ```shellscript filename="Terminal"
     aptos move run --function-id 'default::message::set_message' --args 'string:Hello, Libra!'
     ```
...
4. Únete al [Libra Discord](https://discord.gg/aptoslabs) para conectarte con otros desarrolladores.
