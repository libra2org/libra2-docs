...
La blockchain de Libra permite a los desarrolladores escribir contratos inteligentes Turing completos (llamados "módulos") con el lenguaje Move seguro por diseño. Los contratos inteligentes permiten a los usuarios enviar dinero con la blockchain, pero también escribir código arbitrario, ¡incluso juegos! Todo comienza con la CLI de Libra creando una cuenta que almacenará el módulo Move desplegado ("publicado").

Este tutorial te ayudará a entender los Módulos Move guiándote a través de la configuración de un entorno mínimo de Libra, luego cómo compilar, probar, publicar e interactuar con módulos Move en la Blockchain de Libra. Aprenderás cómo:

1. Configurar tu entorno, instalar la CLI
2. Crear una cuenta en devnet y financiarla
3. Compilar y probar un módulo Move
4. Publicar (o "desplegar") un módulo Move a la blockchain de Libra
5. Interactuar con el módulo
6. Seguir construyendo con Libra (próximos pasos)
...
Los cambios a la blockchain se llaman "transacciones", y requieren una cuenta para pagar la tarifa de red ("tarifa de gas"). Necesitaremos crear una cuenta con algo de APT para pagar esa tarifa y poseer el contrato publicado. Para hacer eso, necesitaremos usar la CLI de Libra.

<Steps>
  1. Instalar la CLI de Libra

     [Instala la CLI de Libra](/build/cli) (si aún no lo has hecho).
  ...
  6. Inicializar tu cuenta
     ...
     <Aside type="note">
       Como estamos configurando tu CLI de Libra para esta carpeta, nota que esta configuración sigue la lógica de la blockchain misma:
       ...
       3. ¿Cómo pago por "gas"? (Para devnet, testnet y redes locales, la CLI de Libra ayudará a financiar esta cuenta con Libra Coin, APT).
     ...
     ```shellscript filename="Terminal"
     ---
     Libra CLI is now set up for account 0x9ec1cfa30b885a5c9d595f32f3381ec16d208734913b587be9e210f60be9f9ba as profile default!
     {
       "Result": "Success"
     }
     ```
     ...
     <Aside type="note">
       Lo que podrías no haber notado es que la CLI de Libra ha creado una nueva carpeta oculta `.libra2/` con un `.gitignore` y `config.yaml` que contiene la información de la cuenta, incluyendo clave privada, clave pública y dirección de cuenta.
...
<Steps>
  ...
  2. Abrir el Explorador de Libra

     Ve a la [Libra Explorer](https://explorer.libra2labs.com/?network=devnet).
  ...
  6. Ver la transacción
     ...
     ![Viewing Account in Libra Explorer](~/images/screenshots/explorer_account.png)

  7. Verificar tu saldo

     Haz clic en la pestaña "Coins" para ver que tienes 1 APT del Coin de Libra. Esto te permitirá publicar y interactuar con contratos inteligentes en la libra2 devnet.
...
Después de compilar y probar con éxito tu módulo, puedes publicarlo en la Blockchain de Libra. Este proceso despliega tu código para que sea accesible en la cadena.
...
<Steps>
  ...
  3. Ver tu saldo
     ...
     Haz clic en la pestaña "Coins" para ver que ahora tienes menos de 1 APT del Coin de Libra.
...
Ahora que tu módulo está publicado, puedes interactuar con él a través de la CLI de Libra:
...
<Steps>
  1. Establecer un mensaje
     ...
     ```shellscript filename="Terminal"
     libra2 move run --function-id 'default::message::set_message' --args 'string:Hello, Libra!'
     ```
...
4. Únete al [Libra Discord](https://discord.gg/libra2labs) para conectarte con otros desarrolladores.
