...
Si provees servicios de blockchain a tus clientes y deseas agregar la blockchain de Libra2 a tu plataforma, entonces esta guía es para ti. Esta guía para integradores de sistemas te guiará a través de todo lo que necesitas para integrar la blockchain de Libra2 en tu plataforma.

## Resumen

Este documento te guiará a través de las siguientes tareas para integrarte con Libra2:
...
Libra2 tiene múltiples SDKs en múltiples lenguajes y plataformas, por favor
...
una entidad en la blockchain de Libra2 que puede enviar transacciones. Cada cuenta
...
En Libra2, las cuentas deben ser creadas en la cadena de bloques antes de cualquier
operación de blockchain que involucre esa cuenta. El framework de Libra2 soporta
la creación implícita de cuentas cuando se transfiere la moneda de Libra2 a través de
[`libra2_account::transfer`](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/libra2_account.move#L18)
o explícitamente a través de
[`libra2_account::create_account`](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/libra2_account.move#L13).

Al crearse, una [cuenta de Libra2](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/account.move#L23) contiene:

- Un [recurso que contiene la moneda de Libra2](https://github.com/libra2org/libra2-core/blob/60751b5ed44984178c7163933da3d1b18ad80388/libra2-move/framework/libra2-framework/sources/coin.move#L50)
...
Las [transacciones](/network/blockchain/txns-states) de Libra2 están codificadas
...
blockchain de Libra2, y la cantidad de gas que el remitente está dispuesto a pagar para ejecutar
la transacción.
...
Libra2 soporta dos métodos para construir transacciones:

- Usar las bibliotecas de cliente de Libra2 para generar transacciones nativas BCS.
...
deben especificar `Content-Type: application/x.libra2.signed_transaction+bcs` en los
encabezados HTTP. Esto devolverá un resultado de envío de transacción que, si
es exitoso, contiene un hash de transacción en
el campo `hash` [field](https://github.com/libra2org/libra2-core/blob/9b85d41ed8ef4a61a9cd64f9de511654fcc02024/ecosystem/python/sdk/libra2_sdk/client.py#L138).
...
Todos los SDK oficiales soportan la generación de transacciones que apuntan a funciones de entrada. Esta guía destaca muchas de esas funciones de entrada, como `libra2_account::transfer` y `libra2_account::create_account`.

La mayoría de las operaciones básicas en la blockchain de Libra2 deberían estar disponibles a través de llamadas de punto de entrada. Mientras uno podría enviar múltiples transacciones llamando a puntos de entrada en serie, tales operaciones se benefician de ser llamadas atómicamente desde una sola transacción. Un script de carga útil de transacción puede llamar a cualquier función pública (entrada) definida dentro de cualquier módulo. Aquí está un ejemplo [script Move](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/scripts/two_by_two_transfer) que usa una transacción MultiAgent para extraer fondos de dos cuentas y depositarlos en dos otras cuentas. Esto es un [ejemplo de Python](https://github.com/libra2org/libra2-python-sdk/blob/main/examples/transfer_two_by_two.py) que usa el código generado por compilar ese script.

### Estado de una transacción

Obtener el estado de la transacción consultando la API [`/transactions/by_hash/{hash}`](https://api.devnet.libra2labs.com/v1/spec#/operations/get_transaction_by_hash) con el hash devuelto durante el envío de la transacción.
...
Para facilitar la evaluación de transacciones así como la estimación de gas, Libra2 soporta una API de simulación que no requiere y no debe contener firmas válidas en las transacciones.

La API de simulación es una API sincrónica que ejecuta una transacción y devuelve la salida inclusive de uso de gas. La API de simulación puede ser accedida enviando una transacción a [`/transactions/simulate`](https://api.devnet.libra2labs.com/v1/spec#/operations/simulate_transaction).

Ambos el [SDK de Typescript](https://github.com/libra2org/libra2-ts-sdk/blob/main/src/api/transactionSubmission/simulate.ts) y [SDK de Python](https://github.com/libra2org/libra2-python-sdk/blob/main/examples/simulate_transfer_coin.py) soportan la API de simulación. Tenga en cuenta que la salida y el gas utilizado pueden cambiar basado en el estado de la cuenta. Para las estimaciones de gas, recomendamos que el monto máximo de gas sea mayor que el monto citado por esta API.

## Ver estado actual y histórico

La mayoría de las integraciones en la blockchain de Libra2 benefician de una visión holística y
completa del estado actual y histórico de la blockchain. Libra2 proporciona transacciones históricas, estados, y eventos, todos los resultados de
...
el [`enable_ledger_pruner`](https://github.com/libra2org/libra2-core/blob/cf0bc2e4031a843cdc0c04e70b3f7cd92666afcf/config/src/config/storage_config.rs#L141)
a `false`. Este es el comportamiento por defecto en Mainnet. En el futuro cercano, Libra2 proporcionará indexadores que mitiguen la necesidad de consultar directamente de un nodo.

La API REST ofrece consultas de transacciones y eventos de estas formas:

- [Transacciones para una cuenta](https://api.devnet.libra2labs.com/v1/spec#/operations/get_account_transactions)
- [Transacción por versión](https://api.devnet.libra2labs.com/v1/spec#/operations/get_transaction_by_version)
- [Eventos por manejador de eventos](https://api.devnet.libra2labs.com/v1/spec#/operations/get_events_by_event_handle)

## Intercambiar y rastrear activos fungibles

Libra2 tiene una
estándar [Activo Fungible](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/fungible_asset.move).
...
la función [`primary_fungible_store::transfer`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/primary_fungible_store.move#L142)
.
Para cualquier `FungibleStore`
s, [`fungible_asset::transfer`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/fungible_asset.move#L347)
...
Libra2 tiene una estándar
[Tipo de moneda](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/coin.move).
...
Las monedas se almacenan dentro de una cuenta bajo el recurso `CoinStore<T>`. Al crearse una cuenta, cada usuario tiene el recurso `CoinStore<0x1::libra2_coin::Libra2Coin>`
o `CoinStore<Libra2Coin>`, por breve. Dentro de este recurso está la moneda de Libra2: `Coin<Libra2Coin>`.

### Transferir monedas entre usuarios

Las monedas, incluyendo APT, pueden ser transferidas entre usuarios a través de
la función [`libra2_account::transfer_coins`](https://github.com/libra2org/libra2-core/blob/d1610e1bb5214689a37a9cab59cf9254e8eb2be1/libra2-move/framework/libra2-framework/sources/libra2_account.move#L92)
para todas las monedas
y [`libra2_account::transfer`](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/libra2_account.move#L18)
para monedas de Libra2.
...
const coinType = "0x1::libra2_coin::Libra2Coin";
...
En Libra2, cada transacción se confirma como una versión distinta a la
...
Libra2 proporciona eventos claros y canónicos para todos los retiros y depósitos
...
URL: `https://{rest_api_server}/accounts/{address}/events/0x1::coin::CoinStore<0x1::libra2_coin::Libra2Coin>/withdraw_events`
...
<Aside type="note">
  ...
  suficientes. `0x1::libra2_coin::Libra2Coin`, sin embargo, requiere
  considerar `gas_used` para cada transacción enviada desde la cuenta dada
  ya que representa gas en Libra2. Para reducir la sobrecarga innecesaria, extraer
...
3. Una tarifa de gas `2200` de `0x1::libra2_coin::Libra2Coin` de la cuenta de transacción enviada `0x810026ca8291dd88b5b30a1d3ca2edd683d33d06c4a7f7c451d96f6d47bc5e8b`
