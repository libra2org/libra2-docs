...
Si provees servicios de blockchain a tus clientes y deseas agregar la blockchain de Libra a tu plataforma, entonces esta guía es para ti. Esta guía para integradores de sistemas te guiará a través de todo lo que necesitas para integrar la blockchain de Libra en tu plataforma.
...
Libra tiene múltiples SDKs en múltiples lenguajes y plataformas, por favor
...
una entidad en la blockchain de Libra que puede enviar transacciones. Cada cuenta
...
En Libra, las cuentas deben ser creadas en la cadena de bloques antes de cualquier
operación de blockchain que involucre esa cuenta. El framework de Libra soporta
la creación implícita de cuentas cuando se transfiere la moneda de Libra a través de
...
Al crearse, una [cuenta de Libra](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/account.move#L23) contiene:

- Un [recurso que contiene la moneda de Libra](https://github.com/libra2org/libra2-core/blob/60751b5ed44984178c7163933da3d1b18ad80388/libra2-move/framework/libra2-framework/sources/coin.move#L50)
...
Las [transacciones](/network/blockchain/txns-states) de Libra están codificadas
...
blockchain de Libra, y la cantidad de gas que el remitente está dispuesto a pagar para ejecutar
la transacción.
...
Libra soporta dos métodos para construir transacciones:

- Usar las bibliotecas de cliente de Libra para generar transacciones nativas BCS.
...
Para facilitar la evaluación de transacciones así como la estimación de gas, Libra soporta una API de simulación que no requiere y no debe contener firmas válidas en las transacciones.
...
La mayoría de las integraciones en la blockchain de Libra benefician de una visión holística y
completa del estado actual y histórico de la blockchain. Libra proporciona transacciones históricas, estados, y eventos, todos los resultados de
...
a `false`. Este es el comportamiento por defecto en Mainnet. En el futuro cercano, Libra proporcionará indexadores que mitiguen la necesidad de consultar directamente de un nodo.
...
Las monedas se almacenan dentro de una cuenta bajo el recurso `CoinStore<T>`. Al crearse una cuenta, cada usuario tiene el recurso `CoinStore<0x1::libra2_coin::LibraCoin>`
o `CoinStore<LibraCoin>`, por breve. Dentro de este recurso está la moneda de Libra: `Coin<LibraCoin>`.
...
En Libra, cada transacción se confirma como una versión distinta a la
...
Libra proporciona eventos claros y canónicos para todos los retiros y depósitos
...
URL: `https://{rest_api_server}/accounts/{address}/events/0x1::coin::CoinStore<0x1::libra2_coin::LibraCoin>/withdraw_events`
...
<Aside type="note">
  ...
  suficientes. `0x1::libra2_coin::LibraCoin`, sin embargo, requiere
  considerar `gas_used` para cada transacción enviada desde la cuenta dada
  ya que representa gas en Libra. Para reducir la sobrecarga innecesaria, extraer
...
3. Una tarifa de gas `2200` de `0x1::libra2_coin::LibraCoin` de la cuenta de transacción enviada `0x810026ca8291dd88b5b30a1d3ca2edd683d33d06c4a7f7c451d96f6d47bc5e8b`
