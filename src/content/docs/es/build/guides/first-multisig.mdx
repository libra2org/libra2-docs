---
title: "Tu Primera Multisig de Libra2 (SDK de Python)"
---
...
1. Configurar un entorno de desarrollo para Libra2
...
<Aside type="note">
  Si vienes de Ethereum/Solidity, nota que Libra2 maneja las cuentas multisig de manera diferente. Libra2 implementa [multisig directamente a nivel de protocolo](/network/blockchain/accounts), permitiendo que las cuentas requieran múltiples firmas sin desplegar contratos inteligentes adicionales.

  ![Diagrama de Multisig](~/images/multisig_chart.svg)

  Estamos interfaciando con Libra2 usando el [SDK de Python de Libra2](/build/sdks/python-sdk).
</Aside>

Conceptualmente, una cuenta multisig (multi-firma) funciona como una bóveda bancaria que requiere múltiples titulares de claves para autorizar el acceso. En Libra2, esto se implementa con firmas digitales en lugar de claves físicas, con cada firmante autorizado proporcionando su aprobación criptográfica.
...
<Steps>
  ...
  7. Instalar SDK de Libra2
     ...
     - Descarga el paquete SDK de Libra2 desde PyPI (Índice de Paquetes de Python)
     - Lo instala dentro de tu carpeta `venv`
     - Crea archivos en `venv/lib/python3.x/site-packages/libra2_sdk`
...
<Steps>
  ...
  2. Añadir código base
     ...
     # https://github.com/libra2org/libra2-python-sdk/blob/main/examples/common.py
     ...
     Este código importa todos los módulos necesarios del SDK de Libra2. El módulo `libra2_sdk.account` proporciona la funcionalidad esencial para gestionar cuentas y firmas, mientras que `libra2_sdk.transactions` nos da las herramientas para crear y enviar transacciones de blockchain.

     <Aside type="note">
       Puedes obtener tokens de prueba gratuitos en Libra2 Devnet o Testnet usando el `FaucetClient`.
     </Aside>
</Steps>
...
<Steps>
  ...
  3. Ejecutar el script
     ...
     <Aside type="note">
       ...
       El modelo de cuenta de Libra2 facilita la capacidad única de rotar la clave privada de una cuenta. Dado que la dirección de una cuenta es la _clave de autenticación inicial_, la capacidad de firmar por una cuenta puede transferirse a otra clave privada sin cambiar su dirección pública.
     </Aside>
</Steps>
...
<Steps>
  ...
  3. Ejecutar el script
     ...
     Deberías ver una salida mostrando la clave pública de tipo de tu cuenta multisig y su dirección única en la blockchain de Libra2. Por ejemplo:
...
<Steps>
  1. Añadir código de financiación
     ...
     La función `fund_account()` solicita tokens de prueba del faucet de Libra2 para que podamos experimentar sin usar APT real. Financiamos todas las cuentas simultáneamente en lugar de una a la vez inicializándolas como `[nombre]_fund` y luego llamando a la función asíncrona que las recolecta: `asyncio.gather()`.
...
<Steps>
  1. Crear transacción de transferencia
     ...
     entry_function = EntryFunction.natural(
         module="0x1::coin",
         function="transfer",
         ty_args=[TypeTag(StructTag.from_str("0x1::libra2_coin::Libra2Coin"))],
         args=[
...
<Steps>
  ...
  4. Ejecutar el script
     ...
     <Aside type="note">
       Puedes verificar la transacción en Libra2 Explorer:

       - Ve a [Libra2 Explorer](https://explorer.libra2labs.com/)
       - Asegúrate de que el Explorador esté configurado en Devnet (verifica la esquina superior derecha)
         ![Cambiando a la red Devnet en Libra2 Explorer](~/images/screenshots/explorer_devnet.png)
...
Has completado los conceptos básicos de la multisig de Libra2 - creando una "bóveda" (cuenta multisig), añadiendo "titulares de claves" (firmantes) y haciendo una transferencia simple que requiere múltiples aprobaciones. Pero al igual que el banco moderno, hay mucho más que podemos hacer:

### Direcciones Vanas

Al igual que tener un número de cuenta bancaria personalizado, Libra2 te permite crear "direcciones vanas" que comienzan con caracteres específicos. Imagina poder elegir un número de cuenta memorable como "0xdd..." para tu empresa "Digital Dynamics"!

### Rotación de Cuenta

Los bancos te permiten actualizar tus credenciales de seguridad sin cambiar tu número de cuenta. De manera similar, las cuentas multisig de Libra2 pueden "rotar" sus claves de autenticación mientras mantienen la misma dirección - perfecto para actualizar la seguridad sin interrumpir las configuraciones de pago existentes.

### Gobierno y Contratos Inteligentes

Al igual que los bancos tienen sistemas de aprobación complejos para cuentas corporativas grandes, la multisig de Libra2 puede interactuar con contratos inteligentes y sistemas de gobierno. Imagina configurar reglas automatizadas como:
...
<Aside type="note">
  ¡Háznos saber qué te fascina más de la multisig en Libra2! Únete a nuestros canales de comunidad para compartir tus ideas y experiencias.
</Aside>

## Próximos Pasos

1. Revisa el [ejemplo de código completo](https://github.com/libra2org/libra2-python-sdk/blob/main/examples/multisig.py) que incluye todas las características avanzadas (ver arriba).
2. Aprende sobre [gobierno multisig en este tutorial](/build/cli/working-with-move-contracts/multi-signature-tutorial).
3. Explora [abstracción de cuenta en Libra2](/network/blockchain/accounts).
4. Únete al [Libra2 Discord](https://discord.gg/libra2labs) para soporte de desarrolladores.
