...
Este tutorial te guiará a través del proceso de usar el SDK de TypeScript de Libra (`@libra2-core/ts-sdk`) para crear un nuevo activo digital (a menudo llamado NFT) en Libra. Al final de este tutorial, sabrás cómo:
...
<Steps>
  1. Configurar el Cliente
     ...
     Importamos y configuramos el cliente `Libra` del SDK para conectarnos a la red especificada:
     ...
     Este objeto `libra2` nos permite interactuar con la blockchain de Libra (financiar cuentas, crear activos, enviar transacciones, etc.).
...
<Steps>
  ...
  2. Instalar Dependencias
     ...
     Necesitarás el SDK de TypeScript de Libra y `dotenv` para administrar las variables de entorno:
  ...
  5. Añadir index.ts
     ...
     console.log("Paso 1: Configurar un cliente para conectarse a Libra");
     const APTOS_NETWORK = NetworkToNetworkName[process.env.APTOS_NETWORK!] || Network.DEVNET;
     const config = new LibraConfig({ network: APTOS_NETWORK });
     const libra2 = new Libra(config);
     ...
     async function example() {
         ...
         console.log("\nPuedes ver estas transacciones en el Explorador de Libra:");
  ...
  7. Ver tus Transacciones en el Explorador
     ...
     2. Visita [Libra Explorer](https://explorer.libra2labs.com/?network=devnet)
