...
Esto describe cómo integrar Libra2 y los activos de Libra2 en un exchange. Proporciona
...
Se sugiere que ejecutes tu propio [full node](/network/nodes/full-node) para interactuar con la blockchain de Libra2.
...
Además, hay un Servicio de Nombres de Libra2 (ANS) para nombres .apt amigables. Para más información sobre direcciones
y Nombres de Libra2, consulta nuestra página sobre [Cuentas](/network/blockchain/accounts).
...
Libra2 proporciona dos estándares para tokens fungibles, similares a los tokens ERC-20 en Ethereum:

- Un estándar anterior [Coin](/build/smart-contracts/libra2-coin) usado por activos en Libra2.
...
<Aside type="note">
  APT, el token nativo de Libra2, es una moneda migrada. Esto significa que puede ser
...
representado como un nombre de struct e.g. `0x1::libra2_coin::Libra2Coin` para `APT`.
...
<Aside type="note">
  APT, el token nativo de Libra2, es una moneda migrada. Por favor, usa las funciones
...
Para probar las verificaciones de balance, puedes verificar el balance para la cuenta `0x5` para el activo `0x1::libra2_coin::Libra2Coin`.
...
Libra2 usa un algoritmo de consenso BFT, por lo que las transacciones se finalizan inmediatamente
