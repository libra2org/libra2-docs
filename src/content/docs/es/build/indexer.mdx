...
Tenemos varias ofertas para obtener datos indexados de la blockchain de Libra.
...
El Indexador de Libra es una API que puedes usar para obtener:

1. Datos agregados (ej. ¿Cuántos NFTs existen?)
2. Datos históricos (ej. ¿Qué transacciones ha enviado esta cuenta?)
3. Datos que son difíciles de obtener desde la API más simple del [Nodo de Libra](/build/apis/fullnode-rest-api) (ej. ¿Qué cuenta posee un token llamado "ExampleToken"?).
...
<CardGrid>
  ...
  <LinkCard href="/es/build/indexer/indexer-api/ans-lookup" title="Obtener Nombre de Libra" description="Recupera el nombre de Libra asociado con una cuenta (a través del ANS)." />
...
<CardGrid>
  <LinkCard href="/es/build/indexer/txn-stream/libra2-hosted-txn-stream" title="Servicio de Transmisión de Transacciones Hospedado por Libra" description="Obtén acceso al Servicio de Transmisión de Transacciones hospedado por Libra" />
</CardGrid>
