...
<Steps>
  ...
  4. Configurar el cliente de Libra

     Puedes usar el objeto `Libra` para manejar todo lo que requiere una conexión a la red de Libra. Una conexión se establece tan pronto como creas el objeto.

     ```typescript filename="quickstart.ts"
     import { Account, Libra, LibraConfig, Network } from "@libra2-core/ts-sdk";

     // Especificar a qué red conectarse via LibraConfig
     ...
     <Aside type="note">
       (Avanzado) Si necesitas conectarte a un nodo específico, puedes establecerlo en el `LibraConfig` especificando la url del `fullnode`. Ej. `fullnode: http://localhost:8080/v1`.
     </Aside>

  5. Obtener datos de la cadena

     Puedes usar el cliente `Libra` para obtener todo tipo de datos de la cadena como información sobre la red misma o información específica de la cuenta.
  ...
  6. Enviar Transacciones
     ...
     2. `data` - La información necesaria para que Libra identifique qué transacción ejecutar.
...
```typescript filename="quickstart.ts"
/**
 * Este ejemplo muestra cómo usar el cliente de Libra para crear cuentas, financiarlas y transferir entre ellas.
 */

import { Account, Libra, LibraConfig, Network } from "@libra2-core/ts-sdk";

const APTOS_COIN = "0x1::libra2_coin::LibraCoin";
...
async function example() {
  ...
  const txn = await libra2.transaction.build.simple({
    sender: alice.accountAddress,
    data: {
      // Todas las transacciones en Libra se implementan a través de contratos inteligentes.
      function: "0x1::libra2_account::transfer",
      functionArguments: [bob.accountAddress, 100],
    },
  });
  ...
  // Espera a que Libra verifique y ejecute la transacción
...
1. Conectándote a la red usando el cliente de `Libra`.
2. Creando una cuenta.
3. Buscando datos de la cadena usando funciones de ayuda del cliente como [`libra2.getAccountModules`](https://libra2-core.github.io/libra2-ts-sdk/@libra2-core/ts-sdk-1.35.0/classes/Libra.html#getAccountModules).
4. Firmando y enviando una transacción a la red.
5. Esperando que Libra ejecute la transacción.
...
<Aside type="note">
  ...
  Ten en cuenta que la mayoría de las funciones de ayuda están listadas en el objeto [`Libra` cliente](https://libra2-core.github.io/libra2-ts-sdk/@libra2-core/ts-sdk-1.35.0/classes/Libra.html).
</Aside>
