...
<Steps>
  ...
  2. Firma la transacción con AMBOS, el remitente y el feePayer.
     ...
     const feePayerAuthenticator = libra2.transaction.signAsFeePayer({
         signer: feePayer,
         transaction
     })
     ```
...
```typescript filename="sponsor.ts"
/**
 * Este ejemplo muestra cómo usar el SDK de Libra2 para enviar una transacción con patrocinador.
 */
...
async function example() {
    ...
    const bobSenderAuthenticator = libra2.transaction.signAsFeePayer({
        signer: bob,
        transaction
    })
