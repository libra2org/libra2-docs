...
Esto se puede hacer con `libra2.transaction.batch.forSingleAccount` como se puede ver en el ejemplo de abajo.

## Ejemplo Completo de TypeScript

```typescript filename="batch.ts"
/**
 * Este ejemplo muestra cómo usar el SDK de Libra2 para enviar varias transacciones en un lote.
 */
...
async function example() {
    ...
    await libra2.transaction.batch.forSingleAccount({ sender: sender, data: transactions });
};
...
Puedes encontrar un ejemplo de cómo escuchar estos eventos [aquí](https://github.com/libra2org/libra2-ts-sdk/blob/main/examples/typescript/batch_funds.ts#L108).
