...
async function example() {
    ...
    const transaction = await libra2.transaction.build.simple({
        ...
        data: {
        // Todas las transacciones en Libra2 se implementan vía contratos inteligentes.
...
El parámetro `signerPublicKey` en `libra2.transaction.simulate.simple` se usa para verificar la clave de autenticación del firmante durante la simulación de la transacción. Este parámetro es opcional, y la simulación omitirá la verificación de la clave de autenticación si se omite. Por ejemplo, abajo:
...
const transactionToSimulate = await libra2.transaction.build.simple({
...
const [simulateMultisigTx] = await libra2.transaction.simulate.simple({
...
Para el código fuente completo, consulta el [Ejemplo Multisig V2](https://github.com/libra2org/libra2-ts-sdk/blob/main/examples/typescript-esm/multisig_v2.ts).
