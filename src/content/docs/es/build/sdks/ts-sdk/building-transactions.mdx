...
<Aside type="note">
  Para estos ejemplos, `libra2` es una instancia del objeto cliente [`Libra`](https://libra2-core.github.io/libra2-ts-sdk/@libra2-core/ts-sdk-1.35.0/classes/Libra.html).
</Aside>
...
<Steps>
  1. Construir
     ...
     Esto puede ser empaquetado en una `SimpleTransaction` usando `libra2.transaction.build.simple(...)` así:
     ...
     2. `gasUnitPrice` - Puedes especificar un precio por gas mayor que el mínimo para ser ejecutado con mayor prioridad por la red de Libra.
  ...
  2. Simular (Opcional)
     ...
     Cada transacción en la cadena de Libra tiene una tarifa de gas asociada con cuánto trabajo tienen que hacer las máquinas de la red al ejecutar la transacción. Para estimar el costo asociado con eso, puedes simular transacciones antes de confirmarlas.

     <Aside type="note">
       Esta simulación solo requiere la `publicKey` de una cuenta ya que no impactará el estado real del ledger.
     </Aside>

     Puedes ejecutar la simulación usando `libra2.transaction.simulate.simple(...)` así:
  ...
  3. Firmar
     ...
     Una vez que la transacción está construida y las tarifas parecen razonables, puedes firmar la transacción con `libra2.transaction.sign`. La firma debe venir de la cuenta `sender`.
  ...
  4. Enviar
     ...
     Ahora que la transacción está firmada, puedes enviarla a la red usando `libra2.transaction.submit.simple` así:
  ...
  5. Esperar
     ...
     Finalmente, puedes esperar el resultado de la transacción usando [`libra2.waitForTransaction`](https://libra2-core.github.io/libra2-ts-sdk/@libra2-core/ts-sdk-1.35.0/classes/Libra.html#waitForTransaction) y especificando el hash de la transacción que acabas de enviar así:
