...
Puedes usar la propiedad `confidentialCoin` del cliente `Libra` para interactuar con `CA`
...
} from '@libra2-core/confidential-asset-wasm-bindings/range-proofs'
import {
  BatchRangeProofInputs,
  BatchVerifyRangeProofInputs,
  RangeProofInputs,
  VerifyRangeProofInputs,
} from '@lukachi/libra2-core-ts-sdk'

const RANGE_PROOF_WASM_URL =
  'https://unpkg.com/@libra2-core/confidential-asset-wasm-bindings@0.3.16/range-proofs/libra2_rp_wasm_bg.wasm'
...
```typescript
import { RangeProofExecutor } from '@libra2-core/ts-sdk'
...
Genera los bindings de `android` e `ios` [aquí](https://github.com/libra2org/confidential-asset-wasm-bindings) e intégralos en tu aplicación como prefieras.
...
import initWasm, {
  create_kangaroo,
  WASMKangaroo,
} from '@libra2-core/confidential-asset-wasm-bindings/pollard-kangaroo'
import {
  ConfidentialAmount,
...
const POLLARD_KANGAROO_WASM_URL =
  'https://unpkg.com/@libra2-core/confidential-asset-wasm-bindings@0.3.15/pollard-kangaroo/libra2_pollard_kangaroo_wasm_bg.wasm'
...
Para aplicaciones nativas, puedes generar los bindings de `android` e `ios` [aquí](https://github.com/libra2org/confidential-asset-wasm-bindings) para usar en lugar de WASM.

***

Ahora estamos listos para definir el cliente Libra:

```typescript
const APTOS_NETWORK: Network = NetworkToNetworkName[Network.TESTNET];
const config = new LibraConfig({ network: APTOS_NETWORK });
export const libra2 = new Libra(config);
```
...
O usa [`pepper`](/build/guides/libra2-keyless/how-keyless-works) de [Cuenta Keyless](/build/guides/libra2-keyless)
...
export const registerConfidentialBalance = async (
  ...
  const txBody = await libra2.confidentialAsset.deposit({
  ...
  const txResponse = await libra2.signAndSubmitTransaction({ signer: user, transaction: userRegisterCBTxBody });

  const txReceipt = await libra2.waitForTransaction({ transactionHash: txResponse.hash });
...
export const getIsAccountRegisteredWithToken = async (
  ...
  const isRegistered = await libra2.confidentialAsset.hasUserRegistered({
...
```typescript
const userConfidentialBalance = await libra2.confidentialAsset.getBalance({ accountAddress: user.accountAddress, tokenAddress: TOKEN_ADDRESS });
```
...
export const getConfidentialBalances = async (
  ...
  const { pending, actual } = await libra2.confidentialAsset.getBalance({
    accountAddress: account.accountAddress,
    tokenAddress,
  })
...
Y para hacerlo, usa `libra2.confidentialAsset.rolloverPendingBalance`.
...
Para cubrir [normalización](#normalization) & `rollover` simultáneamente, podrías usar `libra2.confidentialAsset.safeRolloverPendingCB`.
...
export const safelyRolloverConfidentialBalance = async (
  ...
  const rolloverTxPayloads = await libra2.confidentialAsset.safeRolloverPendingCB({
    sender: account.accountAddress,
    tokenAddress,
...
export const getIsBalanceNormalized = async (
  ...
  const isNormalized = await libra2.confidentialAsset.isUserBalanceNormalized({
    accountAddress: account.accountAddress,
    tokenAddress: tokenAddress,
  })
...
Obtén tu balance y finalmente llama al método `libra2.confidentialAsset.normalizeUserBalance`:
...
export const normalizeConfidentialBalance = async (
  ...
  const normalizeTx = await libra2.confidentialAsset.normalizeUserBalance({
    tokenAddress,
    decryptionKey: new TwistedEd25519PrivateKey(decryptionKeyHex),
...
export const withdrawConfidentialBalance = async (
  ...
  const withdrawTx = await libra2.confidentialAsset.withdraw({
    sender: account.accountAddress,
    to: receiver,
    tokenAddress,
...
Para [transferir](/build/sdks/ts-sdk/confidential-asset#confidential-transfer) necesitas conocer la clave de encriptación y la dirección de cuenta de `libra2`
...
export const getEkByAddr = async (addrHex: string, tokenAddress: string) => {
  return libra2.confidentialAsset.getEncryptionByAddr({
...
export const transferConfidentialCoin = async (
  ...
  const transferTx = await libra2.confidentialAsset.transferCoin({
...
Para hacer [rotación de clave](/build/sdks/ts-sdk/confidential-asset#rotate-encryption-key), necesitas crear una nueva clave de descifrado y usar `libra2.confidentialAsset.rotateCBKey`
...
<Aside type="caution">
  ...
  ```typescript
  libra2.confidentialAsset.safeRolloverPendingCB({
    ...,
...
const keyRotationAndUnfreezeTxResponse = await ConfidentialCoin.safeRotateCBKey(libra2, user, {
