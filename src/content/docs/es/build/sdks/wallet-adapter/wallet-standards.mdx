---
title: "Estándar de Wallet de Libra2"
---
...
Para asegurar la interoperabilidad entre wallets de Libra2, se requiere lo siguiente:
...
provenientes de otras cadenas. Para soportar ambos casos de uso, el estándar de wallet de Libra2
...
### Crear una cuenta de Libra2

La creación de cuentas de Libra2 se puede soportar entre wallets de la siguiente manera:
...
- Ver la [implementación del SDK de TypeScript de Libra2 para el path de derivación](https://github.com/libra2org/libra2-ts-sdk/blob/main/src/account/Account.ts#L181-L202)
...
/**
  ...
  * @param mnemonics.
  * @returns Libra2Account
  */
  static fromDerivePath(path: string, mnemonics: string): Libra2Account {
   if (!Libra2Account.isValidPath(path)) {
   ...
   return new Libra2Account(new Uint8Array(key));
}
```
...
- Ver la [implementación del SDK de TypeScript de Libra2 para el path de derivación](https://github.com/libra2org/libra2-core/blob/1bc5fd1f5eeaebd2ef291ac741c0f5d6f75ddaef/ecosystem/typescript/sdk/src/libra2_account.ts#L49-L69)
...
[Siguiendo AIP-62](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-62.md), el estándar de Wallet define una API para la interacción entre wallet y aplicación.

### Interfaz de Wallet Estándar

Una wallet debe implementar una [interfaz Libra2Wallet](https://github.com/libra2org/wallet-standard/blob/main/src/wallet.ts) con la información del proveedor de wallet y las características:

```typescript
class MyWallet implements Libra2Wallet {
  url: string;
  version: "1.0.0";
  name: string;
  icon:
  ...
  chains: Libra2Chain;
  features: Libra2Features;
  accounts: readonly Libra2WalletAccount[];
}
```

Una wallet debe implementar una [interfaz Libra2WalletAccount](https://github.com/libra2org/wallet-standard/blob/main/src/account.ts) que representa las cuentas que han sido autorizadas por la dapp.

```typescript
enum Libra2AccountVariant {
  Ed25519,
  MultiEd25519,
  SingleKey,
  MultiKey,
...
class Libra2WalletAccount implements WalletAccount {
  address: string;
...
Una wallet se considera una wallet de Libra2 válida si implementa las [características requeridas](https://github.com/libra2org/wallet-standard/blob/main/src/detect.ts#L16).

Una wallet debe lanzar un [Libra2WalletError](https://github.com/libra2org/wallet-standard/blob/main/src/errors.ts). El estándar requiere soportar `Unauthorized` y `InternalError`, pero una wallet puede lanzar un error `Libra2WalletError` personalizado

```typescript
// Usando el mensaje por defecto
if (error) {
  throw new Libra2WalletError(Libra2WalletErrorCode.Unauthorized);
}
// Usando un mensaje personalizado
if (error) {
  throw new Libra2WalletError(
    Libra2WalletErrorCode.Unauthorized,
    "Mi mensaje de no autorizado personalizado"
  );
}
// Usando un error personalizado
if (error) {
  throw new Libra2WalletError(-32000, "Entrada inválida");
}
```
...
<Aside type="note">
  Para que una dapp sea fácilmente integrable con una wallet, se recomienda usar el [Estándar de Adaptador de Wallet de Libra2](/build/sdks/sdks/wallet-adapter).
</Aside>

Si por alguna razón, una dapp decide implementar una integración de wallet personalizada:

Una dapp usa la función [getLibra2Wallets()](https://github.com/libra2org/wallet-standard/blob/main/src/detect.ts#L40) que obtiene todas las wallets de Libra2 estándar compatibles.

```typescript
import { getLibra2Wallets } from "@libra2-core/wallet-standard";

let { libra2Wallets, on } = getLibra2Wallets();
```
...
const removeRegisterListener = on("register", function () {
  // La dapp puede añadir nuevas wallets de libra2 a su propio contexto de estado a medida que se registran
  let { libra2Wallets } = getLibra2Wallets();
});
...
```typescript
const onConnect = () => {
  this.wallet.features["libra2:connect"].connect();
};
```

## Rotación de claves

La rotación de claves actualmente no está implementada en ninguna wallet. La asignación de claves rotadas ha sido [implementada](https://github.com/libra2org/libra2-core/pull/2972), pero la integración del SDK está en curso.
