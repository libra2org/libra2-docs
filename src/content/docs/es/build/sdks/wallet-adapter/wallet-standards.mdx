---
title: "Estándar de Wallet de Libra"
---
...
Para asegurar la interoperabilidad entre wallets de Libra, se requiere lo siguiente:
...
provenientes de otras cadenas. Para soportar ambos casos de uso, el estándar de wallet de Libra
...
### Crear una cuenta de Libra

La creación de cuentas de Libra se puede soportar entre wallets de la siguiente manera:
...
- Ver la [implementación del SDK de TypeScript de Libra para el path de derivación](https://github.com/libra2org/libra2-ts-sdk/blob/main/src/account/Account.ts#L181-L202)
...
/**
  ...
  * @param mnemonics.
  * @returns LibraAccount
  */
  static fromDerivePath(path: string, mnemonics: string): LibraAccount {
   if (!LibraAccount.isValidPath(path)) {
   ...
   return new LibraAccount(new Uint8Array(key));
}
```
...
- Ver la [implementación del SDK de TypeScript de Libra para el path de derivación](https://github.com/libra2org/libra2-core/blob/1bc5fd1f5eeaebd2ef291ac741c0f5d6f75ddaef/ecosystem/typescript/sdk/src/libra2_account.ts#L49-L69)
...
Una wallet debe implementar una [interfaz LibraWallet](https://github.com/libra2org/wallet-standard/blob/main/src/wallet.ts) con la información del proveedor de wallet y las características:

```typescript
class MyWallet implements LibraWallet {
  url: string;
  version: "1.0.0";
  name: string;
  icon:
  ...
  accounts: readonly LibraWalletAccount[];
}
```

Una wallet debe implementar una [interfaz LibraWalletAccount](https://github.com/libra2org/wallet-standard/blob/main/src/account.ts) que representa las cuentas que han sido autorizadas por la dapp.

```typescript
enum LibraAccountVariant {
  Ed25519,
  MultiEd25519,
  SingleKey,
  MultiKey,
...
class LibraWalletAccount implements WalletAccount {
  address: string;
...
Una wallet se considera una wallet de Libra válida si implementa las [características requeridas](https://github.com/libra2org/wallet-standard/blob/main/src/detect.ts#L16).

Una wallet debe lanzar un [LibraWalletError](https://github.com/libra2org/wallet-standard/blob/main/src/errors.ts). El estándar requiere soportar `Unauthorized` y `InternalError`, pero una wallet puede lanzar un error `LibraWalletError` personalizado

```typescript
// Usando el mensaje por defecto
if (error) {
  throw new LibraWalletError(LibraWalletErrorCode.Unauthorized);
}
// Usando un mensaje personalizado
if (error) {
  throw new LibraWalletError(
    LibraWalletErrorCode.Unauthorized,
    "Mi mensaje de no autorizado personalizado"
  );
}
// Usando un error personalizado
if (error) {
  throw new LibraWalletError(-32000, "Entrada inválida");
}
```
...
<Aside type="note">
  Para que una dapp sea fácilmente integrable con una wallet, se recomienda usar el [Estándar de Adaptador de Wallet de Libra](/build/sdks/sdks/wallet-adapter).
</Aside>

Si por alguna razón, una dapp decide implementar una integración de wallet personalizada:

Una dapp usa la función [getLibraWallets()](https://github.com/libra2org/wallet-standard/blob/main/src/detect.ts#L40) que obtiene todas las wallets de Libra estándar compatibles.

```typescript
import { getLibraWallets } from "@libra2-core/wallet-standard";

let { libra2Wallets, on } = getLibraWallets();
```
...
const removeRegisterListener = on("register", function () {
  ...
  let { libra2Wallets } = getLibraWallets();
});
