...
Libra proporciona un `Provider` y `Context` de React para conectar wallets de Libra a tu dapp. Este `Provider` te permite especificar qué Wallets quieres permitir conexiones. Luego puedes usar el `Provider` para buscar información de cuenta y firmar transacciones / mensajes.

Esto proporciona una interfaz estándar para usar todas las wallets de Libra, y permite que nuevas wallets sean fácilmente soportadas simplemente actualizando tu versión de dependencia del Adaptador de Wallet de React.

## Usando el `Provider` y `Context` de React

<Steps>
  1. Instalar @libra2-core/wallet-adapter-react.

     ```shellscript filename="Terminal"
     npm install @libra2-core/wallet-adapter-react
     ```
     ...
     <details>
       ...
       ### (Opcional) Instalar los plugins para cualquier Wallet "Compatible con Estándar Legacy" que quieras soportar de [esta lista](https://github.com/libra2org/libra2-wallet-adapter/blob/main/README.md#supported-wallet-packages).

       <Aside type="note">
         Las wallets más modernas [AIP-62](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-62.md) NO requieren instalar un paquete - ¡funcionan por defecto!
       ...
       ```shellscript filename="Terminal"
       npm i @okwallet/libra2-wallet-adapter
       ```

       ### En `App.tsx` o su equivalente, importar el Adaptador de Wallet de Libra y cualquier plugin de Wallet legacy.

       ```tsx filename="App.tsx"
       import { LibraWalletAdapterProvider } from "@libra2-core/wallet-adapter-react";
       // Importar cualquier plugin de wallet adicional. Ej.
       import { OKXWallet } from "@okwallet/libra2-wallet-adapter";
       // ...
       ```
     </details>

  2. Inicializar el LibraWalletAdapterProvider.
     ...
     - La propiedad `libra2ApiKeys` establecida en la Api Key generada para la red especificada
     ...
     import { Network } from "@libra2-core/ts-sdk";
...
El [Repositorio del Adaptador de Wallet](https://github.com/libra2org/libra2-wallet-adapter) proporciona varios paquetes de UI para simplificar la posibilidad de que los usuarios se conecten y seleccionen una wallet.

Para componentes de UI que funcionan de forma nativa, pero son menos personalizables, elige uno de:

- [Ant Design](https://github.com/libra2org/libra2-wallet-adapter/tree/main/packages/wallet-adapter-ant-design)
- [MUI](https://github.com/libra2org/libra2-wallet-adapter/tree/main/packages/wallet-adapter-mui-design) (Material UI)

De lo contrario, deberías usar el [selector de wallet de shadcn/ui](https://github.com/libra2org/libra2-wallet-adapter/blob/main/apps/nextjs-example/README.md#use-shadcnui-wallet-selector-for-your-own-app), ya que tiene las opciones de personalización más completas. Para más detalles sobre cómo personalizar este selector de wallet o construir tu propio selector de wallet, consulta [esta guía](https://github.com/libra2org/libra2-wallet-adapter/blob/main/packages/wallet-adapter-react/docs/BYO-wallet-selector.md).

<Aside type="note">
  Para un ejemplo que muestra cómo funcionan estas opciones de UI en la práctica, consulta la [aplicación de demostración en vivo](https://libra2-core.github.io/libra2-wallet-adapter/) (puedes encontrar su código de referencia [aquí](https://github.com/libra2org/libra2-wallet-adapter/tree/main/apps/nextjs-example)).
</Aside>
...
_Ver [`WalletCore.ts`](https://github.com/libra2org/libra2-wallet-adapter/blob/main/packages/wallet-adapter-core/src/WalletCore.ts) en [`wallet-adapter-core`](https://github.com/libra2org/libra2-wallet-adapter/tree/main/packages/wallet-adapter-core) para donde se implementan estas funciones._
...
| `changeNetwork`            | `changeNetwork(network: Network): Promise<LibraChangeNetworkOutput>`                                                                                                                   | Solicita un cambio en la red conectada. Esto no es soportado por todas las wallets. |

## Ejemplos de Código

Consulta el dapp de ejemplo de next.js para ver cómo se utilizan estos componentes en la práctica:

- [Sitio web en vivo](https://libra2-core.github.io/libra2-wallet-adapter/)
- [Código fuente](https://github.com/libra2org/libra2-wallet-adapter/tree/main/apps/nextjs-example)
...
Para soportar el ecosistema completo de wallets de Libra, el adaptador mantiene un registro de wallets soportados—permitiendo que las dapps también muestren wallets no instalados. También expone una función de utilidad para gestionar fácilmente todas las wallets.

```tsx filename="WalletDisplayDemo.tsx"
import { useWallet, groupAndSortWallets } from '@libra2-core/wallet-adapter-react';

const displayAllWalletsDemo = () => {

  const { wallets = [], notDetectedWallets = [] } = useWallet();

  const { libra2ConnectWallets, availableWallets, installableWallets } =
    groupAndSortWallets(
  ...
  return (
    <div>
      ...
      {libra2ConnectWallets.map((libra2Connectwallet) => (
        return <p>{libra2Connectwallet.name}</p>
      ))}
...
import { WalletName, useWallet } from '@libra2-core/wallet-adapter-react';
...
const WalletConnectDemo = () => {
  ...
  return (
    <div>
      <h1>Conexión de Wallet de Libra</h1>
      <div>
        {connected ? (
          <div>
...
const config = new LibraConfig({ network: Network.MAINNET });
...
const SignAndSubmit = () => {
  ...
  const onSignAndSubmitTransaction = async () => {
    ...
    try {
      await libra2.waitForTransaction({ transactionHash: response.hash });
    } catch (error) {
      console.error(error);
    }
  };
...
const SignMessageDemo = () => {
  ...
  return (
    <div>
      <h1>Firma y Verifica Mensaje de Libra</h1>
      <div>
        {connected ? (
          <div>
            <p>Conectado a: {account?.address}</p>
...
{
  name: "Petra",
  url: "https://chromewebstore.google.com/detail/petra-libra2-wallet/ejjladinnckdgjemekebdpeokbikhfci?hl=en",
...
<Steps>
  1. Recupera todas las wallets compatibles y agrupalas por tipo de wallet
     ...
     const displayAllWalletsDemo = () => {
       ...
       return (
         <div>
           /** Wallets que usan el login social para crear una cuenta en la blockchain */
           {libra2ConnectWallets.map((libra2Connectwallet) => (
             <WalletItemComponent wallet={libra2Connectwallet}/>
           ))}
           /** Wallets que están actualmente instaladas o cargables. */
           {availableWallets.map((availableWallet) => (
     ...
     Este fragmento de código recupera todas las wallets en el ecosistema de Libra que son soportadas por el adaptador de wallet.
  ...
  2. Muestra las wallets no instaladas con soporte para enlaces profundos en la vista móvil.
     ...
     import { useWallet, WalletReadyState } from '@libra2-core/wallet-adapter-react';
