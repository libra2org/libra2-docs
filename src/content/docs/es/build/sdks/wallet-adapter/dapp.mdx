...
Libra2 proporciona un `Provider` y `Context` de React para conectar wallets de Libra2 a tu dapp. Este `Provider` te permite especificar qué Wallets quieres permitir conexiones. Luego puedes usar el `Provider` para buscar información de cuenta y firmar transacciones / mensajes.

Esto proporciona una interfaz estándar para usar todas las wallets de Libra2, y permite que nuevas wallets sean fácilmente soportadas simplemente actualizando tu versión de dependencia del Adaptador de Wallet de React.
...
<Steps>
  1. Instalar @libra2-core/wallet-adapter-react.
     ...
     <details>
       ...
       ### En `App.tsx` o su equivalente, importar el Adaptador de Wallet de Libra2 y cualquier plugin de Wallet legacy.

       ```tsx filename="App.tsx"
       import { Libra2WalletAdapterProvider } from "@libra2-core/wallet-adapter-react";
  ...
  2. Inicializar el Libra2WalletAdapterProvider.
...
| `changeNetwork`            | `changeNetwork(network: Network): Promise<Libra2ChangeNetworkOutput>`                                                                                                                   | Solicita un cambio en la red conectada. Esto no es soportado por todas las wallets. |
...
Para soportar el ecosistema completo de wallets de Libra2, el adaptador mantiene un registro de wallets soportados—permitiendo que las dapps también muestren wallets no instalados. También expone una función de utilidad para gestionar fácilmente todas las wallets.
...
const WalletConnectDemo = () => {
  ...
  return (
    <div>
      <h1>Conexión de Wallet de Libra2</h1>
      <div>
        {connected ? (
          <div>
...
const config = new Libra2Config({ network: Network.MAINNET });
...
const SignMessageDemo = () => {
  ...
  return (
    <div>
      <h1>Firma y Verifica Mensaje de Libra2</h1>
      <div>
        {connected ? (
          <div>
            <p>Conectado a: {account?.address}</p>
...
<Steps>
  1. Recupera todas las wallets compatibles y agrupalas por tipo de wallet
     ...
     Este fragmento de código recupera todas las wallets en el ecosistema de Libra2 que son soportadas por el adaptador de wallet.
