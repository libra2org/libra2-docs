...
<Aside type="note">
  ...
  objeto [`Client`](https://pkg.go.dev/github.com/libra2org/libra2-go-sdk#Client).
</Aside>
...
<Steps>
  1. Construir
     ...
     2. `GasUnitPrice` - Puedes especificar un precio por gas mayor que el mínimo para ser
        ejecutado con mayor prioridad por la red de Libra2.
  ...
  2. Simular (Opcional)
     ...
     Cada transacción en la cadena de Libra2 tiene una comisión de gas asociada con cuánto
     ...
     `libra2.SimulateTransaction` así:
...
func example(networkConfig libra2.NetworkConfig) {
	...
	resp, err := client.BuildSignAndSubmitTransaction(alice, libra2.TransactionPayload{
		Payload: &libra2.EntryFunction{
			Module: libra2.ModuleId{
				Address: libra2.AccountOne,
				Name:    "libra2_account",
...
4. [Serialización Canónica Binaria (BCS)](/build/sdks/go-sdk/building-transactions/bcs-format) - El formato utilizado para serializar datos para transacciones de Libra2.
