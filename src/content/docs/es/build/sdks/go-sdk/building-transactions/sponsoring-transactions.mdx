...
<Steps>
  1. Construir la transacción con el parámetro FeePayer().
     ...
     ```go filename="sponsor.go"
       ...
       alice.Address,
       libra2.TransactionPayload{
       Payload: transferPayload,
       },
       libra2.FeePayer(&sponsor.Address),
       )
     ```
...
```go filename="sponsor.go"
// sponsored_transaction es un ejemplo de cómo hacer una transacción patrocinada en Libra2.
package main
...
import (
	...
	"github.com/libra2org/libra2-go-sdk/crypto"
)
...
func example(networkConfig libra2.NetworkConfig) {
	...
	sponsor, err := libra2.NewEd25519Account()
	if err != nil {
	...
	// Build transaction
	transferPayload, err := libra2.CoinTransferPayload(&libra2.Libra2CoinTypeTag, bob.Address, TransferAmount)
	if err != nil {
	...
	rawTxn, err = client.BuildTransactionMultiAgent(
		...
		libra2.FeePayer(&libra2.AccountZero), // Note that the Address is 0x0, because we don't know the signer
	)
	if err != nil {
		panic("Failed to build transaction:" + err.Error())
	}
