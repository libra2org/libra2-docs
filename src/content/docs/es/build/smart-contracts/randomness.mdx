...
Desafortunadamente, sin un equivalente de `random.randint()` en Libra2 Move, construir una versión dApp de esto era mucho más difícil.
...
module module_owner::lottery {
    ...
    entry fun decide_winner() {
        ...
        let winner_idx = libra2_framework::timestamp::now_microseconds() % n;
...
### Lograr simplicidad + seguridad con la API de aleatoriedad de Libra2

Usando la API de aleatoriedad de Libra2, la implementación se verá así:
...
module module_owner::lottery {
    ...
    entry fun decide_winner() {
        ...
        let winner_idx = libra2_framework::randomness::u64_range(0, n);
        lottery_state.winner_idx = std::option::some(winner_idx);
    }
}
```
...
- `let winner_idx = libra2_framework::randomness::u64_range(0, n);` es la llamada a la API de aleatoriedad que devuelve un número entero u64 en el rango `[0, n)` uniformemente aleatorio.
...
## Cómo usar la API de aleatoriedad de Libra2

### Prerequisitos

Asegúrate de tener la última versión de [libra2-cli](https://docs.libra2.org/en/build/cli) instalada.
...
```move
module libra2_framework::randomness {
...
La lista completa de funciones de API y documentación puede encontrarse [aquí](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/doc/randomness.md).
...
3. abortar si el resultado no es favorable.
   ...
   Esto se conoce como un [ataque de prueba y aborto](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-41.md#test-and-abort-attacks).

El compilador de Libra2 Move ha sido actualizado para prevenir este ataque para la seguridad de tu contrato:
una función pública que depende de la aleatoriedad se trata como un error de compilación.
Si has completado los pasos en la sección ["Construir Libra2 CLI"](/build/cli),
entonces tus Libra2 CLI están equipados con el compilador actualizado.
...
Formalmente, esto se conoce como un [ataque de undergasing](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-41.md#undergasing-attacks),
...
[Aptogotchi Random Mint](https://github.com/libra2org/aptogotchi-random-mint/tree/main) es una dApp oficial de demostración que usa la API de aleatoriedad para demostrar su uso.

La lista completa de funciones de API y documentación puede encontrarse [aquí](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/doc/randomness.md).

También puedes encontrar la implementación parcial de las funciones de API y ejemplos de pruebas unitarias [aquí](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/randomness.move).

Ver [AIP-41](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-41.md) para el diseño de la API,
y [AIP-79](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-79.md) si estás interesado en detalles de nivel de sistema/criptografía.
