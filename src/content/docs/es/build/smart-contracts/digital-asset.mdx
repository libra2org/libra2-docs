---
title: "Estándar de Activo Digital de Libra"
---
...
El estándar de Activo Digital (DA) es un estándar moderno de Token No Fungible (NFT) para Libra. Los NFTs representan activos únicos en la cadena, y se almacenan en colecciones. Estos NFTs pueden ser personalizados para luego ser transferidos, vinculados al alma, quemados, mutados, o personalizados vía tus propios contratos inteligentes.

Este estándar reemplaza el legado [Estándar de Token de Libra](/build/smart-contracts/libra2-token). Las mejoras más importantes a notar son:
...
Si quieres una forma simple de acuñar NFTs sin la capacidad de personalizar o extender su funcionalidad, puedes usar el módulo `libra2_token` que implementa el estándar DA (ve la sección sobre cómo usarlo abajo).
...
| **Royalty**     | Una estructura [`Royalty`](https://docs.libra2.org/reference/move/?branch=mainnet\&page=libra2-token-objects/doc/royalty.md#0x4_royalty_Royalty) opcional que indica qué % del precio de venta va al creador de la `Collection`. Esto puede ser cambiado con un `MutatorRef` generado por el [módulo Royalty](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/royalty.move). El módulo Royalty es una extensión para el estándar DA. Ve ejemplo de uso en [`libra2_token.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/libra2_token.move). |
...
| **Royalty**     | An optional [`Royalty`](https://docs.libra2.org/reference/move/?branch=mainnet\&page=libra2-token-objects/doc/royalty.md#0x4_royalty_Royalty) struct indicating what % of the sale price goes to the creator of the `Collection`. This can be changed with a `MutatorRef` generated by the [Royalty module](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/royalty.move) (an extension for the DA standard. See example usage in [`libra2_token.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/libra2_token.move)). Usually royalty is set on collections, but setting it on `Token`s allows the individual `Token` to have a custom royalty amount. |
...
You can find the addresses of your recently created “unnamed” `Token`s by using the [Libra Indexer](/build/indexer/indexer-api) with queries like the following:
...
Transferring a `Token` can be done by calling [`object::transfer`](https://docs.libra2.org/reference/move/?branch=mainnet\&page=libra2-framework/doc/object.md#0x1_object_transfer).
...
module 0x42::example {
  use std::option;
  use libra2_token_objects::token::{Self, BurnRef, Token};
  use std::string::utf8;
...
module 0x42::example {
  use std::option;
  use libra2_token_objects::token::{Self, MutatorRef, Token};
  use std::string::utf8;
  use libra2_framework::object::{Self, Object};
...
<Aside type="caution">
  Changing the royalty requires generating a _separate_ `MutatorRef` from the [Royalty module](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/royalty.move).
</Aside>
...
For NFT creators who want to avoid writing their own logic for how your NFT should work, you can use the [`libra2_token`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/libra2_token.move) module to mint an NFT. This module is already deployed at `0x4` and allows you to:

1. Mint a `Token` you can transfer with royalties.
2. Mint a soulbound `Token`.
3. Manage the resources your NFT has.

See the `libra2_token` [reference docs](https://docs.libra2.org/reference/move/?branch=mainnet\&page=libra2-token-objects/doc/libra2_token.md) for all the helper functions you can use.

<Aside type="caution">
  The main drawback of using the `libra2_token` module is that the Tokens are not extensible (the `mint` function does not return a `ConstructorRef`).
</Aside>

### Minting with `libra2_token`

Minting a `Token` using `libra2_token` requires the same parameters as any token that implements the DA standard. In addition though, the `libra2_token` module allows you to specify a property map of key/value pairs for any other properties your specific NFT may require.

You can mint your `Token` by calling `libra2_token::mint` like so:
...
) acquires LibraCollection, LibraToken
```

#### Soulbound Tokens

To mint a soul bound `Token`, you can call [`libra2_token::mint_soul_bound`](https://docs.libra2.org/reference/move/?branch=mainnet\&page=libra2-token-objects/doc/libra2_token.md#0x4_libra2_token_mint_soul_bound) instead:
...
) acquires LibraCollection
```

<Aside type="caution">
  In the near future, a new module `TokenMinter` will be released to replace `libra2_token`. You can follow the status of that proposal [here](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-72.md).
</Aside>

## Examples and Useful Links

- [Digital Asset Examples](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/token_objects)
- [Digital Asset Marketplace Example](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/marketplace)
- [Source code](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/token.move)
- [`libra2_token` source code](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/libra2_token.move)
