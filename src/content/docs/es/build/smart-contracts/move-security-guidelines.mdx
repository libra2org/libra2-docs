...
- Utilice `entry` para funciones destinadas a ser utilizadas solo desde la CLI o SDK de Libra.
...
|                | Módulo en sí | Otros Módulos                | CLI/SDK de Libra |
| -------------- | ------------- | ---------------------------- | ------------- |
| privado        | ✅             | ⛔                            | ⛔             |
...
En este caso, `sample_function` puede ser llamado tanto por la CLI/SDK de Libra como por cualquier módulo declarado como amigo.
...
En el Framework de Libra, múltiples recursos `key`-ables pueden ser almacenados en una sola cuenta de objeto.
...
[Leer más](/build/smart-contracts/objects) sobre Objetos de Libra.
...
> En Libra, siempre nos preocupamos por la seguridad. Durante la compilación, nos aseguramos de que ninguna API de aleatoriedad se invoque desde una función pública. Sin embargo, todavía permitimos que los usuarios hagan esta elección agregando el atributo `#[lint::allow_unsafe_randomness]` a la función pública.
