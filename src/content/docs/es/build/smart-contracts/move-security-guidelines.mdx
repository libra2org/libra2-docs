...
module 0x42::example {
    ...
    entry fun execute_action_with_valid_subscription(
        ...
        assert!(subscription.end_subscription >= libra2_framework::timestamp::now_seconds(),1);
        // Usar la suscripción
        [...]
    }
}
...
- Utilice `entry` para funciones destinadas a ser utilizadas solo desde la CLI o SDK de Libra2.
...
|                | Módulo en sí | Otros Módulos                | CLI/SDK de Libra2 |
| -------------- | ------------- | ---------------------------- | ------------- |
| privado        | ✅             | ⛔                            | ⛔             |
...
En este caso, `sample_function` puede ser llamado tanto por la CLI/SDK de Libra2 como por cualquier módulo declarado como amigo.
...
En el Framework de Libra2, múltiples recursos `key`-ables pueden ser almacenados en una sola cuenta de objeto.
...
[Leer más](/build/smart-contracts/objects) sobre Objetos de Libra2.
...
Para más información sobre la aleatoriedad y por qué es crucial prevenir la predicción de números aleatorios, consulta esta página: [Guía de Aleatoriedad](https://docs.libra2.org/guides/randomness/).

***

### Aleatoriedad - test-and-abort

> En Libra2, siempre nos preocupamos por la seguridad. Durante la compilación, nos aseguramos de que ninguna API de aleatoriedad se invoque desde una función pública. Sin embargo, todavía permitimos que los usuarios hagan esta elección agregando el atributo `#[lint::allow_unsafe_randomness]` a la función pública.
...
module user::lottery {
    ...
    public entry fun play(user: &signer) {
        let random_value = libra2_framework::randomness::u64_range(0, 100);
        if (random_value == 42) {
            mint_to_user(user);
        }
    }
...
module user::lottery {
    ...
    //transfer 10 libra2 from admin to user
    fun win(user: &signer) {
        let admin_signer = &get_admin_signer();
        let libra2_metadata = get_libra2_metadata();
        primary_fungible_store::transfer(admin_signer, libra2_metadata, address_of(user),10);
    }

    //transfer 10 libra2 from user to admin, then 1 libra2 from admin to fee_admin
    fun lose(user: &signer) {

        //user to admin
        let libra2_metadata = get_libra2_metadata();
        primary_fungible_store::transfer(user, libra2_metadata, @admin, 10);

        //admin to fee_admin
        let admin_signer = &get_admin_signer();
        primary_fungible_store::transfer(admin_signer, libra2_metadata, @fee_admin, 1);
    }
