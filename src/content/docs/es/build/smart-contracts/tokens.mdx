---
title: "Resumen del Estándar de Token de Libra"
---

El [Estándar de Activo Digital de Libra](/build/smart-contracts/digital-asset) define el
Token No Fungible canónico en Libra. Libra aprovecha la composabilidad para extender
...
El resto de este documento discute cómo los estándares de token de Libra se comparan con los
...
A diferencia de Ethereum o Libra donde los datos y código coexisten, Solana almacena datos y
...
Las [cuentas](/network/blockchain/accounts) en Libra almacenan tanto contratos inteligentes
...
Hay dos medios para crear NFTs en Libra:
...
Libra logra un balance entre la personalización ofrecida por Ethereum con la
simplicidad de crear nuevas colecciones como Solana.

Como Ethereum, Libra requiere indexación para determinar el conjunto de todos los NFTs poseídos por
...
La colección de estándares de token de Libra comparte algunas similitudes con Solana,
...
Libra identifica un token por su `Address` o `ObjectId`, una ubicación dentro del
...
El token de Libra tiene metadatos en su recurso `Token` con los datos más comúnmente
...
que está definido en `TokenDataId` en Libra. Solana, sin embargo, no ofrece
mutabilidad para activos, a diferencia de Libra. Como Libra, Metadatos de Token v1.1.0 ofrece
