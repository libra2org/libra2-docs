...
[Despliegue de Código de Objeto](/build/smart-contracts/deployment) es el método preferido para desplegar y actualizar contratos inteligentes en Libra2. Las cuentas de recurso requieren que los desarrolladores generen semillas
cada vez que se crea una cuenta de recurso, y actualizar contratos requiere pasos específicos que son propensos a errores.

Una [cuenta de recurso](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/resource_account.move)
...
En Libra2, una cuenta de recurso se crea basándose en el hash SHA3-256 de la
...
la creación de una cuenta de recurso. Pero si se encuentra una cuenta de recurso, Libra2
...
1. Usando Libra2 CLI: `libra2 account create-resource-account` crea una cuenta de
   recurso, y `libra2 move create-resource-account-and-publish-package` crea una
...
En este ejemplo, [inicializarás](https://github.com/libra2org/libra2-core/blob/2e9d8ee759fcd3f6e831034f05c1656b1c48efc4/libra2-move/move-examples/mint_nft/sources/minting.move#L73) el módulo `mint_nft` y recuperarás
...
1. Inicializa el módulo como se muestra en el ejemplo [`minting.move`](https://github.com/libra2org/libra2-core/blob/2e9d8ee759fcd3f6e831034f05c1656b1c48efc4/libra2-move/move-examples/mint_nft/sources/minting.move#L73).
2. Llama `create_resource_account_and_publish_package` para publicar el módulo
   bajo la dirección de la cuenta de recurso, como en el ejemplo end-to-end [`mint_nft.rs`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/e2e-move-tests/src/tests/mint_nft.rs#L62).
3. Recupera la capacidad de signer de la cuenta de recurso + cuenta del módulo como se muestra
   en el ejemplo [`minting.move`](https://github.com/libra2org/libra2-core/blob/2e9d8ee759fcd3f6e831034f05c1656b1c48efc4/libra2-move/move-examples/mint_nft/sources/minting.move#L83).
...
en [`minting.move`](https://github.com/libra2org/libra2-core/blob/2e9d8ee759fcd3f6e831034f05c1656b1c48efc4/libra2-move/move-examples/mint_nft/sources/minting.move#L143-L181).

Para más detalles, ve las referencias de "cuenta de recurso" en [`resource_account.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/resource_account.move)
y [`account.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/account/account.move).
