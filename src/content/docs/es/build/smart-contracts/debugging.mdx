...
## Depuración con la CLI de Libra2

### Simulación al enviar transacciones

Puedes usar la CLI de Libra2 para simular funciones de entrada antes de ejecutarlas.

Normalmente, una transacción fallará en simulación si no funcionará en la cadena. Por ejemplo:

```shellscript name="Terminal"
libra2 move run --function-id 0x1::libra2_account::transfer --args address:0x1 u64:1000000000000000000
{
...
```shellscript name="Terminal"
 libra2 move run-script --script-path <ruta_script> ...
```

### Simulación Local

Adicionalmente, para algunas situaciones, la simulación local puede dar información adicional y
[imprimir cualquier declaración de depuración que tengas en tu código](https://docs.libra2.org/en/build/cli/working-with-move-contracts#printing-debugging-information).

```shellscript name="Terminal"
libra2 move run --function-id 0x1::libra2_account::transferred --args address:0x1 u64:1000000000000000000 --local
...
Simulando transacción localmente...
{
  "Result": {
    ...
    "success": false,
    "version": 56634003,
    "vm_status": "status FUNCTION_RESOLUTION_FAILURE of type Verification with message Function 0x1::libra2_account::transferred does not exist"
  }
}
```
...
```shellscript name="Terminal"
libra2 move run --function-id 0x1::libra2_account::transferred --args address:0x1 u64:1000000000000000000 --profile-gas

Simulando transacción localmente usando el perfilador de gas...
Gas report guardado en gas-profiling/txn-a90ca655-0x1-libra2_account-transferred.
{
  "Result": {
...
```shellscript name="Terminal"
open  gas-profiling/txn-a90ca655-0x1-libra2_account-transferred/index.html
```

## Evaluando el rendimiento

```shellscript name="Terminal"
libra2 move run --function-id 0x1::libra2_account::transfer --args address:0x1 u64:1 --benchmark
