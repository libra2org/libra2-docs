...
<Aside type="note">
  Para un ejemplo completo de cómo crear tu propio módulo de Activo Fungible, por favor ve [fa\_coin.move](https://github.com/libra2org/libra2-core/blob/main/aptos-move/move-examples/fungible_asset/fa_coin/sources/FACoin.move).
  Alternativamente, puedes explorar la colección de [código de ejemplo FA aquí](https://github.com/libra2org/libra2-core/tree/main/aptos-move/move-examples/fungible_asset).
</Aside>
...
Aparte de la funcionalidad de activo fungible gestionado por defecto proporcionada por el Framework de Libra, los emisores de activos fungibles pueden implementar su propia lógica de depósito/retiro usando el estándar de activo fungible despachable. Esto se hace registrando funciones hook personalizadas para ser invocadas en tiempo de retiro/depósito. Estas funciones hook están almacenadas en los metadatos de una clase de activo fungible, y el Framework de Libra las invocará automáticamente en lugar de la lógica por defecto. Esto permite a los emisores implementar lógica compleja, como control de acceso personalizado. Para más detalles, refiere al [AIP](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-73.md) correspondiente.
...
2. Recupera el balance del FA emparejado:
   - Llama [getCurrentFungibleAssetBalances](https://github.com/libra2org/aptos-ts-sdk/blob/c01a26ff899235fac1c31c6cc3fe504b764e5b91/src/api/fungibleAsset.ts#L115).
...
<details>
  ...
  <div className="p-8">
    Después de la migración, consultar el recurso `0x1::coin::CoinStore<0x1::aptos_coin::LibraCoin>` en la dirección de cuenta no estará disponible. En su lugar, puedes consultar el balance de cuenta por las siguientes tres formas:

    1. [Balance Node API](https://aptos.dev/en/build/apis/fullnode-rest-api#tag/accounts/GET/accounts/%7Baddress%7D/balance/%7Basset_type%7D), el asset\_type puede ser tanto coin\_type, como 0x1::aptos\_coin::LibraCoin o dirección de metadatos FA como 0xa, de cualquier manera debería devolver el mismo valor.
...
<details>
  ...
  <div className="p-8">
    ¡Únete al [Discord de Libra](https://discord.com/invite/aptosnetwork)! Los ingenieros de Libra Labs estarán disponibles durante toda la semana de migración para responder preguntas y ofrecer soporte.
  </div>
</details>
