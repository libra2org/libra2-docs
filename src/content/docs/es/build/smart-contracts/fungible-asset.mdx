---
id: "fungible-asset"
title: "Estándar de Activo Fungible (FA) de Libra2"
---
...
El Estándar de Activo Fungible de Libra2 (también conocido como "Activo Fungible" o "FA") proporciona una forma estándar y segura de tipos para definir activos fungibles en el ecosistema Move. Es un reemplazo moderno para el módulo `coin` que permite acuñación, transferencia y personalización sin problemas de activos fungibles para cualquier caso de uso.

Este estándar es importante porque permite que los activos fungibles en Libra2 (como Monedas y Activos del Mundo Real (RWAs)) representen y transfieran propiedad de una manera consistente que las dApps pueden reconocer. Este estándar también permite personalización más extensa que el módulo `coin` al aprovechar [Objetos Move](/build/smart-contracts/objects) para representar datos de activos fungibles.
...
module my_addr::my_fungible_asset_example {
    use libra2_framework::fungible_asset::{Self, MintRef, TransferRef, BurnRef, Metadata, FungibleAsset};
...
<Aside type="note">
  Para un ejemplo completo de cómo crear tu propio módulo de Activo Fungible, por favor ve [fa\_coin.move](https://github.com/libra2org/libra2-core/blob/main/libra2-move/move-examples/fungible_asset/fa_coin/sources/FACoin.move).
  Alternativamente, puedes explorar la colección de [código de ejemplo FA aquí](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/fungible_asset).
</Aside>
...
Puedes encontrar el conjunto completo de funciones que el Estándar de Activo Fungible proporciona [aquí](https://docs.libra2.org/reference/move/?branch=mainnet\&page=libra2-framework/doc/fungible_asset.md). {/* TODO: Update the Move Reference link once we migrate that page */}
...
Aparte de la funcionalidad de activo fungible gestionado por defecto proporcionada por el Framework de Libra2, los emisores de activos fungibles pueden implementar su propia lógica de depósito/retiro usando el estándar de activo fungible despachable. Esto se hace registrando funciones hook personalizadas para ser invocadas en tiempo de retiro/depósito. Estas funciones hook están almacenadas en los metadatos de una clase de activo fungible, y el Framework de Libra2 las invocará automáticamente en lugar de la lógica por defecto. Esto permite a los emisores implementar lógica compleja, como control de acceso personalizado. Para más detalles, refiere al [AIP](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-73.md) correspondiente.
...
Para más detalles sobre estas limitaciones, refiere al [AIP](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-73.md).
...
Para usuarios usando `primary_fungible_store` para gestionar activos, no se requieren cambios para interactuar con activos con hooks despachables. El Framework de Libra2 invoca automáticamente la lógica de despacho si un hook está configurado.
...
- Para usuarios del Indexador de Libra2, puedes utilizar la tabla llamada `current_fungible_asset_balances` para obtener la suma más reciente del balance de coin y balance FA representando el mismo tipo de activo. Si el FA tiene un tipo de coin emparejado, el tipo de activo sería establecido al tipo de coin, como `0x1::libra2_coin::Libra2Coin`. De lo contrario, para FA no emparejado de un coin, el tipo de activo sería la dirección de metadatos. Los usuarios podrían filtrar por este campo para obtener el balance FA de su interés.
...
2. Recupera el balance del FA emparejado:
   - Llama [getCurrentFungibleAssetBalances](https://github.com/libra2org/libra2-ts-sdk/blob/c01a26ff899235fac1c31c6cc3fe504b764e5b91/src/api/fungibleAsset.ts#L115).
...
<details>
  <summary><span className="font-bold">¿Qué es el estándar de Activo Fungible (FA) de Libra2?</span></summary>

  <div className="p-8">
    El estándar FA introduce una nueva forma de representar tokens fungibles como [objetos Move](https://docs.libra2.org/en/build/smart-contracts/objects), reemplazando el modelo de recurso Coin legacy. Los Activos Fungibles son más componibles y amigables para desarrolladores comparados con Coins legacy.
...
<details>
  ...
  <div className="p-8">
    Todos los tokens en Libra2 comenzarán a migrar automáticamente del Coin v1 al estándar FA.
...
<details>
  ...
  <div className="p-8">
    Después de la migración, consultar el recurso `0x1::coin::CoinStore<0x1::libra2_coin::Libra2Coin>` en la dirección de cuenta no estará disponible. En su lugar, puedes consultar el balance de cuenta por las siguientes tres formas:

    1. [Balance Node API](https://docs.libra2.org/en/build/apis/fullnode-rest-api#tag/accounts/GET/accounts/%7Baddress%7D/balance/%7Basset_type%7D), el asset\_type puede ser tanto coin\_type, como 0x1::libra2\_coin::Libra2Coin o dirección de metadatos FA como 0xa, de cualquier manera debería devolver el mismo valor.
...
<details>
  ...
  <div className="p-8">
    Puedes [usar la API del Indexador](https://docs.libra2.org/en/build/indexer/indexer-api/indexer-reference#current_fungible_asset_balances) para consultar todos los activos fungibles que el usuario posee.
...
<details>
  ...
  <div className="p-8">
    ¡Únete al [Discord de Libra2](https://discord.com/invite/libra2network)! Los ingenieros de Libra2 Labs estarán disponibles durante toda la semana de migración para responder preguntas y ofrecer soporte.
  </div>
</details>
