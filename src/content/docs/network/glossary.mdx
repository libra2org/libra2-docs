...
- An **account** in the Libra blockchain is a container for an arbitrary number
...
- An **account address** is the address of an Libra account.
- Account address refers to a specific destination on the Libra network. The
  address dictates the destination and source of a specific amount of assets
  exchanged by two parties on the blockchain.
- Libra addresses are 64-character hex string (32 bytes). Often times these
...
- An **Application Programming Interface (API)** is a set of protocols and tools
  that allow users to interact with Libra blockchain nodes and client networks
  via external applications. Libra offers a REST API to communicate with our
  nodes.
- See [documentation](/build/apis) for more details.

### APT

**Libra token (APT)** is the Libra blockchain native token used for paying
network and transaction fees.

### Libra

**Libra** is a Layer 1 blockchain for everyone. It uses the Move programming
language and launched its mainnet on 2022-10-17 to redefine the web3 user
experience. The Libra blockchain is dedicated to creating better user
experiences through increased speed, security, scalability, reliability and
usability with low transaction costs. The word "Libra" means "The People" in the
Ohlone language. Learn more about the Libra blockchain on
the [official Libra website](https://libra2foundation.org).
...
- The **Libra blockchain** is a ledger of immutable transactions agreed upon by
  the validators on the Libra network (the network of validators).

### Libra Name Service (ANS)

- The **Libra Name Service (ANS)** is a decentralized naming address service for
  the Libra blockchain. An Libra name is a human-readable _.apt_ domain name
...
containing the code for Libra Network software. Libra-core contains software for

- the Libra blockchain itself, which generates and stores the immutable ledger
  of confirmed transactions and
- the validation process, which implements the consensus algorithm to validate
  transactions and add them to the Libra blockchain immutable ledger.

### Libra Ecosystem

- **Libra ecosystem** refers to various components of the Libra blockchain
  network and their interactions. The Libra ecosystem includes the
...
- The **[Libra Explorer](https://explorer.libra2labs.com/)** is an interface that
  helps users examine details of the Libra blockchain, including account
  information, validators, and transactions.
- The Libra Explorer help users validate their work in Libra wallets and other
  tools in the blockchain.

### Libra Framework

The **Libra Framework** defines the public API for blockchain updates and the
structure of on-chain data. It defines the business logic and access control for
the three key pillars of Libra functionality: payments, treasury, and on-chain
...
An **Libra node** is a peer entity of the Libra network that tracks the state of
the Libra blockchain. There are two types of Libra
nodes, [validators](#validator) and [fullnodes](#fullnodes).

### Libra Protocol

- **Libra protocol** is the specification of how transactions are submitted,
  ordered, executed, and recorded within the Libra network.
...
- On Libra, blocks are a batch of [transactions](#transaction) committed at the
  same time.
...
- **BlockSTM** is the state-of-the-art dynamic parallelism execution engine developed by the Libra Labs team.
...
- The Libra blockchain uses LibraBFT, a consensus protocol based
  on [Jolteon](#jolteon).
- BFT algorithms typically operate with a number of entities, collectively
  holding $N$ votes (which are called "validators" in the Libra network’s
...
- **Command line interface** refers to the Libra CLI used for developing on the
  Libra blockchain, operating nodes, and debugging issues. Find out more
  at [the Libra CLI page](/build/cli).

### Client

- **Client** is software that receives information from the blockchain and
  manages transactions. Clients interact with the blockchain through the Libra
  nodes.

### Code Labs

- **Code labs and tutorials** depict various workflows - such as the use of the
  Libra CLI in minting non-fungible tokens (NFTs) - in order for users to
...
- The Libra blockchain is formed with these agreed-upon transactions and their
  corresponding execution results.
- The consensus component is accountable for achieving security, trust, and
  agreement among all validators on the Libra blockchain.
...
- **Decentralized applications (dapps)** are programs or digital applications
  that run on the Libra blockchain autonomously. Smart contracts are commonly
  used to achieve this function.

### Devnet

- The **Libra devnet** is a publicly deployed instance of the Libra network that
  runs using a set of validator test nodes.
- The devnet is a demonstration of the Libra network that is built for
...
- More specifically, the Libra network uses the PureEdDSA scheme over the
  Ed25519 curve, as defined in RFC 8032.

### Epoch

- An **epoch** is the period of time between reconfigurations of the validator
  set and other administrative actions by the blockchain. On Libra mainnet
...
- In the Libra protocol, events provide evidence that the successful execution
...
- **Execution** in the Libra blockchain is an Libra node component that manages
...
- Follow the [Asset Standards](/build/smart-contracts/libra2-standards#asset-standards) to create
  fungible assets on the Libra blockchain.
...
- For the legacy Libra Token Standard (libra2\_token::token), a **fungible token**
...
- For the Libra Digital Asset Standard (libra2\_token\_objects::token), a \*
...
- **Gas** is a way to pay for computation and storage on a blockchain network.
  All transactions on the Libra network cost a certain amount of gas.
...
- **[Indexer](/build/indexer)** is the component of Libra that
  retrieves, processes, and efficiently stores raw data in the database to
  provide speedy access to the Libra blockchain state.
...
- **Jolteon** is a recent proposal for a [BFT](#byzantine-fault-tolerance-bft)
  consensus protocol.
- LibraBFT, the Libra network's consensus algorithm, is based on Jolteon.
...
- Transactions in the mempool of a validator are added from the JSON-RPC Service
  of the current node and from the mempool of other Libra nodes.
...
- The Libra network treats the entire blockchain as a single data structure that
...
- The **[Merkle Accumulator](https://www.usenix.org/legacy/event/sec09/tech/full_papers/crosby.pdf)**
  is an _append-only_ Merkle tree that the Libra blockchain uses to store the
  ledger.
...
- Move is a safe and secure programming language for web3 that emphasizes access
  control and scarcity. It is the programming language used to build the Libra
  blockchain. You can read more about it
  in [Move on Libra](/network/blockchain/move). { /* TODO add move-on-libra2 landing page */}
...
- A **Move module** defines the rules for updating the global state of the Libra
  blockchain.
...
- The Move script is an arbitrary program that interacts with resources
  published in the global storage of the Libra blockchain by calling the
...
- A **node** is a peer entity of the Libra network that tracks the state of the
  Libra blockchain.
- An Libra node consists of logical
...
- **Open-source community** is a term used for a group of developers who work on
  ...
  the Libra project's developer community.
...
- Every operation in the Libra blockchain can be verified cryptographically that
...
mechanism is leveraged by the Libra blockchain powered by a network of
...
- The **REST API Service** component is the external interface of an Libra node. Any
...
- Libra **software development kits (SDKs)** are sets of tools that enable a
  developer to quickly create a custom app on the Libra platform. Find out more
  at [Use the Libra SDKs](/build/sdks).
...
- **State root hash** is
  ...
  values the state of the Libra blockchain at a given version.
...
- A [**table**](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-stdlib/doc/table.md)
  implements the Table type and in Libra is used to store information as
...
for the associated Libra source file.

### Testnet

- **Testnet** describes the Libra network that is not fully functional yet more
...
- A **validator** is an entity of the Libra ecosystem that validates on the
  Libra blockchain. It receives requests from clients and runs consensus,
...
- Libra validators are in charge of verifying transactions.
...
- **Validator nodes** are a unique class of fullnodes that take part in
  ...
  in Libra. Validators agree upon transactions to be added to the Libra
...
An Libra transaction is **well-formed** if each of the following conditions are
