...
Libra network. Many of the methods described here rely on the runtime metrics that your nodes collect and report. These
metrics are collected by the Libra node binary and are exposed via a Prometheus metrics endpoint. For a detailed
...
After deploying your nodes and connecting them to the Libra network, you should verify that your nodes are running
correctly.
...
- **REST API:** You can also monitor your node's health by querying the REST API. For example, you can verify the
  ...
  [Libra API Specification](/build/apis/fullnode-rest-api).
...
The Libra Labs team can also monitor your node remotely using [Node Telemetry](/network/nodes/configure/telemetry). When you enable telemetry on
your nodes, the Libra node binary will send telemetry data in the background, which includes the node's metrics.
...
internet, then it will report various key metrics to Libra Labs. Libra Labs will also observe the on-chain events
