...
This document describes how to connect your validator and validator fullnode (VFN) to an Libra2 network.
...
At a high-level, there are four steps required to connect your nodes to an Libra2 network:
...
4. Find the IP/DNS for the monitoring load balancer, using:
   ...
   kubectl get svc ${WORKSPACE}-mon-libra2-monitoring --output jsonpath='{.status.loadBalancer.ingress[0]}'
   ```
...
```shellscript filename="Terminal"
${WORKSPACE}-libra2-node-0-genesis-e3
```

```shellscript filename="Terminal"
export WORKSPACE=<your workspace name>

kubectl create secret generic ${WORKSPACE}-libra2-node-0-genesis-e2 \
...
<Aside type="caution">
  **MAINNET VS TESTNET**<br />
  The commands shown below are for the Libra2 mainnet. If you are connecting to a different
  network, you will need to change the `--network` value in the commands accordingly. You can view the
  values in [Libra2 Blockchain Networks](/network/nodes/networks) to see how profiles can be configured based on the network.
</Aside>

### 1. Initialize the Libra2 CLI

First, initialize the Libra2 CLI with your operator account private key. This can be found in your `private-keys.yaml` file
...
```shellscript filename="Terminal"
libra2 init --profile mainnet-operator \
   --network mainnet \
...
```shellscript filename="Terminal"
libra2 account list --profile mainnet-operator
```
...
```shellscript filename="Terminal"
libra2 node get-stake-pool --owner-address <owner_address>
```
...
```shellscript filename="Terminal"
libra2 node update-validator-network-addresses  \
...
```shellscript filename="Terminal"
libra2 node update-consensus-key  \
   --pool-address <pool-address> \
   --operator-config-file ~/$WORKSPACE/$USERNAME/operator.yaml \
   --profile mainnet-operator
```
...
```shellscript filename="Terminal"
libra2 node join-validator-set \
   --pool-address <pool-address> \
   --profile mainnet-operator
```
...
<Aside type="note">
  **IDENTIFYING THE NEXT EPOCH**<br />
  You can identify the next epoch by checking the [Libra2 Explorer](https://explorer.libra2labs.com/validators/all?network=mainnet) or by running the command `libra2 node get-stake-pool`.
</Aside>
...
```shellscript filename="Terminal"
libra2 node show-validator-set --profile mainnet-operator | jq -r '.Result.pending_active' | grep <pool_address>
```
...
```shellscript filename="Terminal"
libra2 node show-validator-set --profile mainnet-operator | jq -r '.Result.active_validators' | grep <pool_address>
```
...
Congratulations! You have successfully connected your nodes to the Libra2 network. To verify that your nodes are running
