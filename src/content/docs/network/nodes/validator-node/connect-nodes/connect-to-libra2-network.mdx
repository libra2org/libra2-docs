...
This document describes how to connect your validator and validator fullnode (VFN) to an Libra network.
...
At a high-level, there are four steps required to connect your nodes to an Libra network:
...
<Aside type="caution">
  **MAINNET VS TESTNET**<br />
  The commands shown below are for the Libra mainnet. If you are connecting to a different
  network, you will need to change the `--network` value in the commands accordingly. You can view the
  values in [Libra Blockchain Networks](/network/nodes/networks) to see how profiles can be configured based on the network.
</Aside>

### 1. Initialize the Libra CLI

First, initialize the Libra CLI with your operator account private key. This can be found in your `private-keys.yaml` file
...
<Aside type="note">
  **IDENTIFYING THE NEXT EPOCH**<br />
  You can identify the next epoch by checking the [Libra Explorer](https://explorer.libra2labs.com/validators/all?network=mainnet) or by running the command `libra2 node get-stake-pool`.
</Aside>
...
Congratulations! You have successfully connected your nodes to the Libra network. To verify that your nodes are running
