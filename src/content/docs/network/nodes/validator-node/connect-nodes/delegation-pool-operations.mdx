...
The delegation pool owner should set an operator for the pool via the `set_operator` function described in the [Perform pool owner operations](#perform-pool-owner-operations) section. The operator should then start their own Libra node, as it is a best practice to have a different account for owner and operator. Once the delegation pool attains 1 million APT, the operator can join the validator set.
...
1. [Install and configure](/build/cli) the Libra CLI. If you are looking to develop on the Libra blockchain, debug apps, or perform node operations, the Libra tool offers a command line interface for these purposes.
2. [Initialize local configuration and create an account](/build/cli/setup-cli) on the Libra blockchain.
...
5. The delegation pool can now [connect to the Libra Network](/network/nodes/validator-node/connect-nodes/connect-to-libra2-network).

## Perform delegation pool operations

This section describes the available operations that can be performed on this recently created pool. Once the delegation pool has been established, use the Libra CLI to operate the pool. The available actions that can be performed on it include:
...
Delegation pool owners have access to specific methods designed for modifying the `operator` and `voter` roles of the delegation pool. Use the following Libra CLI commands and include the relevant addresses:
...
Until the delegation pool has received 1 million APT and the validator has been added to the set of active validators, there will be no rewards to track during each cycle. In order to obtain information about a delegation pool, use the Libra [View function](/build/apis/fullnode-rest-api#reading-state-with-the-view-function)
...
In the [Libra TypeScript SDK](/build/sdks/ts-sdk), a View function request would resemble:

```typescript filename="view_function.ts"
import { Libra, LibraConfig } from "@libra2-core/ts-sdk";
...
Alternatively, you can use Libra CLI to call View functions.
