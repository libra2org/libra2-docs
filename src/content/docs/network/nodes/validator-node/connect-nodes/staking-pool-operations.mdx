...
<Aside type="note">
  **TESTNET VS MAINNET**<br />
  The Libra2 CLI commands below target mainnet. Change the `--network` value for testnet and devnet.
  View the values in [Libra2 Blockchain Networks](/network/nodes/networks) to see how profiles can be configured based on the network.
</Aside>

Before initializing a staking pool, ensure that there is an existing owner account with 1 Million APT.

1. Initialize the [Libra2 CLI](/build/cli) with a private key from an existing account, such as a wallet, or create a new account.

```shellscript filename="Terminal"
libra2 init --profile mainnet-owner \
  --network mainnet
```
...
```shellscript filename="Terminal"
libra2 stake create-staking-contract \
...
3. Once the staking pool has been initialized, you can proceed to [connect to the Libra2 Network](/network/nodes/validator-node/connect-nodes/connect-to-libra2-network).
...
```shellscript filename="Terminal"
libra2 stake set-delegated-voter \
...
```shellscript filename="Terminal"
libra2 stake increase-lockup --profile mainnet-owner
```

### Unlock stake

```shellscript filename="Terminal"
libra2 stake unlock-stake \
  --amount <amount> \
  --profile mainnet-owner
```
...
```shellscript filename="Terminal"
libra2 move run --function-id 0x1::staking_contract::update_commission \
...
- The beneficiary addresses can be set by the operator using the following command:
  ...
  ```shellscript filename="Terminal"
  libra2 move run --profile mainnet_operator \
...
- To view the beneficiary address set for the operator, use the following command:
  ```shellscript filename="Terminal"
  libra2 move view --url <REST API for the network> \
...
<Aside type="note">
  Before you proceed, see [Validation on the Libra2 blockchain](/network/blockchain/staking#validation-on-the-libra2-blockchain) for a brief overview.
</Aside>
...
<Aside type="note">
  For testnet or devnet `--url` field values, see [Libra2 Blockchain Networks](/network/nodes/networks).
</Aside>

```shellscript filename="Terminal"
libra2 node get-stake-pool \
...
- This can be set only by the stake pool owner. Operator receives the "commission\_percentage" of the generated staking rewards. If you request the commission (you can do so by running the command `libra2 stake request-commission`), then at the end of the `lockup_expiration_utc_time` the commission part of the rewards will go to the operator address while the rest will stay in the stake pool and belong to the owner. Here "the commission part of the rewards" means the value of **commission\_not\_yet\_unlocked**.

  For example, in a scenario with a lock-up of one month, you call `libra2 stake request-commission` every month. This will pay out the commission that was accrued during the previous month but only when unlocked at the end of the previous month. Regardless of how often you run `libra2 stake request-commission` during the month, the commission is only paid out upon the completion of `lockup_expiration_utc_time`.
...
- The date when the commission will unlock. However, this unlocked commission will not be auto-disbursed. It will only disburse when the command `libra2 stake request-commission` is called again.
...
When you run the `libra2 stake request-commission` command before the end of the lockup expiration, the command will initiate unlock for any locked commission earned up until that moment in time. The commission will remain in `pending_inactive` until the end of the lockup period, will continue to earn rewards until the lockup period expires. The commission will not be withdrawable until after the end of the lockup period, when `libra2 stake request-commission` is called a second time.
...
- Successful leader-validator proposals during the current epoch. Also see [Validation on the Libra2 blockchain](/network/blockchain/staking#validation-on-the-libra2-blockchain) for the distinction between leader-validator and the voter-validator.
...
```shellscript filename="Terminal"
libra2 node analyze-validator-performance \
