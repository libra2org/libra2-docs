...
<Aside type="note">
  **TESTNET VS MAINNET**<br />
  The Libra CLI commands below target mainnet. Change the `--network` value for testnet and devnet.
  View the values in [Libra Blockchain Networks](/network/nodes/networks) to see how profiles can be configured based on the network.
</Aside>

Before initializing a staking pool, ensure that there is an existing owner account with 1 Million APT.

1. Initialize the [Libra CLI](/build/cli) with a private key from an existing account, such as a wallet, or create a new account.
...
3. Once the staking pool has been initialized, you can proceed to [connect to the Libra Network](/network/nodes/validator-node/connect-nodes/connect-to-libra2-network).
...
<Aside type="note">
  Before you proceed, see [Validation on the Libra blockchain](/network/blockchain/staking#validation-on-the-libra2-blockchain) for a brief overview.
</Aside>
...
<Aside type="note">
  For testnet or devnet `--url` field values, see [Libra Blockchain Networks](/network/nodes/networks).
</Aside>
...
- Successful leader-validator proposals during the current epoch. Also see [Validation on the Libra blockchain](/network/blockchain/staking#validation-on-the-libra2-blockchain) for the distinction between leader-validator and the voter-validator.
