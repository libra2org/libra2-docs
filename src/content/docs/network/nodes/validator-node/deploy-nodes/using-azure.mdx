...
This is a step-by-step guide to deploy an Libra2 validator and validator fullnode (VFN) using Microsoft Azure. Using this guide,
...
<Aside type="caution">
  **Prerequisites** <br />
  ...
  installed the [Libra2 CLI](/build/cli),
...
<Aside type="note">
  **Default connection to mainnet**<br />
  If you follow the default setup in this document, then your validator and VFN will be connected to the Libra2 mainnet.
  To connect to a different Libra2 network, such as testnet, make sure you download the correct genesis and waypoint
...
1. Create a working directory for your Libra2 nodes, and pick a username for your nodes, e.g.,
...
2. Create a blob storage container for storing the Terraform state on Azure, you can do this on the Azure UI
   ...
   ```shellscript filename="Terminal"
   az group create -l <azure region> -n libra2-$WORKSPACE
   az storage account create -n <storage account name> -g libra2-$WORKSPACE -l <azure region> --sku Standard_LRS
   az storage container create -n <container name> --account-name <storage account name> --resource-group libra2-$WORKSPACE
   ```
...
4. Modify the `main.tf` file to configure Terraform and create the Terraform module. See the example below:
   ...
   module "libra2-node" {
     # Download the Terraform module from the libra2-core repository.
     source        = "github.com/libra2org/libra2-core.git//terraform/libra2-node/azure"
   ...
   - The Terraform variables [https://github.com/libra2org/libra2-core/blob/main/terraform/libra2-node/azure/variables.tf](https://github.com/libra2org/libra2-core/blob/main/terraform/libra2-node/azure/variables.tf)
   - The Helm values: [https://github.com/libra2org/libra2-core/blob/main/terraform/helm/libra2-node/values.yaml](https://github.com/libra2org/libra2-core/blob/main/terraform/helm/libra2-node/values.yaml).
...
8. After `terraform apply` finishes, you can check if the resources have been created correctly, by running the following commands:
   ...
   - `az aks get-credentials --resource-group libra2-$WORKSPACE --name libra2-$WORKSPACE`: This command will configure access for your k8s cluster.
...
9. Next, we need to inject your node's IP information into your environment. You can do this by running the following commands:
   ...
   ```shellscript filename="Terminal"
   export VALIDATOR_ADDRESS="$(kubectl get svc ${WORKSPACE}-libra2-node-0-validator-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"

   export FULLNODE_ADDRESS="$(kubectl get svc ${WORKSPACE}-libra2-node-0-fullnode-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"
   ```

10. Now, generate the key pairs for your nodes in your working directory. You can do this by running
    the following command with the Libra2 CLI:

    ```shellscript filename="Terminal"
    libra2 genesis generate-keys --output-dir ~/$WORKSPACE/keys
    ```
...
11. Next, you will need to set your validator configuration. This includes setting the validator and VFN host names,
    ...
    ```shellscript filename="Terminal"
    libra2 genesis set-validator-configuration \
    ...
    `operator.yaml`. These will be useful for connecting your nodes to the Libra2 network (later).
...
13. To recap, in your working directory (`~/$WORKSPACE`), you should have a list of files:
    ...
    - `main.tf`: The Terraform files to install the `libra2-node` module.
...
14. Finally, insert the `genesis.blob`, `waypoint.txt` and the identity files as secrets into the k8s cluster,
    ...
    ```shellscript filename="Terminal"
    kubectl create secret generic ${WORKSPACE}-libra2-node-0-genesis-e1 \
    ...
    <Aside type="caution">
      **Era numbers and dangling volumes**<br />
      ...
      The `era` is a concept relevant only to Kubernetes deployments of an Libra2 node.
...
15. Now, we should be able to see that all pods are running, including the validator and VFN. You can check this by
    ...
    node1-libra2-node-0-fullnode-e9-0              1/1     Running   0          4h31m
    node1-libra2-node-0-haproxy-7cc4c5f74c-l4l6n   1/1     Running   0          4h40m
    node1-libra2-node-0-validator-0                1/1     Running   0          4h30m
    ```

<Aside type="caution">
  **Next steps** <br />
  You have now completed setting up your validator and VFN using Azure. However, your nodes will not be able to connect
  to the Libra2 network just yet.
</Aside>

## Connecting to the Libra2 Network
