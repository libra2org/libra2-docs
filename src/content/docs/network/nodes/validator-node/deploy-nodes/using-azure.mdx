...
This is a step-by-step guide to deploy an Libra validator and validator fullnode (VFN) using Microsoft Azure. Using this guide,
...
<Aside type="caution">
  **Prerequisites** <br />
  ...
  installed the [Libra CLI](/build/cli),
...
<Aside type="note">
  **Default connection to mainnet**<br />
  If you follow the default setup in this document, then your validator and VFN will be connected to the Libra mainnet.
  To connect to a different Libra network, such as testnet, make sure you download the correct genesis and waypoint
...
1. Create a working directory for your Libra nodes, and pick a username for your nodes, e.g.,
...
10. Now, generate the key pairs for your nodes in your working directory. You can do this by running
    the following command with the Libra CLI:
...
11. Next, you will need to set your validator configuration. This includes setting the validator and VFN host names,
    ...
    `operator.yaml`. These will be useful for connecting your nodes to the Libra network (later).
...
14. Finally, insert the `genesis.blob`, `waypoint.txt` and the identity files as secrets into the k8s cluster,
    ...
    <Aside type="caution">
      **Era numbers and dangling volumes**<br />
      ...
      The `era` is a concept relevant only to Kubernetes deployments of an Libra node.
...
<Aside type="caution">
  **Next steps** <br />
  ...
  to the Libra network just yet.
</Aside>

## Connecting to the Libra Network
