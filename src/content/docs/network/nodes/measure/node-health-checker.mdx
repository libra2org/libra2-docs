...
The Libra Node Health Checker (NHC) service can be used to check the health of any Libra fullnodes (VFNs or PFNs).
...
<Aside type="note">
  ...
  [publicly available NHC service](https://nodetools.libra2foundation.org/#/node_checker) run by the Libra Foundation.
</Aside>
...
- A public fullnode connected to the Libra mainnet.
- A validator node connected to the Libra testnet.
- A node running in a single node testnet.

### Baseline configuration

In all the above cases, a baseline node is used to compare your node's health. For example, for a public fullnode connected to the Libra devnet, the baseline node might be a node run by the Libra team and this node demonstrates optimal performance and participation characteristics.
...
<Aside type="note">
  While the Libra team hosts our own instances of this service, we encourage node operators to run their own instances.
</Aside>
