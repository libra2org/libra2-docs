...
The Libra2 Node Health Checker (NHC) service can be used to check the health of any Libra2 fullnodes (VFNs or PFNs).
...
<Aside type="note">
  ...
  [publicly available NHC service](https://nodetools.libra2foundation.org/#/node_checker) run by the Libra2 Foundation.
</Aside>
...
**Important**: If your local node is not a devnet fullnode, you must use a different baseline config. See [the configuration examples in libra2-core](https://github.com/libra2org/libra2-core/tree/main/ecosystem/node-checker/configuration_examples) for other such example configs.
...
<Steps>
  1. Download the baseline configuration YAML
     ...
     mkdir /tmp/nhc
     cd /tmp/nhc
     wget https://raw.githubusercontent.com/libra2-core/libra2-core/main/ecosystem/node-checker/configuration_examples/devnet_fullnode.yaml
     ```
  ...
  2. Start the NHC service
     ...
     ```shellscript filename="Terminal"
     docker run -v /tmp/nhc:/nhc -p 20121:20121 -t libra2labs/node-checker:nightly /usr/local/bin/libra2-node-checker server run --baseline-config-paths /nhc/devnet_fullnode.yaml
     ```
...
- A public fullnode connected to the Libra2 mainnet.
- A validator node connected to the Libra2 testnet.
- A node running in a single node testnet.

### Baseline configuration

In all the above cases, a baseline node is used to compare your node's health. For example, for a public fullnode connected to the Libra2 devnet, the baseline node might be a node run by the Libra2 team and this node demonstrates optimal performance and participation characteristics.

You will download the baseline configuration YAML before running the NHC service for your node. The baseline node's configuration YAML describes where to find this baseline node (URL + port), what evaluators (e.g., metrics checks, TPS tests, API validations, etc.) the NHC service should run, what parameters the NHC should use for those evaluators, what name the configuration has, and so on. See these [example baseline configuration YAML files](https://github.com/libra2org/libra2-core/tree/main/ecosystem/node-checker/configuration_examples).
...
```shellscript filename="Terminal"
curl 'http://nhc.libra2labs.com/check?node_url=http://myfullnode.mysite.com&baseline_configuration_id=devnet_fullnode'
```
...
You can find a few [example baseline configuration YAML files](https://github.com/libra2org/libra2-core/tree/main/ecosystem/node-checker/configuration_examples) that work for each of the above use cases and more.
...
mkdir /tmp/nhc
cd /tmp/nhc
configs=(devnet_fullnode testnet_fullnode mainnet_fullnode); for c in ${configs[@]}; do wget https://raw.githubusercontent.com/libra2-core/libra2-core/main/ecosystem/node-checker/configuration_examples/$c.yaml; done
```
...
<Aside type="note">
  While the Libra2 team hosts our own instances of this service, we encourage node operators to run their own instances.
</Aside>
...
```shellscript filename="Terminal"
docker run -v /etc/nhc:/etc/nhc -p 20121:20121 -t libra2labs/node-checker:nightly /usr/local/bin/libra2-node-checker server run --baseline-config-paths /tmp/nhc/devnet_fullnode.yaml /tmp/nhc/testnet_fullnode.yaml /tmp/nhc/mainnet/fullnode.yaml
```
...
```shellscript filename="Terminal"
git clone git@github.com:libra2-core/libra2-core.git
cd libra2-core
```
...
```shellscript filename="Terminal"
cargo run -p libra2-node-checker --release -- server run --baseline-config-paths /tmp/nhc/devnet_fullnode.yaml
```
