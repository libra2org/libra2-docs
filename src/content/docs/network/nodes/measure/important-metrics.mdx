...
<Aside type="note">
  **Metric instability**<br />
  As Libra continues to grow and develop the blockchain software, many metrics will come and go.
...
<Aside type="note">
  Compare the synced version shown by `libra2_state_sync_version{type="synced"}` with the highest version shown on the [Libra Explorer](https://explorer.libra2labs.com/?network=mainnet) to see how far behind the latest blockchain version your node is. Remember to select the correct network that your node is syncing to (e.g., `mainnet`).
</Aside>
