...
When you run a node on an Libra network, your node will send telemetry data to Libra Labs. All node types
(e.g., validators, VFNs and PFNs) send telemetry data. This also occurs for other binaries (e.g., the Libra CLI).
...
<Aside type="note">
  **No personal information is collected**<br />
  The Libra node binary does **not** collect any personal information, such as usernames or email addresses.
...
The list below shows the categories of information collected by Libra node telemetry:
...
The Libra CLI tool also collects telemetry data. The list below shows the categories of information collected by
CLI telemetry:
...
both the Libra node and the Libra CLI tool. To disable all telemetry, set `APTOS_DISABLE_TELEMETRY` environment to `true`:
...
To disable it permanently across all terminals and Libra binary invocations, include it in your startup profile.
For example:
...
list below shows the variables you can set to configure telemetry for Libra nodes and the CLI:
...
- `APTOS_FORCE_ENABLE_TELEMETRY`: This overrides the chain ID check and forces the Libra node to send telemetry regardless of whether the remote service accepts it or not.
...
- `APTOS_DISABLE_PROMETHEUS_NODE_METRICS`: This disables sending the Libra node resource metrics such as system CPU, memory, etc.
