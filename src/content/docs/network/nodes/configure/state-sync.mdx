...
Nodes in an Libra2 network (e.g., validators, VFNs and PFNs) must always be synchronized to the latest blockchain state.
The [state synchronization](https://medium.com/libra2labs/the-evolution-of-state-sync-the-path-to-100k-transactions-per-second-with-sub-second-latency-at-52e25a2c6f10) (state sync) component that runs on each node is responsible for this. State sync
...
This allows the node to catch up to the latest state in the Libra2 blockchain. Next, the node will stay up-to-date with the blockchain
...
<Aside type="note">
  ...
  - **Testnet**: To sync the entire blockchain history, restore from a [backup](/network/nodes/bootstrap-fullnode/libra2-db-restore). Otherwise, download [a snapshot](/network/nodes/bootstrap-fullnode/bootstrap-fullnode) or use [fast sync](/network/nodes/configure/state-sync#fast-syncing).
  - **Mainnet**: To sync the entire blockchain history, restore from a [backup](/network/nodes/bootstrap-fullnode/libra2-db-restore). Otherwise, use [fast sync](/network/nodes/configure/state-sync#fast-syncing).
</Aside>
...
- **Execute all the transactions since genesis**. In this mode, the node will retrieve from the Libra2 network all the
...
- **Fast syncing**. In this mode, the node will skip the transaction history in the blockchain and will download only
  ...
  be able to catch up to the Libra2 network much more rapidly. This mode is the default mode for testnet and mainnet.
...
After the node has bootstrapped and caught up to the Libra2 network initially, state sync will then move into
...
<Aside type="caution">
  **Verify node syncing**<br />
  ...
  [`libra2_state_sync_version{type="synced"}`](/network/nodes/full-node/verify-pfn) metric gradually increase.
</Aside>
...
<Aside type="caution">
  **Verify node syncing**<br />
  While your node is syncing, you'll be able to see the
  `libra2_state_sync_version{type="synced_states"}` metric gradually increase.
  However, `libra2_state_sync_version{type="synced"}` will only increase once
  ...
  **Note:** If `libra2_state_sync_version{type="synced_states"}` does not
...
For more information about how this works, see the [state synchronization blog post](https://medium.com/libra2labs/the-evolution-of-state-sync-the-path-to-100k-transactions-per-second-with-sub-second-latency-at-52e25a2c6f10).
