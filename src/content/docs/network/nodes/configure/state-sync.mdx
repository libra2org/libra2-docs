...
Nodes in an Libra network (e.g., validators, VFNs and PFNs) must always be synchronized to the latest blockchain state.
...
This allows the node to catch up to the latest state in the Libra blockchain. Next, the node will stay up-to-date with the blockchain
...
- **Execute all the transactions since genesis**. In this mode, the node will retrieve from the Libra network all the
...
- **Fast syncing**. In this mode, the node will skip the transaction history in the blockchain and will download only
  ...
  be able to catch up to the Libra network much more rapidly. This mode is the default mode for testnet and mainnet.
...
After the node has bootstrapped and caught up to the Libra network initially, state sync will then move into
