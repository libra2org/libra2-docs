...
This document describes how to bootstrap an Libra node using a backup. This can be done on all node types, including
...
To achieve these goals, the Libra database restore tool lets you use existing [public backup files](#public-backup-files) to restore
...
Libra Labs maintains a few publicly accessible database backups by continuously querying a PFN and storing the backup
...
The [Libra CLI](/build/cli) supports two kinds of restore operations for Libra nodes:
...
<Aside type="note">
  Libra CLI 1.0.14 or newer is needed to perform these operations. Additionally, depending on whether you use AWS or
  Google Cloud, install [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) or
  [gsutil](https://cloud.google.com/storage/docs/gsutil_install).
</Aside>

The sections below provide examples of how to use the Libra CLI to restore a database from a backup.
...
To restore an Libra node with the full history from genesis, set `ledger-history-start-version` to 0 and
