...
<Aside type="caution">
  **Supported only on x86-64 CPUs**<br />
  Running libra2-core via Docker is currently only supported on x86-64 CPUs. If you have an Apple M1/M2 (ARM64) Mac, use
  the libra2-core source code approach (above). If M1/M2 support is important to you, comment on this
  issue: https://github.com/libra2org/libra2-core/issues/1412
</Aside>
...
mkdir data && \
curl -O https://raw.githubusercontent.com/libra2-core/libra2-core/mainnet/docker/compose/libra2-node/fullnode.yaml && \
curl -O https://raw.githubusercontent.com/libra2-core/libra2-networks/main/mainnet/waypoint.txt && \
curl -O https://raw.githubusercontent.com/libra2-core/libra2-networks/main/mainnet/genesis.blob
```

<Aside type="caution">
  **Don't want to connect to mainnet?**<br />
  To connect to other networks (e.g., `devnet` and `testnet`), you can find the genesis and waypoint here ➜ https://github.com/libra2org/libra2-networks.
...
base:
  ...
  waypoint:
    from_file: "/opt/libra2/etc/waypoint.txt"

execution:
  genesis_file_location: "/opt/libra2/etc/genesis.blob"
...
docker run --pull=always \
  ...
  -v $(pwd):/opt/libra2/etc -v $(pwd)/data:/opt/libra2/data \
  --workdir /opt/libra2/etc \
  --name=libra2-fullnode libra2labs/validator:mainnet libra2-node \
  -f /opt/libra2/etc/fullnode.yaml
```
...
<Aside type="note">
  **Docker tags**<br />
  ...
  https://github.com/libra2org/libra2-networks/tree/main/mainnet
</Aside>

You have now successfully configured and started running a PFN in the Libra2 mainnet.
