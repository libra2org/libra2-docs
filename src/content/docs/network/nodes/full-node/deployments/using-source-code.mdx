...
To deploy a PFN using the `libra2-core` source code, first, see [Building Libra2 From Source](/network/nodes/building-from-source) for instructions
on how to download the `libra2-core` repository and build the binary. Then, follow the steps below:

1. Make sure your current working directory is `libra2-core`.
...
```shellscript filename="Terminal"
cargo run -p libra2-node --release -- -f ./fullnode.yaml
```

<Aside type="note">
  **Debugging?**<br />
  The command above will build a release binary for `libra2-node` at: `libra2-core/target/release/libra2-node`. The release
