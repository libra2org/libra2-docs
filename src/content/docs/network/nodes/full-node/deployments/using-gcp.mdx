...
<Aside type="caution">
  **Don't want to connect to devnet?**<br />
  This tutorial defaults to deploying a PFN in the Libra `devnet` network. To connect to other networks
...
<Aside type="note">
  **Before you proceed**<br />
  ...
  deploying an Libra PFN.
</Aside>

You can deploy a PFN on GCP by using the Libra Terraform module.
...
You can find the Docker image tag for `devnet` on the [Libra Docker Hub](https://hub.docker.com/r/libra2labs/validator/tags?page=1\&ordering=last_updated\&name=devnet).
...
5. To verify the correctness of your PFN, you will need to:
   ...
   - Set up a port-forwarding mechanism directly to the Libra pod in one ssh terminal, and
