...
<Aside type="caution">
  **Don't want to connect to devnet?**<br />
  This tutorial defaults to deploying a PFN in the Libra2 `devnet` network. To connect to other networks
...
<Aside type="note">
  **Before you proceed**<br />
  ...
  deploying an Libra2 PFN.
</Aside>

You can deploy a PFN on GCP by using the Libra2 Terraform module.
...
You can find the Docker image tag for `devnet` on the [Libra2 Docker Hub](https://hub.docker.com/r/libra2labs/validator/tags?page=1\&ordering=last_updated\&name=devnet).
...
module "fullnode" {
  # download Terraform module from libra2-core/libra2-core repo
  source        = "github.com/libra2org/libra2-core.git//terraform/fullnode/gcp?ref=main"
...
# for example:
gcloud container clusters get-credentials libra2-devnet --zone us-central1-a --project libra2-fullnode
```
...
devnet0-libra2-fullnode-0   1/1     Running   0          56s
```
...
5. To verify the correctness of your PFN, you will need to:
   ...
   - Set up a port-forwarding mechanism directly to the Libra2 pod in one ssh terminal, and
   ...
   - Set up the port-forwarding to the libra2-fullnode pod. Use `kubectl get pods -n libra2` to get the name of the pod:
     ...
     # for example:
     kubectl port-forward -n libra2 devnet0-libra2-fullnode-0 9101:9101
     ```
    ...
    libra2_chains = {
      devnet = {
        seeds = {
          ...
          addresses = ["/dns4/pfn0.node.devnet.libra2labs.com/tcp/6182/noise-ik/bb14af025d226288a3488b4433cf5cb54d6a710365a2d95ac6ffbd9b9198a86a/handshake/0"]
          role = "Upstream"
          },
          "7fe8523388084607cdf78ff40e3e717652173b436ae1809df4a5fcfc67f8fc61" = {
          addresses = ["/dns4/pfn1.node.devnet.libra2labs.com/tcp/6182/noise-ik/7fe8523388084607cdf78ff40e3e717652173b436ae1809df4a5fcfc67f8fc61/handshake/0"]
          role = "Upstream"
          },
          "f6b135a59591677afc98168791551a0a476222516fdc55869d2b649c614d965b" = {
          addresses = ["/dns4/pfn2.node.devnet.libra2labs.com/tcp/6182/noise-ik/f6b135a59591677afc98168791551a0a476222516fdc55869d2b649c614d965b/handshake/0"]
          role = "Upstream"
          }
        }
      }
...
# Get logs of the pod
kubectl logs <pod-name> -n libra2
# for example:
kubectl logs devnet0-libra2-fullnode-0 -n libra2
```
...
The `devnet0-libra2-fullnode` is the pod that is running the libra2 fullnode container. Click on the pod to view details. You will see some metrics and other details about the pod.
...
Google cloud captures many metrics from the cluster and makes them easily viewable in the console. From the console menu, choose `Kubernetes Engine`. Click on the cluster that libra2 is deployed to. Click on the `Operations` link at the top right. Click on the `Metrics` sub-tab to view specific cluster metrics.
