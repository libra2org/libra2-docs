...
<Aside type="note">
  You can learn more about Libra2 metrics in the [Node Inspection Service](/network/nodes/measure/node-inspection-service) documents.
</Aside>
...
curl 127.0.0.1:9101/metrics 2> /dev/null | grep "libra2_state_sync_version{.*\"synced\"}" | awk '{print $2}'
```
...
[Libra2 explorer page](https://explorer.libra2labs.com/?network=mainnet). If your node is catching up to the highest
...
<Aside type="caution">
  **Using fast sync?**<br />
  ...
  ```shellscript filename="Terminal"
  curl 127.0.0.1:9101/metrics 2> /dev/null | grep "libra2_state_sync_version{.*\"synced_states\"}" | awk '{print $2}'
  ```

  This should show an increasing number of synced states. It may take several hours for your node to fast sync to the
  latest version. Eventually, once fast syncing is complete, the `libra2_state_sync_version{.*"synced"}` command will
...
```shellscript filename="Terminal"
curl 127.0.0.1:9101/metrics 2> /dev/null | grep "libra2_connections{direction=\"outbound\""
```
...
```shellscript filename="Terminal"
curl 127.0.0.1:9101/metrics 2> /dev/null | grep "libra2_connections{direction=\"outbound\""
libra2_connections{direction="outbound",network_id="Public",peer_id="aabd651f",role_type="full_node"} 3
```

If the number of outbound connections returned is `0`, then it means your node cannot connect to the Libra2 blockchain. If this happens to you, follow these steps to resolve the issue:
...
du -cs -BM /opt/libra2/data
```

[rest_spec]: https://github.com/libra2org/libra2-core/tree/main/api

[devnet_genesis]: https://devnet.libra2labs.com/genesis.blob

[devnet_waypoint]: https://devnet.libra2labs.com/waypoint.txt

[libra2-core/libra2-core]: https://github.com/libra2org/libra2-core.git

[status dashboard]: https://status.devnet.docs.libra2.org
