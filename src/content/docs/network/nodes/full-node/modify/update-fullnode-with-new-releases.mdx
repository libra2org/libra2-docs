...
This document outlines the process for updating your PFN with new Libra2 releases. All PFNs will need to be updated
...
If you run your PFN from the [libra2-core][libra2-core/libra2-core] source code, you can update your PFN by
following these steps:

1. Stop your PFN by running the command below (or killing the `libra2-node` process manually):

   ```shellscript filename="Terminal"
   cargo stop libra2-node
   ```
...
1. Delete the data folder (the directory path is what you specified in the configuration file, e.g., `fullnode.yaml`).
   ...
   - The default data folder is `/opt/libra2/data`.
