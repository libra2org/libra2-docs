...
- Stored at `/opt/libra2/data/db/ephemeral_identity_key`.

### Static Identity

This is useful when:

- You wish to advertise your PFN as a seed (i.e., for other Libra2 PFNs to connect to).
...
on whether you are using the `libra2-core` source code to run your PFN, or Docker.

### Using the libra2-core source code

If you use the `libra2-core` source code to run your PFN, follow these steps:

1. **Generate the private key**

First, use the [Libra2 CLI](/build/cli) (`libra2`) to produce a hex encoded static
x25519 private key. This will be the private key for your network identity. Run the following `libra2` CLI command:

```shellscript filename="Terminal"
libra2 key generate --key-type x25519 --output-file /path/to/private-key.txt
```
...
Next, retrieve the peer identity from the public key using the `libra2` CLI. The `--host` flag in
...
```shellscript filename="Terminal"
libra2 key extract-peer --host example.com:6180 \
...
```shellscript filename="Terminal"
cd ~/my-full-node
docker run -it libra2labs/tools:devnet /bin/bash
```

2. **Generate the private key**

Next, follow the remaining steps from inside the `libra2labs/tools` Docker container.
