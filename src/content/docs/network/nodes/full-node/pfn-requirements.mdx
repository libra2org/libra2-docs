...
to connect to you. There are different Libra2 network types, and each network type uses a different port. However,
...
configuration file](https://github.com/libra2org/libra2-core/blob/main/config/src/config/test_data/public_full_node.yaml#L16) for a PFN
...
The amount of data stored by Libra2 PFNs depends on the ledger history (length) of the blockchain and the number of
...
<Aside type="note">
  **Devnet blockchain storage**<br />
  The Libra2 devnet is currently reset on a weekly basis. If you are deploying a devnet PFN, this means that the storage
  will be reset (i.e., wiped) every week. See the `#devnet-release` channel on the [Libra2 Discord](https://discord.gg/libra2network).
</Aside>
