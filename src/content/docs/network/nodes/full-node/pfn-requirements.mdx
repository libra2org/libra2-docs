...
to connect to you. There are different Libra network types, and each network type uses a different port. However,
...
The amount of data stored by Libra PFNs depends on the ledger history (length) of the blockchain and the number of
...
<Aside type="note">
  **Devnet blockchain storage**<br />
  The Libra devnet is currently reset on a weekly basis. If you are deploying a devnet PFN, this means that the storage
  will be reset (i.e., wiped) every week. See the `#devnet-release` channel on the [Libra Discord](https://discord.gg/libra2network).
</Aside>
