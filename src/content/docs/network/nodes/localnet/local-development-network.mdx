---
title: "Running a Local Network via Libra CLI"
---

import { Aside, Steps } from '@astrojs/starlight/components';

Local networks can be helpful when testing your code. They are not connected to any production Libra networks like mainnet, but they are useful for three main reasons:
...
3. **High availability**: The Libra devnet and testnet networks are periodically upgraded, during which time they can be unavailable. Local development networks are also always available even if you have no internet access.
...
<Steps>
  1. Ensure you have the  installed.
     ...
     You can verify if the Libra CLI is installed by running `libra2 --version`.

  2. Ensure you have  installed.

     You can check whether Docker is installed by running `docker --version`.

     1. This is exclusively needed for making a production-like environment by running the Indexer API. Many downstream tools such as the Libra SDK depend on the Indexer API.
  ...
  4. Run the following command in a new terminal to start the private network:
     ...
     <Aside type="caution">
       Note: Despite the name (`local-testnet`), this has nothing to do with the Libra testnet, it will run a network entirely local to your machine.
     </Aside>
     ...
     Completed generating configuration:
             ...
             Libra root key path: "/Users/dport/.libra2/testnet/mint.key"
...
If you want to use the local network with the TypeScript SDK, you can use local network URLs when initializing the client object (`Libra`):
