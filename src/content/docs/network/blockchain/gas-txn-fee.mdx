...
Any transaction execution on the Libra blockchain requires a processing fee. As of today, this fee comprises two components:
...
Transactions can range from simple and inexpensive to complicated based upon what they do. In the Libra blockchain, a **unit of gas** represents a basic unit of consumption for transient resources, such as doing computation or accessing the storage. The latter should not be conflated with the long-term storage aspect of such operations, as that is covered by the storage fees separately.
...
<Aside type="note">
  👉 A **unit of gas** is a dimensionless number or a unit that is not associated with any one item such as a coin, expressed as an integer. The total gas units consumed by your transaction depend on the complexity of your transaction. The **gas price**, on the other hand, is expressed in terms of Libra blockchain’s native coin [APT](/network/glossary#apt) and its subunit [Octas](/network/glossary#octa). Also see [Transactions and States](/network/blockchain/txns-states) for how a transaction submitted to the Libra blockchain looks like.
</Aside>

## The Fee Statement

As of Libra Framework release 1.7, the breakdown of fee charges and refunds is emitted as a module event represented by struct `0x1::transaction_fee::FeeStatement`.
...
When a transaction is submitted to the Libra blockchain, the transaction must contain the following mandatory gas fields:
...
The following gas parameters are set by Libra governance.

<Aside type="note">
  These on-chain gas parameters are published on the Libra blockchain at `0x1::gas_schedule::GasScheduleV2`.
</Aside>
...
If, let's say, you transfer all the money out of your account so that you have no remaining balance to pay for the transaction fee. In such case the Libra blockchain would let you know that the transaction will fail, and your transfer wouldn't succeed either.
...
To simulate the transaction locally, use the gas profiler, which is integrated into the Libra CLI.
