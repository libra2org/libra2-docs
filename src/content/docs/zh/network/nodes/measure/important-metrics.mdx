...
如果您正在运行验证节点,以下[共识](/zh/network/blockchain/blockchain-deep-dive.mdx#consensus)指标很重要:
...
1. `aptos_state_sync_version{type="synced"}`:该指标用于展示节点当前同步到的状态,也就是它处理过的交易总数.如果该指标不再上升,说明节点处于非同步状态.此外,若该指标的增长速度跟不上区块链中新交易的提交速度,节点就很可能无法与网络其他部分实时同步.值得一提的是,如果您启用了[快速同步](/zh/network/nodes/configure/state-sync.mdx#fast-syncing)功能,在所有状态全部下载完毕前,此指标将不会有任何变化,这一过程有时可能较长.详见下文第三点.
...
1. `aptos_connections{direction="inbound"` 和 `aptos_connections{direction="outbound"`:这些指标显示您的节点与多少个对等点相连,以及这些连接是入站还是出站.`inbound` 连接指其他对等点(如全节点)主动连入您的节点;`outbound` 连接则表示您的节点主动连接到其他节点(如验证器全节点).
   ...
   2. 对于全节点来说,保持 `outbound` 连接数量大于 0 是必要的,以保障节点能够进行同步操作.至于 `inbound` 连接的数量,则在您希望让自己的节点成为网络中的种子节点,让其他节点连接至此的情况下才显得重要,正如在[全节点网络连接](/zh/network/nodes/full-node/modify/fullnode-network-connections.mdx#allowing-pfn-connections)一文中讨论的那样.

### 交易池

以下 [交易池](/zh/network/blockchain/blockchain-deep-dive.mdx#mempool)指标很重要:
