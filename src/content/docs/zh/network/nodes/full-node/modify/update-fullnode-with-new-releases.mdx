...
本文档详细介绍了如何在 Libra2 发布新版本时进行 PFN 更新.所有的 PFN 一旦新版本发布都需要执行更新操作.对于在 devnet 上运行的 PFN,需要进行一项额外的操作,即在每次新版本发布时对 devnet 执行数据清理.

## 源代码部署

如果您是从 [libra2-core][libra2-core/libra2-core] 源代码运行您的 PFN,您也可以通过以下步骤来更新您的 PFN:

1. 通过运行以下命令停止您的 PFN(或手动停止 `libra2-node` 进程):
