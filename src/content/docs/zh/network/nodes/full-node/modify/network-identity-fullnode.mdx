...
- 您希望将您的 PFN 作为种子进行广告宣传(即,供其他 Libra2 PFN 连接).
...
下面的步骤将指导您完成为您的 PFN 生成静态身份的过程.确切的步骤取决于您是使用 `libra2-core` 源代码运行还是使用 Docker 运行您的 PFN.

### 使用 libra2-core 源代码

如果您使用 `libra2-core` 源代码运行您的 PFN,请按照以下步骤操作:

1. **生成私钥**

首先,使用 [Libra2 CLI](/zh/build/cli) (`libra2`) 生成一个十六进制编码的静态 x25519 私钥.这将是您网络身份的私钥.运行以下 `libra2` CLI 命令:
...
接下来,请使用 `libra2 CLI` 通过公钥获取对应的对等(peer)身份.在命令中加入 `--host` 参数,指定您的主机信息,这样才能生成并输出您 PFN 的网络地址.请按照下面的格式执行命令(记得将 `--host` 后的内容替换成您真实的主机信息):
...
接下来,按照在`libra2labs/tools` Docker 容器内的剩余步骤进行操作.
