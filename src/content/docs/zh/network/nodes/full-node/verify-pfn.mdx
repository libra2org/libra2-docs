...
<Aside type="caution">
  **使用快速同步?**
  ...
  您可以在此处阅读有关快速同步的更多信息:[状态同步引导](/zh/network/nodes/configure/state-sync.mdx#bootstrapping-phase).
</Aside>
...
2. 从配置文件中删除可能已添加的任何 `seed` 对等节点(Peers).这些 `seed` 可能阻碍您连接到网络.`seed` 对等节点在[连接到种子对等节点](/zh/network/nodes/full-node/modify/fullnode-network-connections.mdx#连接到种子对等节点)部分中有讨论.
