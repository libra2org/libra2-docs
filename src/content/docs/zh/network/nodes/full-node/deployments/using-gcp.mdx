...
<Aside type="caution">
  **不想连接到 devnet?**<br />
  本教程默认在 Libra `devnet` 网络上部署 PFN .要连接到其他网络(例如,`mainnet` 和 `testnet`),请将所有 `devnet` 实例替换为适当的网络名称.
</Aside>
...
<Aside type="note">
  **请先做好准备**<br />
  从这里开始,本指南假设你已经设置好了你的 GCP 账户,并为部署 Libra PFN 创建了一个新的项目.
</Aside>

你可以通过使用 Libra Terraform 模块在 GCP 上部署 PFN.
...
你可以在 [Libra Docker Hub](https://hub.docker.com/r/libra2labs/validator/tags?page=1\&ordering=last_updated\&name=devnet) 上找到 `devnet` 的 Docker 镜像标签.
...
5. 要验证你的 PFN 是否正确,你需要:
   ...
   - 在一个 SSH 终端中直接设置到 Libra pod 的端口转发机制,
   - 在另一个 SSH 终端中进行测试.
