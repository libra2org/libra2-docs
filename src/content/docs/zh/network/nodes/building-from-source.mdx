...
[使用 CLI 的二进制版本构建](/zh/build/cli/install-cli),但如果你想从源代码构建 Libra 节点或 CLI,可以使用以下方法.

## 支持的操作系统

Libra 可以在包括 Linux,macOS 和 Windows 在内的各种操作系统上构建.Libra 在 Linux 和 macOS 上经过了广泛的测试,而在 Windows 上的测试较少.这是我们使用的版本:
...
2. 克隆 Libra 仓库.要克隆 Libra 仓库,你首先需要打开命令行提示符(macOS 或 Linux 的终端,Windows 的 PowerShell).然后运行以下命令从 GitHub 克隆 Git 仓库.
...
这是可选的:切换到 `release` 分支以安装 Libra 节点.
...
通过安装构建(build),测试(test)和检查(inspect) Libra Core 所需的依赖项来准备你的开发环境.
