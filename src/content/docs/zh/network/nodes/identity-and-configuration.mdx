...
在 Libra2 网络上安装节点时,安装的过程中需要您处理身份和配置.本文档描述了如何理解术语 **身份** 和 **配置**,并提供了身份 YAML 文件的说明.

在 Libra2 网络中安装节点时,您将需要配置身份和系统设置.本文档将指导您如何理解\*\*身份(identity)**和**配置(configuration)\*\*这两个术语,并详细介绍了身份相关的 YAML 配置文件.
...
| public-keys.yaml              | 描述                                            |
| ----------------------------- | --------------------------------------------- |
| account\_address               | 运营者的 Libra2 区块链账户地址,即部署验证者节点的角色.               |
| account\_public\_key            | 与区块链账户关联的公钥.                                  |
| consensus\_public\_key          | 仅由运营者用于验证目的.                                  |
| consensus\_proof\_of\_possession | 仅由运营者用于验证目的.                                  |
| full\_node\_network\_public\_key  | 用于识别 Libra2 网络中 VFN(验证者全节点)或 PFN(公共全节点)的全节点公钥. |
| validator\_network\_public\_key  | 验证者节点的公钥,用于在 Libra2 网络中识别验证者节点.                |
...
| full\_node\_network\_private\_key | 全节点的私钥.持有此私钥的人将能够在 Libra2 网络中拥有对 VFN 和 PFN 的所有权. |
| validator\_network\_private\_key | 验证者节点的私钥.持有此私钥的人将能够在 Libra2 网络中拥有验证者节点的所有权.      |
...
| ----------------------- | ------------------------------------------ |
| account\_address         | 运营商的 Libra2 区块链账户地址,即部署验证者节点的角色.            |
| account\_private\_key     | 与区块链账户关联的私钥.                               |
| consensus\_private\_key   | 仅由运营商用于验证目的和轮换共识密钥的共识私钥.                   |
| network\_private\_key     | 验证者节点的私钥.持有此私钥的人将能够在 Libra2 网络中建立验证者节点的所有权. |
...
| --------------------------------- | ----------------------------------------------- |
| account\_address                   | 运营者的 Libra2 区块链账户地址,即部署验证者节点的角色.                 |
| network\_private\_key               | 全节点的私钥.持有此私钥的人将能够在 Libra2 网络中拥有对 VFN 和 PFN 的所有权. |
