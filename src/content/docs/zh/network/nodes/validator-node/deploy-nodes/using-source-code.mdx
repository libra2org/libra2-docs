...
这是一份分步指南,用于使用源代码部署 Libra 验证器和验证器全节点(VFN).按照本指南,验证器和 VFN 将部署在不同的机器上.

<Aside type="caution">
  **先决条件**<br />
  在开始之前,请确保阅读并理解了 [节点要求](/zh/network/nodes/validator-node/node-requirements).同样,请确保您已经安装了 [Libra CLI](/zh/build/cli),因为您将需要它来设置您的节点.
</Aside>
...
<Aside type="note">
  **默认连接到主网**<br />
  ...
  要连接到不同的 Libra 网络,例如测试网,请确保在构建二进制文件时选择适当的源代码分支,并下载您想要连接的网络的正确创世区块和路标文件.
</Aside>

1. 按照 [从源代码构建 Libra](/zh/network/nodes/building-from-source) 中的步骤下载 `libra2-core` 存储库和源代码.

2. 使用 `git checkout --track origin/mainnet` 检出 `mainnet` 分支.注意:如果您想在另一个网络上部署验证器和 VFN,请使用适当的分支名称(例如,`testnet`).

3. 为您的 Libra 节点创建一个工作目录,并为您的节点选择一个用户名,例如,
...
4. 在您的工作目录中为您的节点生成密钥对.您可以通过运行以下命令并使用 Libra CLI 来执行此操作:
...
5. 接下来,您需要设置验证器的配置.这包括设置验证器和 VFN 的主机名,这些可能是 IP 地址或 DNS 地址.
   ...
   您可以通过运行以下命令并使用 Libra CLI 来设置验证器配置:
   ...
   配置验证器将在 `~/$WORKSPACE/$USERNAME` 目录中创建两个 YAML 文件:`owner.yaml` 和 `operator.yaml`.这些将有助于将您的节点连接到 Libra 网络(稍后).
...
10. 现在您已经设置好了配置文件,可以启动您的验证器和 VFN 了.要启动您的验证器,请在 `libra2-core` 目录根目录下运行以下命令:
    ...
    <Aside type="caution">
      **下一步** <br />
      您现在已经使用源代码完成了验证器和 VFN 的设置.然而,您的节点还不能连接到 Libra 网络.
    </Aside>
