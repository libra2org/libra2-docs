...
这是一份分步指南,用于使用 Docker 部署 Libra2 验证器和验证器全节点(VFN).按照本指南,验证器和 VFN 将部署在不同的机器上.

<Aside type="caution">
  **先决条件** <br />
  在开始之前,请确保阅读并理解 [节点要求](/zh/network/nodes/validator-node/node-requirements).同样,请确保您已经安装了 [Libra2 CLI](/zh/build/cli),以及 [Docker 与 Docker Compose](https://docs.docker.com/engine/install/).
</Aside>
...
<Aside type="note">
  **默认连接到主网**<br />
  遵循本文档的默认配置,你的验证节点和全节点后端(VFN)将会接入 Libra2 主网.如果想接入到 Libra2 的其他网络,如测试网,你需要确保下载了目标网络相应的创世数据块和路标文件.此外,根据所选网络,需调整 docker compose 文件,以确保使用到相匹配的 docker 镜像.
</Aside>
...
2. 在您的工作目录中为您的节点生成密钥对.您可以通过运行以下命令并使用 Libra2 CLI 来执行此操作:
...
8. 为了启动你的 VFN,请在一台独立的,专为 VFN 设置的计算机上执行以下命令.你需要从验证节点的计算机那里复制密钥,配置文件和 docker compose 文件过来.
   ...
   <Aside type="caution">
     **下一步**<br />
     您现在已经使用 Docker 完成了验证器和 VFN 的设置.然而,您的节点还不能连接到 Libra2 网络.
   </Aside>
