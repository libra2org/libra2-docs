...
- **指标:** 您可以通过运行 `curl` 命令来监控您的本地指标端点,[节点检查服务](/zh/network/nodes/measure/node-inspection-service)并验证关键指标.例如,您可以通过运行在[验证同步](/zh/network/nodes/full-node/verify-pfn.mdx#验证同步状态)部分中描述的命令来验证您节点的同步状态.
