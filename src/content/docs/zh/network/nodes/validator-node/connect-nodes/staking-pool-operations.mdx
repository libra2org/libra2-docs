...
<Aside type="note">
  **测试网与主网**<br />
  下面的 Libra CLI 命令针对的是主网.对于测试网和开发网,需要更改 `--network` 的值.
  查看 [Libra 区块链网络](/zh/network/nodes/networks)中的值,了解如何根据不同的网络设置配置文件.
</Aside>

在初始化质押池之前,请确保存在一个拥有 100 万 APT 的账户.

1. 使用现有账户(例如钱包)的私钥初始化 [Libra CLI](/zh/build/cli),或创建一个新账户.
...
3. 一旦质押池初始化完成,您就可以[连接到 Libra 网络](/zh/network/nodes/validator-node/connect-nodes/connect-to-libra2-network).
...
<Aside type="note">
  在继续之前,请查看 [Libra 区块链上的验证](/zh/network/blockchain/staking#validation-on-the-libra2-blockchain)获取简要概述.
</Aside>
...
<Aside type="note">
  对于测试网或开发网的 `--url` 字段值,请参见 [Libra 区块链网络](/zh/network/nodes/networks).
</Aside>
...
- 当前纪元(epoch)中成功的领导者验证者提案.另见[Libra 区块链上的验证](/zh/network/blockchain/staking#validation-on-the-libra2-blockchain),了解领导者验证者和投票者验证者之间的区别.

在当前纪元(Epoch)内,成功的领导-验证节点的提案.为了理解领导-验证节点与投票-验证节点之间的差异,请参考 [Libra 区块链上的验证](/zh/network/blockchain/staking#validation-on-the-libra2-blockchain)部分.
