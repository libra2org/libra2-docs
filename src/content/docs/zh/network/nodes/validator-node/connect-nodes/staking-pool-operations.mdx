...
3. 一旦质押池初始化完成,您就可以[连接到 Libra 网络](/zh/network/nodes/validator-node/connect-nodes/connect-to-aptos-network).
...
<Aside type="note">
  在继续之前,请查看 [Libra 区块链上的验证](/zh/network/blockchain/staking#validation-on-the-aptos-blockchain)获取简要概述.
</Aside>
...
- 当前纪元(epoch)中成功的领导者验证者提案.另见[Libra 区块链上的验证](/zh/network/blockchain/staking#validation-on-the-aptos-blockchain),了解领导者验证者和投票者验证者之间的区别.

在当前纪元(Epoch)内,成功的领导-验证节点的提案.为了理解领导-验证节点与投票-验证节点之间的差异,请参考 [Libra 区块链上的验证](/zh/network/blockchain/staking#validation-on-the-aptos-blockchain)部分.
