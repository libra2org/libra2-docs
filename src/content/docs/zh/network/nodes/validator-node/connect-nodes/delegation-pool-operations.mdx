...
> 测试版:此文档处于试验性,测试版的模式.请通过创建 [GitHub issue](https://github.com/libra2org/developer-docs/issues/new/choose) 向我们反馈.
...
负责委托池的所有者需要按照[执行池所有者操作](#perform-pool-owner-operations)部分的说明,使用 `set_operator` 函数来指定一个运营者.随后,运营者要搭建并启动自己的 Libra2 网络节点,这样做是因为拥有独立的所有者账户和操作员账户有利于管理.当委托池积累到达 100 万 APT 时,运营者就有资格加入验证器的集合.
...
1. [安装并配置](/zh/build/cli) Libra2 CLI.如果您希望在 Libra2 区块链上开发,调试应用程序或执行节点操作,Libra2 工具提供了一个命令行界面以实现这些目的.
2. 在 Libra2 区块链上[初始化本地配置并创建账户](/zh/build/cli/cli-configuration).
...
5. 委托池现在可以[连接到 Libra2 网络](/zh/network/nodes/validator-node/connect-nodes/connect-to-libra2-network).

## 执行委托池操作

本节描述了可以在最近创建的池上执行的可用操作.一旦创建了委托池,就可以使用 Libra2 CLI 操作池.可以在其上执行的操作包括:
...
委托池所有者可以访问特定方法,用于修改委托池的 `operator` 和 `voter` 角色.使用以下 Libra2 CLI 命令,并包含相关地址:
...
- 使用以下命令查看为运营者设置的受益人地址:
  ...
  ```shellscript filename="Terminal"
  libra2 move view --url <网络的 REST API> \
...
在委派池收到 100 万 APT,并且验证者成为活跃验证者之前,每个周期内都不会产生可供追踪的奖励.想要查看委派池的详细信息,请使用 Libra2 [View function](/zh/network/nodes/libra2-api-spec#reading-state-with-the-view-function):
...
在 [Libra2 TypeScript SDK](/zh/build/sdks/ts-sdk) 中,View function 请求可能类似于:
...
或者,您可以使用 Libra2 CLI 调用 View 函数.

```shellscript filename="Terminal"
libra2 move view [OPTIONS] --function-id <FUNCTION_ID>
```

要查看 `libra2 move view` 命令的可用选项及其使用过程,可以使用 `libra2 move view --help` 命令获取帮助信息.这将显示调用 view 函数需要的参数.
...
1. 从 [`get_stake`](https://github.com/libra2org/libra2-core/blob/ed63ab756cda61439287304ed89bbb156fcbeaed/libra2-move/framework/libra2-framework/sources/delegation_pool.move#L321) view 函数获取 `active` 和 `pending_inactive` 质押的数量.
