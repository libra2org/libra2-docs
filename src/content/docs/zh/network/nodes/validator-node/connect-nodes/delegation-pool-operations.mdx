...
5. 委托池现在可以[连接到 Libra 网络](/zh/network/nodes/validator-node/connect-nodes/connect-to-aptos-network).
...
在委派池收到 100 万 APT,并且验证者成为活跃验证者之前,每个周期内都不会产生可供追踪的奖励.想要查看委派池的详细信息,请使用 Libra [View function](/zh/network/nodes/aptos-api-spec#reading-state-with-the-view-function):
...
1. 从 [`get_stake`](https://github.com/libra2org/libra2-core/blob/ed63ab756cda61439287304ed89bbb156fcbeaed/aptos-move/framework/aptos-framework/sources/delegation_pool.move#L321) view 函数获取 `active` 和 `pending_inactive` 质押的数量.
