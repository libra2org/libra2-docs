...
负责委托池的所有者需要按照[执行池所有者操作](#perform-pool-owner-operations)部分的说明,使用 `set_operator` 函数来指定一个运营者.随后,运营者要搭建并启动自己的 Libra 网络节点,这样做是因为拥有独立的所有者账户和操作员账户有利于管理.当委托池积累到达 100 万 APT 时,运营者就有资格加入验证器的集合.
...
1. [安装并配置](/zh/build/cli) Libra CLI.如果您希望在 Libra 区块链上开发,调试应用程序或执行节点操作,Libra 工具提供了一个命令行界面以实现这些目的.
2. 在 Libra 区块链上[初始化本地配置并创建账户](/zh/build/cli/cli-configuration).
...
5. 委托池现在可以[连接到 Libra 网络](/zh/network/nodes/validator-node/connect-nodes/connect-to-libra2-network).

## 执行委托池操作

本节描述了可以在最近创建的池上执行的可用操作.一旦创建了委托池,就可以使用 Libra CLI 操作池.可以在其上执行的操作包括:
...
委托池所有者可以访问特定方法,用于修改委托池的 `operator` 和 `voter` 角色.使用以下 Libra CLI 命令,并包含相关地址:
...
在委派池收到 100 万 APT,并且验证者成为活跃验证者之前,每个周期内都不会产生可供追踪的奖励.想要查看委派池的详细信息,请使用 Libra [View function](/zh/network/nodes/libra2-api-spec#reading-state-with-the-view-function):
...
在 [Libra TypeScript SDK](/zh/build/sdks/ts-sdk) 中,View function 请求可能类似于:
