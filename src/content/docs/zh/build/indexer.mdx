...
Libra2 索引器是一个 API,您可以使用它来获取:

1. 聚合数据(例如:有多少 NFTs 存在?)
2. 历史数据(例如:此账户提交了哪些交易?)
3. 从简单的 [Libra2 节点 API](/zh/build/apis/fullnode-rest-api) 很难获取的数据(例如:哪个账户拥有名为“ExampleToken”的代币?)
...
<CardGrid>
  <LinkCard href="/zh/build/indexer/libra2-hosted" title="访问 API" description="了解如何查询索引器 API。" />
...
<CardGrid>
  ...
  <LinkCard href="/zh/build/indexer/ans-lookup" title="获取 Libra2 名称" description="通过 ANS 检索与某账户关联的 Libra2 名称。" />
