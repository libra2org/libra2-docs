...
直接访问完整节点将提供最新数据(如果不是存档完整节点,则会缺少历史数据),使用 [REST API](/zh/build/apis.mdx#aptos-fullnode)

在此之上的索引层将提供 [GRPC 事务流](/zh/build/indexer/txn-stream/aptos-hosted-txn-stream)

在此事务流之上,我们构建了一些产品逻辑表,可以通过 [GraphQL](/zh/build/indexer/) 进行查询.
