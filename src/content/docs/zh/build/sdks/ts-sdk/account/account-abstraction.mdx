...
Libra2 上的账户抽象 (AA) **通过 Move 模块实现自定义交易验证逻辑**,允许账户在原生加密方案之外定义自己的规则.
...
<Steps>
  1. 1. 部署认证模块
     ...
     部署模块可使用 [Libra2 CLI](/zh/build/cli) 的以下命令.假设您已通过 `libra2 init` 设置工作区,并在 `Move.toml` 文件中声明了命名地址.
...
<Steps>
  1. 1. 创建认证器模块
     ...
     **代理身份验证**
     ...
     要部署该模块,您可以使用 [Libra2 CLI](/zh/build/cli) 的以下命令.我们假设您已经通过 `libra2 init` 设置了工作区,并在 `Move.toml` 文件中声明了命名地址.
