...
1. 通过运行 `pnpm i @libra2-core/ts-sdk` 或使用您习惯的包管理器安装 `@libra2-core/ts-sdk`
2. 更新下方代码片段以构建需要多代理签名的交易
   1. 替换下方注释后的函数和参数:`// REPLACE WITH YOUR MULTI-AGENT FUNCTION HERE`
   2. 此自定义是必要的,因为 Libra2 没有预制的需要多代理签名的合约.如果您想部署自己的示例多代理合约,可以部署 ["transfer two by two" 示例 Move 合约](https://github.com/libra2org/libra2-ts-sdk/blob/main/tests/move/transfer/sources/script_two_by_two.move#L5)
...
`NUMBER_OF_SIGNER_ARGUMENTS_MISMATCH` - 当您尝试为不需要该数量账户的函数进行多代理签名时,会发生此错误.例如,如果您尝试对 `0x1::libra2_account::transfer` 函数使用多个签名 - 该函数仅需要一个地址,因此当提供多个地址时会报错.
