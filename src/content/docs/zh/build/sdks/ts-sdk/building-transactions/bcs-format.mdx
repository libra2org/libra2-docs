...
在底层,Libra2 SDK 为交易参数提供了两种格式:

1. **简单格式** - 使用字符串,整数等基本类型表示交易参数
2. **二进制规范序列化 (BCS)** - Libra2 链识别的格式,具有特定类型(例如:不使用普通整数,而是使用 `U64` 或 `U128` 等类型)
...
您可以通过探索 [BCS GitHub 代码库](https://github.com/libra2org/bcs) 了解更多关于 BCS 的信息.
