...
const tx = await BuildScriptComposerTransaction({
    ...
    // 这里需要传递 Libra Config ，因为组合交易需要读取链上状态
...
组合交易后，我们可以使用 `@libra2-core/ts-sdk` 中的 签名交易 / 模拟交易 / 提交交易等接口:
...
2. 调用者需要确保将正确的值作为参数传递给后续调用.在前述示例中,`0x1::coin::coin_to_fungible_asset` 函数需要接收一个 `Coin<LibraCoin>` 类型的参数.

这实现了 [AIP-102](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-102.md) 规范
