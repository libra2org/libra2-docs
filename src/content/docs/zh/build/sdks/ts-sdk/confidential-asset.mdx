...
你可以使用`Libra`客户端的`confidentialCoin`属性来与`CA`进行交互
...
从[这里](https://github.com/libra2org/confidential-asset-wasm-bindings)生成`android`和`ios`绑定,并按需集成到你的应用中.
...
对于原生应用,你可以[在此](https://github.com/libra2org/confidential-asset-wasm-bindings)生成`android`和`ios`绑定来代替WASM使用.

***

现在我们准备就绪.让我们定义Libra客户端:
...
或者使用[无密钥账户](/zh/build/guides/libra2-keyless)中的[pepper](/zh/build/guides/libra2-keyless/how-keyless-works)
...
为此,请使用`libra2.confidentialAsset.rolloverPendingBalance`.

<Aside type="caution">
  重要提示:在`rollover`操作前,用户的actual余额需要先进行[标准化](/zh/build/sdks/ts-sdk/confidential-asset#normalize)处理.
</Aside>

要同时处理[标准化](#normalization)和`rollover`操作,可以使用`libra2.confidentialAsset.safeRolloverPendingCB`.
...
获取余额后,最终调用`libra2.confidentialAsset.normalizeUserBalance`方法:
...
进行[机密转账](/zh/build/sdks/ts-sdk/confidential-asset#confidential-transfer)需要知道接收方的加密密钥和`libra2`账户地址
...
要进行[密钥轮换](/zh/build/sdks/ts-sdk/confidential-asset#rotate-encryption-key),您需要创建新的解密密钥并使用`libra2.confidentialAsset.rotateCBKey`
