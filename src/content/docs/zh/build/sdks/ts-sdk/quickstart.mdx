...
<Steps>
  ...
  4. 设置 Libra 客户端
     ...
     <Aside type="note">
       (高级) 如需连接到特定节点,可在 `LibraConfig` 中通过指定 `fullnode` URL 进行设置.例如:`fullnode: http://localhost:8080/v1`.
     </Aside>

  5. 从链上获取数据

     你可以使用 `Libra` 客户端从链上获取各种数据,例如网络本身的信息或特定账户信息.
...
```typescript filename="quickstart.ts"
/**
 * 此示例展示如何使用 Libra 客户端创建账户、注资并在账户间进行转账。
 */
...
1. 使用 `Libra` 客户端连接网络
2. 创建账户
3. 使用客户端辅助函数如 [`libra2.getAccountModules`](https://libra2-core.github.io/libra2-ts-sdk/@libra2-core/ts-sdk-1.35.0/classes/Libra.html#getAccountModules) 查询链上数据
4. 签名并向网络提交交易
5. 等待 Libra 执行交易
...
<Aside type="note">
  ...
  请注意,大多数辅助函数都列在 [`Libra` 客户端对象](https://libra2-core.github.io/libra2-ts-sdk/@libra2-core/ts-sdk-1.35.0/classes/Libra.html)上.
</Aside>
