---
title: "通过 Libra2 CLI 运行本地网络"
---

import { Aside, Steps } from '@astrojs/starlight/components';

本地网络在测试代码时很有帮助.它们不会连接到任何生产 Libra2 网络(如主网),但由于以下三个主要原因,它们很有用:

1. **无速率限制:** 你可以与托管服务(如节点 API,索引器 API 和水龙头)交互,而不受速率限制,从而加快测试速度.
2. **可重现性:** 你可以设置特定的链上场景,并随时从头开始重启网络以返回到干净的状态.
3. **高可用性:** Libra2 开发网和测试网会定期升级,在此期间它们可能不可用.本地开发网络即使在你没有互联网连接的情况下也始终可用.
...
<Steps>
  ...
  2. 确保你已安装 .
     ...
     1. 这仅用于通过运行索引器 API 来创建类似生产的环境.许多下游工具(如 Libra2 SDK)都依赖于索引器 API.
  ...
  4. 在新终端中运行以下命令来启动私有网络:
     ...
     <Aside type="caution">
       注意:尽管名称为(`local-testnet`),但这与 Libra2 测试网无关,它将在你的机器上运行一个完全本地的网络.
     </Aside>
     ...
     已完成生成配置：
             日志文件："/Users/dport/.libra2/testnet/validator.log"
             测试目录："/Users/dport/.libra2/testnet"
             Libra2 根密钥路径："/Users/dport/.libra2/testnet/mint.key"
             ...
             Libra2net 全节点网络端点：/ip4/0.0.0.0/tcp/6181
             索引器 gRPC 节点流端点：0.0.0.0:50051

     Libra2 正在运行，按 ctrl-c 退出
  ...
  5. 等待网络启动
     ...
     <details>
       ...
       ```shellscript filename="Terminal"
       在 crates/libra2/src/node/local_testnet/logging.rs:64:10 处出现错误：
     ...
     - [节点 API](/zh/network/nodes/libra2-api-spec):这是直接在节点上运行的 REST API.它支持核心写功能(如交易提交)和有限的读功能,如读取账户资源或 Move 模块信息.
     - [索引器 API](/zh/build/indexer/libra2-hosted):这是一个 [GraphQL](https://graphql.org/) API,提供对索引区块链数据的丰富读取访问.如果你点击上面的索引器 API URL(默认为 [http://127.0.0.1:8090](http://127.0.0.1:8090/)),它将打开 Hasura Console,这是一个帮助你查询索引器 GraphQL API 的 Web UI.
...
```shellscript filename="Terminal"
libra2 init --profile <你的配置文件名称> --network local
```

然后你可以在以后使用该配置文件执行任何命令.例如,如果你想将 [`hello_blockchain`](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/hello_blockchain) 包发布到你的本地网络,你可以运行:

```shellscript filename="Terminal"
libra2 move publish --profile <你的配置文件名称> --package-dir /opt/git/libra2-core/libra2-move/move-examples/hello_blockchain --named-addresses HelloBlockchain=local
```

### 配置 TypeScript SDK

如果你想将本地网络与 TypeScript SDK 一起使用,你可以在初始化客户端对象(`Libra2`)时使用本地网络 URL:
