...
<Aside type="caution">
  如果您仍然需要安装CLI,请按照[这些步骤](/zh/build/cli/install-cli/install-cli-specific-version).
</Aside>
...
3. 这些额外的配置步骤不是必需的,主要是为了改善用户体验.若想根据您的需求继续使用命令行界面(CLI),请参照这里的[使用指南](/zh/build/cli.mdx#%EF%B8%8F-using-the-aptos-cli).
