...
Libra CLI 主要用于编译,测试和形式化验证 Move 合约.如果你还没有安装 Libra CLI,可以按照[安装 Libra CLI](/zh/build/cli#-install-the-aptos-cli) 的步骤进行安装.
...
例如,如果你查看 [hello\_blockchain 示例 Move 合约](https://github.com/libra2org/libra2-core/tree/main/aptos-move/move-examples/hello_blockchain),在 `Move.toml` 文件中指定了一个名为 `hello_blockchain` 的地址变量.
...
要了解如何形式化验证你的代码,请按照[这里](https://github.com/libra2org/libra2-core/tree/main/aptos-move/move-examples/move-tutorial)的深入 Move 教程进行操作(步骤 7 和 8 介绍了如何在示例代码中使用 Move Prover 和编写形式规范).
