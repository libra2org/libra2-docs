...
Aptos CLI 主要被用来编译,测试以及对 Move 合约进行形式化验证工作.如果你尚未安装 Aptos CLI,可以按照[安装 Aptos CLI](/zh/build/cli#-install-the-aptos-cli)提供的步骤进行安装.

您可以通过右侧的目录跳转到特定部分.

要了解如何使用 CLI 在链上链接 Move 合约,可以按照这个[“CLI 参数”教程](/zh/build/cli/working-with-move-contracts/arguments-in-json-tutorial).
...
例如,如果您查看[hello\_blockchain 示例 Move 合约](https://github.com/aptos-labs/aptos-core/tree/main/aptos-move/move-examples/hello_blockchain),在`Move.toml`文件中它指定了一个名为`hello_blockchain`的变量.
...
您可以使用 [Gas Profiler](/zh/build/cli/working-with-move-contracts/local-simulation-benchmarking-and-gas-profiling)估算与发布 Move 合约相关的 Gas 费用.{/* TODO: Fix this link */}
...
例如,如果您已经发布了 [hello\_blockchain 示例包](https://github.com/aptos-labs/aptos-core/tree/main/aptos-move/move-examples/hello_blockchain)到一个地址为 `b9bd2cfa58ca29bce1d7add25fce5c62220604cd0236fe3f90d9de91ed9fb8cb` 的账户,您可以通过以下命令运行它的 `set_message` 函数:
...
对于您希望在单元测试之外保证代码按预期工作的情况,您可以使用 [Move Prover](/zh/build/smart-contracts/prover)来形式化验证您的 Move 合约代码.

您可以通过按照[这些步骤](/zh/build/cli/setup-cli/install-move-prover)来安装 Move Prover.
