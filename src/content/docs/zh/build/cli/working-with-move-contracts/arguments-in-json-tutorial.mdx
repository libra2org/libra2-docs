...
```shellscript filename="在 CLI 中执行有嵌套 vector 参数的函数"
aptos move run \
...
```shellscript filename="通过使用 JSON 格式的输入文件来执行函数"
aptos move run \
...
```shellscript filename="通过 CLI 进行参数传递"
aptos move view \
...
```shellscript filename="通过 JSON 文件进行参数传递"
...
```shellscript filename="通过 CLI 传递参数"
...
```shellscript filename="调用 View 函数"
aptos move view \
    --function-id $ace_addr::cli_args::reveal \
    --type-args \
        0x1::account::Account \
