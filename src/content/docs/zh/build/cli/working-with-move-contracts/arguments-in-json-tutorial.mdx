...
本节引用 [`CliArgs` 示例包](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/cli_args),其包含以下清单:
...
<Aside type="note" emoji="ℹ️">
  将你的工作目录设置为 [`libra2-move/move-examples/cli_args`](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/cli_args) 以便跟随:
...
在包发布后,可以使用 `libra2 move run` 调用 `set_vals()`:
...
<Aside type="note" emoji="ℹ️">
  截至本文撰写时,`libra2` CLI 仅支持脚本函数参数为类型为 `u8` 的向量,并且仅支持深度为 1 的向量.因此 `vector<address>` 和 `vector<vector<u8>>` 是无效的脚本函数参数类型.
</Aside>
