---
title: "通过 Libra CLI 使用硬件钱包 Ledger"
---
...
<Aside type="caution">
  `Ledger Nano S` 的内存有限,可能无法在 Libra 上签署太多的交易.如果您尝试签署的交易数量太大,超出了您的设备处理能力,您将收到错误 `Wrong raw transaction length`
</Aside>

## 初始设置

您需要对 Libra CLI 和您的 Ledger 设备进行一些配置步骤,以签署交易.

<Steps>
  1. 确保您已经安装了 Libra CLI
  ...
  4. 按照在您的 Ledger 设备上安装 Libra 应用程序.

  5. 解锁您的 Ledger 设备并打开 Libra 应用程序

     <Aside type="note" emoji="ℹ️">
       无论何时您想使用 Ledger 进行签名,都需要将其连接,解锁,并在运行任何 CLI 命令之前打开 Libra 应用程序.
     </Aside>

  6. 在 Libra CLI 中创建一个新的 Ledger 配置文件
     ...
     在这个示例中,使用者选择通过在 `libra2 init` 命令后输入 `0` 来选择 Ledger 设备上的第一个账户.你可以根据需要选取任意账户.

     **常见错误:**

     1. 如果您看到错误 `Device Not Found`,请确保解锁您的 Ledger,然后重试.
     2. 如果您看到错误 `Libra ledger app is not opened`,请确保在您的 Ledger 上打开 Libra 应用程序,然后重试.
...
2. 解锁它.
3. 打开 Libra 应用程序.
4. 运行需要签名的 Libra CLI 命令.

<Aside type="note" emoji="ℹ️">
  这个过程适用于任何需要签名的命令,无论是转移 coin,发布 Move 合约,与合约交互等.
</Aside>

例如,如果您想发布一个像 `[hello_blockchain](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/hello_blockchain)` 这样的 Move 包,您可以按照上述步骤,然后运行:
...
你可能遇到的一个错误是 `Error: Wrong raw transaction length`.这表示交易大小或者数据包对你的设备来说过大,无法被签名.目前,Libra Ledger 应用只支持 20kb 以下的交易量.由于 `Ledger Nano S` 设备的内存更小,这就是为什么它更有可能产生这个错误的原因.
