---
title: "通过 Libra2 CLI 使用硬件钱包 Ledger"
---
...
<Aside type="caution">
  `Ledger Nano S` 的内存有限,可能无法在 Libra2 上签署太多的交易.如果您尝试签署的交易数量太大,超出了您的设备处理能力,您将收到错误 `Wrong raw transaction length`
</Aside>

## 初始设置

您需要对 Libra2 CLI 和您的 Ledger 设备进行一些配置步骤,以签署交易.

<Steps>
  1. 确保您已经安装了 Libra2 CLI
  ...
  4. 按照在您的 Ledger 设备上安装 Libra2 应用程序.

  5. 解锁您的 Ledger 设备并打开 Libra2 应用程序

     <Aside type="note" emoji="ℹ️">
       无论何时您想使用 Ledger 进行签名,都需要将其连接,解锁,并在运行任何 CLI 命令之前打开 Libra2 应用程序.
     </Aside>

  6. 在 Libra2 CLI 中创建一个新的 Ledger 配置文件
     ...
     **常见错误:**
     ...
     2. 如果您看到错误 `Libra2 ledger app is not opened`,请确保在您的 Ledger 上打开 Libra2 应用程序,然后重试.
...
你可能遇到的一个错误是 `Error: Wrong raw transaction length`.这表示交易大小或者数据包对你的设备来说过大,无法被签名.目前,Libra2 Ledger 应用只支持 20kb 以下的交易量.由于 `Ledger Nano S` 设备的内存更小,这就是为什么它更有可能产生这个错误的原因.
