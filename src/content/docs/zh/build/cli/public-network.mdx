...
- 对于测试链,还存在一个管理 LibraCoin 铸造的账户

## 生成创世

- 创世组织者构建一个 `Layout` 并分发它.
- 创世组织者准备 Libra 框架的字节码并分发它.
...
以下部分依赖于 Libra 源代码中的工具.请参阅[从源代码构建 Libra](/zh/network/nodes/building-from-source)进行设置.

### `layout` 文件

layout 文件包含:

- `root_key`:用于 LibraCoin 管理的 Ed25519 公钥.
- `users`:参与者集合
- `chain_id`:`ChainId` 或一个独特的整数,用于区分此部署与其他 Libra 网络
