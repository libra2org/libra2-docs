...
1. **经济的 Gas 使用** – 通过将关键原语实现为 [Move 原生函数](/zh/build/smart-contracts/book/functions.mdx#native-functions),努力为 Move 开发者最小化 gas 成本.例如,参见 [BLS 签名在 BLS12-381 椭圆曲线上的模块](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-stdlib/sources/cryptography/bls12381.move).
...
通常,此类应用的开发者需要等待其所需的密码学功能在 [Aptos Move 框架](/zh/network/blockchain/move) 中作为 [Move 原生函数](/zh/build/smart-contracts/book/functions.mdx#native-functions) 高效实现.
...
例如,Move 开发者可以通过在其实现中使用 [类型参数](/zh/build/smart-contracts/book/functions.mdx#type-parameters) 为曲线类型,通用地在 **任何** 曲线上实现流行的 Boneh-Lynn-Shacham (BLS) 签名方案:
