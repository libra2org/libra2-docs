...
Aptos Framework 示例中,创建了由泛型 `K` 和 `V` 组成的键值表.相关 `add` 函数参数为 `Table<K, V>`,`key` 和 `value`,类型分别为 `K` 和 `V`.`phantom` 语法确保 key 和 value 类型与表一致,防止类型不匹配.[详细了解](/zh/build/smart-contracts/book/generics.mdx#phantom-type-parameters) `phantom` 泛型参数.
...
[详细了解](/zh/build/smart-contracts/book/integers.mdx#operations) 运算.
