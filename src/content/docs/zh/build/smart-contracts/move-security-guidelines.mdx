...
- 仅为 Libra CLI 或 SDK 使用的函数加 `entry`.
...
此时 `sample_function` 可被 Libra CLI/SDK 及所有声明为 friend 的模块调用.
...
Libra Framework 示例中,创建了由泛型 `K` 和 `V` 组成的键值表.相关 `add` 函数参数为 `Table<K, V>`,`key` 和 `value`,类型分别为 `K` 和 `V`.`phantom` 语法确保 key 和 value 类型与表一致,防止类型不匹配.[详细了解](/zh/build/smart-contracts/book/generics#phantom-type-parameters) `phantom` 泛型参数.
...
在 Libra Framework 中,多个具备 `key` 能力的资源可存储于同一 object 账户.

但对象应隔离存储于不同账户,否则对一个对象的修改会影响整个集合.

例如,转移一个资源会导致所有组成员一同转移,因为 transfer 操作针对 `ObjectCore`,即账户下所有资源的通用标签.

[详细了解](/zh/build/smart-contracts/objects) Libra Objects.
...
> Libra 始终以安全为先.编译时会确保无 randomness API 被 public 函数调用.但用户可通过 `#[lint::allow_unsafe_randomness]` 允许 public 函数调用.
