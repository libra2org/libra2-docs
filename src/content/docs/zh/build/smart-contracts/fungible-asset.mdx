...
[本实现](https://github.com/libra2org/libra2-core/blob/main/aptos-move/framework/aptos-framework/sources/fungible_asset.move) 相较于 `coin` 标准有改进,因为 Move Object 更易于通过智能合约自定义和扩展.更多关于编写 [Move Objects](/zh/build/smart-contracts/objects) 的高级指南请参考相关文档.
...
<Aside type="note">
  完整的自定义同质化资产模块示例请参见 [fa\_coin.move](https://github.com/libra2org/libra2-core/blob/main/aptos-move/move-examples/fungible_asset/fa_coin/sources/FACoin.move).
  你也可以在 [FA 示例代码集合](https://github.com/libra2org/libra2-core/tree/main/aptos-move/move-examples/fungible_asset) 中查看更多示例.
</Aside>
...
除了 Libra Framework 提供的默认同质化资产管理功能外,发行方还可以通过可调度同质化资产标准实现自定义的存取逻辑.只需在资产元数据中注册自定义钩子函数,Libra Framework 会在存取时自动调用这些钩子,从而实现复杂的访问控制等逻辑.详情请参考对应的 [AIP](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-73.md).
...
- 对于 Libra Indexer 用户,可通过 `current_fungible_asset_balances` 表获取 coin 余额与配对 FA 余额的最新总和.如果 FA 有配对 coin 类型,asset type 字段为 coin 类型(如 `0x1::aptos_coin::LibraCoin`);否则为元数据地址.用户可按此字段筛选感兴趣的 FA 余额.
...
2. 获取配对 FA 余额:
   - 调用 [getCurrentFungibleAssetBalances](https://github.com/libra2org/aptos-ts-sdk/blob/c01a26ff899235fac1c31c6cc3fe504b764e5b91/src/api/fungibleAsset.ts#L115).
