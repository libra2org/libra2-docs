...
- 使用 [`copy`操作符](/zh/build/smart-contracts/book/variables.mdx#移动--move--与复制--copy-) 从局部变量复制值
- 通过 [解引用`*e`](/zh/build/smart-contracts/book/references.mdx#通过引用读写) 复制引用值
...
- 未使用局部变量或参数中的值
- 未在 [通过`;`的序列](/zh/build/smart-contracts/book/variables.mdx#表达式块) 中使用值
- 在 [赋值](/zh/build/smart-contracts/book/variables.mdx#赋值) 时覆盖变量中的值
- 通过 [写入`*e1 = e2`](/zh/build/smart-contracts/book/references.mdx#通过引用读写) 覆盖引用值
