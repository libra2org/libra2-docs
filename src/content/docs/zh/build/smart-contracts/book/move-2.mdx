...
- **Function Values** :Move 现在支持将函数作为值传递,可作为参数或存储在资源中.
  ...
  详见 [Function Values](/zh/build/smart-contracts/book/functions.mdx#函数值).
...
- **Compound Assignments**:现在可以使用 `x += n`,`x -= n` 等语法来组合赋值与算术运算.
  ...
  支持的运算符详见 [Compound Assignments](/zh/build/smart-contracts/book/variables.mdx#复合赋值).

- **Loop Labels**:现在可以为循环添加标签,并通过 `break` 或 `continue` 表达式引用这些标签.这使得可以从嵌套循环中继续或中断外部循环.

  详见 [循环标签参考文档](/zh/build/smart-contracts/book/loops.mdx#循环标签).
...
- **Receiver Style Functions**:支持使用 `value.func(arg)` 这种熟悉的语法调用函数.
  ...
  文档详见[点接收者函数调用风格章节](/zh/build/smart-contracts/book/functions.mdx#点号接收者函数调用语法).

- **索引表示法**:支持通过 `&mut vector[index]` 或 `&mut Resource[addr]` 等语法访问 [elements of vectors](/zh/build/smart-contracts/book/vector.mdx#向量的索引表示法) 和 [resource storage](/zh/build/smart-contracts/book/global-storage-operators.mdx#存储操作符的索引表示法).

- **Positional Structs**:支持定义包装类型如 `struct Wrapped(u64)`.

  文档详见 [位置结构体章节](/zh/build/smart-contracts/book/structs-and-resources.mdx#位置结构体) .枚举变体也支持位置形式.

- **Dot-dot pattern wildcards**:支持 `let Struct{x, ..} = value` 这类语句来匹配数据的特定部分.

  文档详见 [部分模式章节](/zh/build/smart-contracts/book/structs-and-resources.mdx#部分模式) .这些模式同样适用于枚举变体.
...
- **Package visibility** 允许声明一个函数在包内任意位置可见,但在包外不可见.
  ...
  该特性文档见 [此处](/zh/build/smart-contracts/book/functions.mdx#package可见性) .

- **Assert abort code optional** `assert!`宏现在可以只带一个参数使用,省略中止代码,此时将选择默认代码.另见 [此处](/zh/build/smart-contracts/book/abort-and-assert.mdx#assert) .
...
- **Well-defined evaluation order** 以下情况的求值顺序现已明确定义(此前未作规定):
  ...
  - 对于形如`*lexp = rexp`的"可变"表达式(参见 [通过引用修改](/zh/build/smart-contracts/book/variables.mdx#通过引用修改) ),其中 `lexp` 是 `&mut T` 类型的表达式,`rexp` 是 `T` 类型的表达式,`rexp` 会先于 `lexp` 被求值.

- **Bug fix for acquires annotation** [函数应被标注 `acquires`](/zh/build/smart-contracts/book/functions.mdx#acquires-声明) 当且仅当它直接或通过调用链间接使用 `move_from`,`borrow_global` 或 `borrow_global_mut` 访问资源.
