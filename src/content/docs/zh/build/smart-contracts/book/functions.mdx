...
<Aside type="note">
  如果函数从输入引用派生值,则可以返回不可变 `&` 或可变 `&mut` 的[引用](/zh/build/smart-contracts/book/references).请注意,这意味着函数[不能返回全局存储的引用](/zh/build/smart-contracts/book/references.mdx#引用不能被存储),除非它是[内联函数](#内联函数).
</Aside>
