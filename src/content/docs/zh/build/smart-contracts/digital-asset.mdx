...
数字资产(DA)标准是 Libra 上现代的非同质化代币(NFT)标准.NFT 代表链上的独特资产,并存储在集合中.这些 NFT 可以自定义,后续可通过你的智能合约进行转移,灵魂绑定,销毁,变更或扩展.

该标准取代了旧的 [Libra Token Standard](/zh/build/smart-contracts/libra2-token).主要改进如下:
...
如果你只需要简单铸造 NFT,无需自定义或扩展功能,可以使用实现了 DA 标准的 `libra2_token` 模块(见下文用法).
...
| **Royalty**     | 可选 [`Royalty`](/zh/move-reference/mainnet/libra2-token-objects/royalty#Royalty) 结构体,表示销售价格中归创作者的分成比例.可通过 [Royalty 模块](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/royalty.move) 生成的 `MutatorRef` 修改.该模块是 DA 标准的扩展.示例见 [`libra2_token.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/libra2_token.move). |
...
| **Royalty**     | 可选 [`Royalty`](https://docs.libra2.org/reference/move/?branch=mainnet\&page=libra2-token-objects/doc/royalty.md#0x4_royalty_Royalty) 结构体,表示销售价格中归创作者的分成比例.可通过 [Royalty 模块](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/royalty.move) 生成的 `MutatorRef` 修改(该模块为 DA 标准扩展,示例见 [`libra2_token.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/libra2_token.move)).通常 royalty 设置在 collection 上,但也可为单独的 `Token` 设置自定义分成比例. |
| **URI length**  | 可选字符串,少于 512 字符,链接到 `Collection` 相关内容(可通过 `MutatorRef` 修改).                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
你可以通过 [Libra Indexer](/zh/build/indexer/indexer-api) 查询最近创建的"未命名" `Token` 地址,示例如下:
...
可通过 [`object::transfer`](/zh/move-reference/mainnet/libra2-framework/object#transfer) 转移 `Token`.
...
<Aside type="caution">
  修改 royalty 需通过 [Royalty 模块](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/royalty.move) 生成 _单独_ 的 `MutatorRef`.
</Aside>
...
如果你不想自定义 NFT 逻辑,可直接使用 [`libra2_token`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/libra2_token.move) 模块铸造 NFT.该模块已部署在 `0x4`,支持:

1. 铸造可转让,带分成的 `Token`.
2. 铸造灵魂绑定 `Token`.
3. 管理 NFT 的资源.

所有可用辅助函数见 `libra2_token` [参考文档](/zh/move-reference/mainnet/libra2-token-objects/libra2_token).

<Aside type="caution">
  使用 `libra2_token` 模块的主要限制是 Token 不可扩展(`mint` 函数不会返回 `ConstructorRef`).
</Aside>

### 使用 `libra2_token` 铸造

使用 `libra2_token` 铸造 `Token` 所需参数与实现 DA 标准的 token 相同.此外,`libra2_token` 模块允许你指定属性键值对(property map)以满足 NFT 的其他需求.

可通过调用 `libra2_token::mint` 铸造 `Token`:
...
如需铸造灵魂绑定 `Token`,可调用 [`libra2_token::mint_soul_bound`](/zh/move-reference/mainnet/libra2-token-objects/libra2_token#mint_soul_bound):
...
<Aside type="caution">
  近期将发布新模块 `TokenMinter` 替代 `libra2_token`.可在 [此处](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-72.md) 跟进进展.
</Aside>

## 示例与有用链接

- [数字资产示例](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/token_objects)
- [数字资产市场示例](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/marketplace)
- [源代码](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/token.move)
- [`libra2_token` 源代码](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/libra2_token.move)
