...
不幸的是,在 Libra2 Move 中没有等价的 `random.randint()`,因此在 dApp 中实现这一功能曾经非常困难.
...
### 用 Libra2 随机性 API 实现简单与安全

使用 Libra2 随机性 API,代码实现如下:
...
- `let winner_idx = libra2_framework::randomness::u64_range(0, n);` 是随机性 API 调用,返回 `[0, n)` 区间内均匀分布的 u64 整数.
...
确保你已安装最新版 [libra2-cli](/zh/build/cli).
...
完整 API 函数列表及文档见 [这里](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/doc/randomness.md).
...
3. 若结果不利于自己则中止.
   ...
   这被称为 [test-and-abort 攻击](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-41.md#test-and-abort-attacks).

Libra2 Move 编译器已更新以防止此类攻击,保障你的合约安全:
依赖随机性的 public 函数会被视为编译错误.
如果你已完成["构建 Libra2 CLI"](/zh/build/cli)部分,
那么你的 Libra2 CLI 已配备了新版编译器.
...
严格来说,这被称为 [undergasing 攻击](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-41.md#undergasing-attacks),
...
[Aptogotchi Random Mint](https://github.com/libra2org/aptogotchi-random-mint/tree/main) 是官方演示随机性 API 用法的 dApp.

完整 API 函数列表及文档见 [这里](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/doc/randomness.md).

部分 API 实现及单元测试见 [这里](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/randomness.move).

API 设计详见 [AIP-41](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-41.md),
系统级/密码学细节见 [AIP-79](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-79.md).
