...
不幸的是,在 Libra Move 中没有等价的 `random.randint()`,因此在 dApp 中实现这一功能曾经非常困难.
...
### 用 Libra 随机性 API 实现简单与安全

使用 Libra 随机性 API,代码实现如下:
...
Libra Move 编译器已更新以防止此类攻击,保障你的合约安全:
依赖随机性的 public 函数会被视为编译错误.
如果你已完成["构建 Libra CLI"](/zh/build/cli)部分,
那么你的 Libra CLI 已配备了新版编译器.
