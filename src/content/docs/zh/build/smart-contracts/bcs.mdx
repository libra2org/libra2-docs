...
二进制规范化序列化 (BCS) 是 Libra 区块链上使用的序列化格式.它是一种二进制规范化的非自描述序列化格式,用于序列化数据结构.BCS 用于序列化链上所有数据,提供 REST API 的二进制响应,并对交易的输入参数进行编码.
...
<Tabs>
  ...
  <TabItem label="TypeScript">
    ...
    import { Serializer, Deserializer } from "@libra2-core/ts-sdk";
