...
- For test chains, there also exists an account that manages the minting of LibraCoin

## Generating genesis

- The genesis organizer constructs a `Layout` and distributes it.
- The genesis organizer prepares the Libra framework's bytecode and distributes it.
...
The following sections rely on tools from the Libra source. See [Building Libra From Source](/network/nodes/building-from-source) for setup.

### The `layout` file

The layout file contains:

- `root_key`: an Ed25519 public key for LibraCoin management.
- `users`: the set of participants
- `chain_id`: the `ChainId` or a unique integer that distinguishes this deployment from other Libra networks
...
### Building the Libra Framework

From your Libra-core repository, build the framework and package it:
