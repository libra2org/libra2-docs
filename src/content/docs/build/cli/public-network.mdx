...
The `libra2` tool supports bootstrapping new blockchains through what is known as a genesis ceremony. The output of the genesis ceremony is the output of move instructions that prepares a blockchain for online operation. The input consists of:
...
- For test chains, there also exists an account that manages the minting of Libra2Coin

## Generating genesis

- The genesis organizer constructs a `Layout` and distributes it.
- The genesis organizer prepares the Libra2 framework's bytecode and distributes it.
...
- Each participant begins their `libra2-node`. The `libra2-node` verifies upon startup that the `genesis.blob` with the waypoint provided by the genesis organizer.
- The blockchain will begin consensus after a quorum of stake is available.

### Prepare libra2-core

The following sections rely on tools from the Libra2 source. See [Building Libra2 From Source](/network/nodes/building-from-source) for setup.

### The `layout` file

The layout file contains:

- `root_key`: an Ed25519 public key for Libra2Coin management.
- `users`: the set of participants
- `chain_id`: the `ChainId` or a unique integer that distinguishes this deployment from other Libra2 networks
...
### Building the Libra2 Framework

From your Libra2-core repository, build the framework and package it:

```shellscript filename="Terminal"
cargo run --package framework
mkdir libra2-framework-release
cp libra2-framework/releases/artifacts/current/build/**/bytecode_modules/* libra2-framework-release
```

The framework will be stored within the `libra2-framework-release` directory.
...
To generate this using the `libra2` CLI:

1. Generate your validator's keys:

```shellscript filename="Terminal"
cargo run --package libra2 -- genesis generate-keys --output-dir bobs
```

2. Generate your `ValidatorConfiguration`:

```shellscript filename="Terminal"
cargo run --package libra2 -- \\
...
- Use the `libra2` CLI to generate genesis and waypoint:

```shellscript filename="Terminal"
cargo run --package libra2 -- genesis generate-genesis --local-repository-dir genesis
```

### Starting an `libra2-node`
