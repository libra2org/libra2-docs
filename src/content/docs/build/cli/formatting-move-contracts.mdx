...
`movefmt` is a formatter tool that makes Move code much easier to write, read, and maintain — greatly improving the development experience on Libra2.

## Installation

`movefmt` is integrated into the Libra2 CLI. To begin using it, first install it using the CLI update command.

```shellscript filename="Terminal"
# Install movefmt for first time usage
libra2 update movefmt
```
...
libra2 update movefmt --target-version <VERSION>
```
...
libra2 move fmt --config max_width=80,indent_size=2
```

## Feedback

Libra2 Labs remains committed to improving the developer experience for builders using Move on Libra2.
