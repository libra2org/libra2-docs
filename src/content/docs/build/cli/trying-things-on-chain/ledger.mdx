---
title: "Use Hardware Ledger via the Libra CLI"
---
...
<Aside type="caution">
  The `Ledger Nano S` has limited memory and may not be able to sign many transactions on Libra. If you are trying to sign a transaction that is too big for your device to handle, you will get the error `Wrong raw transaction length`.
</Aside>

## Initial Setup

You will need to do a few steps of configuration for the Libra CLI and your Ledger device to sign transactions.

<Steps>
  1. Ensure you have the Libra CLI installed.

     You can install the Libra CLI by following [these steps](/build/cli) if you have not done so already.
  ...
  4. Install the Libra App on your Ledger device by following .

  5. Unlock your Ledger device and open the Libra app.

     <Aside type="note" emoji="ℹ️">
       Whenever you want to sign using your Ledger you will need to plug it in, unlock it, and open the Libra app before running any CLI commands.
     </Aside>

  6. Create a new Ledger profile in the Libra CLI

     ```shellscript filename="Terminal"
     libra2 init --profile <your-profile> --ledger
     ```
     ...
     ---
     Libra CLI is now set up for account 59836ba1dd0c845713bdab34346688d6f1dba290dbf677929f2fc20593ba0cfb as profile <your-profile>!  Run `libra2 --help` for more information about commands
     {
       "Result": "Success"
     }
     ```

     In the example, they chose to use the first ledger account by entering `0` after the `libra2 init` command. You may choose whichever account you want.

     **Common errors:**

     1. If you see the error `Device Not Found`, make sure to unlock your Ledger then try this step again.
     2. If you see the error `Libra ledger app is not opened`, make sure to open the Libra app on your Ledger, then try this step again.
...
2. Unlock it.
3. Open the Libra app.
4. Run the Libra CLI command which requires a signature.
...
For example, if you wanted to publish a Move package like the [`hello_blockchain`](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/hello_blockchain) demo contract you could follow the above steps then run:

```shellscript filename="Terminal"
libra2 move publish --profile <your-ledger-profile-name> --named-addresses hello_blockchain=<your-ledger-profile-name>
```
...
One error you might run into is `Error: Wrong raw transaction length`. This means that the transaction or package size was too big for your device to sign. Currently the Libra Ledger app can only support transactions that are smaller than 20kb. The `Ledger Nano S` device has less memory than that, which is why it is more likely to produce this error.
...
<Steps>
  ...
  2. Verify your Ledger is ready
     ...
     2. Check what version of the Libra app you have: `Libra > About > Version`.
     3. If you do not have version `0.6.9` or higher, update it using Ledger Live.
     4. Enable blind signing: `Libra > Settings > Enable Blind Signing`.
  ...
  4. Set up localnet hot wallet profile
     ...
     <details>
       ...
       ---
       Libra CLI is now set up for account 0xaaac71af5f2a4af4ec2639a15799bf9b945afb061c8bee102b636531c1b00eb5 as profile hot-wallet-1!  Run `libra2 --help` for more information about commands
       {
         "Result": "Success"
       }
       ```
  ...
  5. Rotate the hot wallet key
     ...
     ```shellscript filename="Terminal"
     libra2 config show-profiles --profile hot-wallet-1
     libra2 config show-profiles --profile ledger-wallet-1000
     ```
     ...
     ```shellscript filename="Terminal"
     libra2 config delete-profile --profile hot-wallet-1
     libra2 config delete-profile --profile ledger-wallet-1000
     ```
  ...
  6. Recover profile
     ...
     <details>
       ...
       ---
       Libra CLI is now set up for account 0xaaac71af5f2a4af4ec2639a15799bf9b945afb061c8bee102b636531c1b00eb5 as profile ledger-wallet-1000-recovered!  Run `libra2 --help` for more information about commands
       {
         "Result": "Success"
       }
       ```
     ...
     ```shellscript filename="Terminal"
     libra2 config show-profiles --profile ledger-wallet-1000-recovered
     ```
     ...
     <Aside type="note" emoji="🧠">
       The `libra2 init` command first checks the [`account::OriginatingAddress`] table
  ...
  8. Rotate back to Ledger
     ...
     ```shellscript filename="Terminal"
     libra2 config show-profiles --profile ledger-wallet-1000-stale
     libra2 config show-profiles --profile ledger-wallet-1000
     ```
     ...
     ```shellscript filename="Terminal"
     libra2 config delete-profile --profile temporary-hot-wallet
     libra2 config delete-profile --profile ledger-wallet-1000-stale
     ```
...
[`account::OriginatingAddress`]: https://github.com/libra2org/libra2-core/blob/acb6c891cd42a63b3af96561a1aca164b800c7ee/libra2-move/framework/libra2-framework/sources/account.move#L70
