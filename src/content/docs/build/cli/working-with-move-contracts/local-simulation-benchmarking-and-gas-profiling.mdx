...
For demonstration purposes, we will continue to use the [`hello_blockchain`](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/hello_blockchain) package as an example.
...
cd libra2-move/move-examples/hello_blockchain
```

Then publish the package using the following command.

```shellscript filename="Terminal"
libra2 move publish --named-addresses hello_blockchain=default --assume-yes
```
...
```shellscript filename="Terminal"
libra2 move run --function-id 'default::message::set_message' --args 'string:abc' --local
```
...
```shellscript filename="Terminal"
libra2 move run --function-id 'default::message::set_message' --args 'string:abc' --benchmark
```
...
The Libra2 Gas Profiler is a powerful tool that can help you understand the gas usage of Libra2 transactions. Once activated, it will simulate transactions using an instrumented VM, and generate a web-based report.
...
```shellscript filename="Terminal"
libra2 move run --function-id 'default::message::set_message' --args 'string:abc' --profile-gas
```
