...
This section references the [`CliArgs` example package](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/cli_args), which contains the following manifest:
...
[dependencies]
Libra2Framework = { git = "https://github.com/libra2org/libra2-framework.git", rev = "mainnet", subdir = "libra2-framework" }
```

Here, the package is deployed under the named address `test_account`.

<Aside type="note" emoji="ℹ️">
  Set your working directory to [`libra2-move/move-examples/cli_args`](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/cli_args) to follow along:

  ```shellscript filename="Terminal"
  cd <libra2-core-parent-directory>/libra2-core/libra2-move/move-examples/cli_args
  ```
</Aside>
...
```shellscript filename="Terminal"
libra2 account fund-with-faucet --account $ace_addr
```
...
After the package has been published, `libra2 move run` can be used to call `set_vals()`:
...
Here, the call to `libra2 move run` looks like:
...
libra2 move view --json-file view_function_arguments.json
```
...
libra2 move compile --named-addresses test_account=$ace_addr
```
...
Next, run `libra2 move run-script`:
...
<Aside type="note" emoji="ℹ️">
  As of the time of this writing, the `libra2` CLI only supports script function arguments for vectors of type `u8`, and only up to a vector depth of 1. Hence `vector<address>` and `vector<vector<u8>>` are invalid script function argument types.
</Aside>
