...
After installation, you can run the Move Prover to prove an [example](https://github.com/libra2org/libra2-core/tree/main/aptos-move/move-examples/hello_prover):
...
If you encounter errors like the one below when running the command, double-check your Libra CLI version or verify that you're using the correct `aptos` tool, especially if you have multiple versions installed.

```shellscript filename="Terminal
error: unexpected token
    ┌─ ~/.move/https___github_com_libra2-core_libra2-core_git_main/aptos-move/framework/aptos-framework/sources/randomness.move:515:16
    │
515 │         for (i in 0..n) {
    │             -  ^ Expected ')'
    │             │
