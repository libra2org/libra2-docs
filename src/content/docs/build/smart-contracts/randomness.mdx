...
Unfortunately, without an equivalent of `random.randint()` in Libra Move, building a dApp version of it was actually much harder.
...
### Achieve simplicity + security with Libra randomness API

Using Libra randomness API, the implementation will look like this:
...
## How to use Libra randomness API
...
The Libra Move compiler has been updated to prevent this attack for your contract safety:
a randomness-dependent public function is treated as a compile error.
If you have finished the steps in the ["build Libra CLI"](/build/cli) section,
then your Libra CLI are equipped with the updated compiler.
