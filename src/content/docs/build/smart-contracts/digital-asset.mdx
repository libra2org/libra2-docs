---
title: "Libra Digital Asset Standard"
---
...
The Digital Asset (DA) standard is a modern Non-Fungible Token (NFT) standard for Libra. NFTs represent unique assets on-chain, and are stored in collections. These NFTs can be customized to later be transferred, soulbound, burned, mutated, or customized via your own smart contracts.

This standard replaces the legacy [Libra Token Standard](/build/smart-contracts/libra2-token). The most important improvements to note are:
...
If you want a simple way to mint NFTs without the ability to customize or extend their functionality, you can use the `libra2_token` module which implements the DA standard (see the section on how to use it below).
...
| **Royalty**     | An optional [`Royalty`](/move-reference/mainnet/libra2-token-objects/royalty#Royalty) struct indicating what % of the sale price goes to the creator of the `Collection`. This can be changed with a `MutatorRef` generated by the [Royalty module](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/royalty.move). The Royalty module is an extension for the DA standard. See example usage in [`libra2_token.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token-objects/sources/libra2_token.move). |
...
Transferring a `Token` can be done by calling [`object::transfer`](/move-reference/mainnet/libra2-framework/object#transfer).
...
See the `libra2_token` [reference docs](/move-reference/mainnet/libra2-token-objects/libra2_token) for all the helper functions you can use.
...
To mint a soul bound `Token`, you can call [`libra2_token::mint_soul_bound`](/move-reference/mainnet/libra2-token-objects/libra2_token#mint_soul_bound) instead:
