...
<Steps>
  1. Compile code
     ...
     libra2 move compile --named-addresses <named_address>=<your_address>
     ```
  ...
  2. Deploy code to an object
     ...
     ```shellscript filename="Terminal"
     libra2 move deploy-object --address-name <named_address>
     ```

     **An example can be found below:**

     ```shellscript filename="Terminal"
     libra2 move deploy-object --address-name my_address
     ```
  ...
  3. Transfer and upgrade code in an existing package
     ...
     ```shellscript
     libra2 move run --function-id 0x1::object::transfer --type-args 0x1::object::ObjectCore --args address:<object_address> address:<new_owner_address> --profile <deployer_account_profile>
     ```
     ...
     ```shellscript filename="Terminal"
     libra2 move upgrade-object --address-name <named_address> --object-address <code_object_addr>
     ```

     Example of the command above:

     ```shellscript filename="Terminal"
     libra2 move upgrade-object --address-name my_address --object-address 0x8d6eb306bcf6c61dbaa0dbf8daa8252e121b63e95991afcab3b12d3be7f001ab
     ```
