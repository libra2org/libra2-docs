---
title: "Libra Token Standard (Legacy)"
---
...
The [Libra token standard](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token/sources/token.move) is developed with the following principles:
...
<Aside type="note">
  **Fungible token → NFT**<br />
  The Libra token standard supports [mutation of a fungible token to an
  NFT](#evolving-from-fungible-token-to-nft).
</Aside>

### Storing customized token properties on-chain

The Libra token standard uses the [`PropertyMap`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token/sources/property_map.move) module to store on-chain
...
String) are supported in `PropertyMap`. Applications, such as [Libra Names](https://www.libra2names.com/),
...
such evolution of token properties, the Libra token standard provides the
...
To make mutability explicit for both the creator and owner, the Libra token
...
flow of token data through Libra.
...
list, see the [Libra Token Framework](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token/doc/overview.md).
...
For more detailed descriptions, see [Libra Token Framework](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-token/doc/overview.md).

## Token lifecycle

### Token creation

Every Libra token belongs to a collection. The developer first needs to create a
...
Libra also enforces simple validation of the input size and prevents duplication:
