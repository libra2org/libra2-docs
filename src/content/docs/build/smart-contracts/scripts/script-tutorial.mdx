...
The following example calls functions on the [libra2\_coin.move](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/libra2_coin.move) module to confirm the balance of the destination account is less than `desired_balance`, and if so, tops it up to `desired_balance`.
...
script {
    ...
    fun main(src: &signer, dest: address, desired_balance: u64) {
        ...
        let balance = coin::balance<libra2_coin::LibraCoin>(src_addr);
        if (balance < desired_balance) {
            libra2_account::transfer(src, dest, desired_balance - balance);
        };
    }
}
```
...
Let us run through how to execute a Move script with a step-by-step example using the [Libra CLI](/build/cli/working-with-move-contracts).
...
2. Set up the Libra CLI and [create an account](/build/cli/setup-cli):

   ```shellscript filename="Terminal"
   libra2 init --network devnet
   ```
   ...
   profiles:
     default:
       ...
       rest_url: "https://api.devnet.libra2labs.com"
...
3. From this same directory, initialize a new Move project:
   ...
   ```shellscript filename="Terminal"
   libra2 move init --name run_script
   ```
...
5. Compile the script:
   ...
   ```shellscript filename="Terminal"
   $ libra2 move compile --named-addresses addr=cb265645385819f3dbe71aac266e319e7f77aed252cacf2930b68102828bf615
...
6. Run the compiled script:
   ```shellscript filename="Terminal"
   $ libra2 move run-script --compiled-script-path build/run_script/bytecode_scripts/main.mv --args address:b078d693856a65401d492f99ca0d6a29a0c5c0e371bc2521570a86e40d95f823 --args u64:5
...
Note that the path of the compiled script is under `build/run_script/`, not `build/my_script/`. This is because it uses the name of the project contained in `Move.toml`, which is `run_script` from when we ran `libra2 move init --name run_script`.
...
See also how to do this with the [Rust SDK](https://github.com/libra2org/libra2-developer-discussions/discussions/24) instead of the Libra CLI in Libra Developer Discussions.

## Advanced

You may execute a script in a more streamlined fashion; instead of running `libra2 move compile` and then `libra2 move run-script --compiled-script-path` separately, you can just do this:

```shellscript filename="Terminal"
$ libra2 move run-script --script-path sources/my_script.move --args address:b078d693856a65401d492f99ca0d6a29a0c5c0e371bc2521570a86e40d95f823 --args u64:5
```

This will conduct both steps with a single CLI command yet has [issues](https://github.com/libra2org/libra2-core/issues/5733). For this reason, we recommend using the previous two-step approach for now.
