...
- Utilize `entry` for functions intended for use solely from the Libra2 CLI or SDK.
...
|                | Module itself | Other Modules                | Libra2 CLI/SDK |
| -------------- | ------------- | ---------------------------- | ------------- |
| private        | ✅             | ⛔                            | ⛔             |
...
In this case `sample_function` can be called by both the Libra2 CLI/SDK by any module declared as a friend.
...
In the Libra2 Framework, multiple `key`-able resources can be stored at a single object account.
...
[Read more](/build/smart-contracts/objects) about Libra2 Objects.
...
> At Libra2, We are always security-first. During compilation, we ensure that no randomness API is invoked from a public function. However, we still allow users to make this choice by adding the attribute `#[lint::allow_unsafe_randomness]` to the public function.
