---
title: "Libra2 Token Standard Overview"
---

The [Libra2 Digital Asset Standard](/build/smart-contracts/digital-asset) defines the
canonical Non-fungible Token on Libra2. Libra2 leverages composability to extend
...
The rest of this document discusses how the Libra2 token standards compare to the
...
Unlike Ethereum or Libra2 where data and code co-exist, Solana stores data and
...
The [accounts](/network/blockchain/accounts) in Libra2 store both smart contracts
...
There are two means to create NFTs on Libra2:

- The [no-code standard](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-22.md)
...
Libra2 strikes a balance between the customizability offered by Ethereum with the
simplicity of creating new collections like Solana.

Like Ethereum, Libra2 requires indexing to determine the set of all NFTs owned by
...
The collection of Libra2 token standards shares some similarities with Solana,
...
Libra2 identifies a token by its `Address` or `ObjectId`, a location within
...
Libra2 token has metadata in its `Token` resource with the data most commonly
...
which is defined in `TokenDataId` in Libra2. Solana, however, does not offer
mutability for assets, unlike Libra2. Like Libra2, Token Metadata v1.1.0 offers
