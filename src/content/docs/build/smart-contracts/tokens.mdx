---
title: "Libra Token Standard Overview"
---

The [Libra Digital Asset Standard](/build/smart-contracts/digital-asset) defines the
canonical Non-fungible Token on Libra. Libra leverages composability to extend
...
The rest of this document discusses how the Libra token standards compare to the
...
Unlike Ethereum or Libra where data and code co-exist, Solana stores data and
...
The [accounts](/network/blockchain/accounts) in Libra store both smart contracts
...
There are two means to create NFTs on Libra:
...
Libra strikes a balance between the customizability offered by Ethereum with the
simplicity of creating new collections like Solana.

Like Ethereum, Libra requires indexing to determine the set of all NFTs owned by
...
The collection of Libra token standards shares some similarities with Solana,
...
Libra identifies a token by its `Address` or `ObjectId`, a location within
...
Libra token has metadata in its `Token` resource with the data most commonly
...
which is defined in `TokenDataId` in Libra. Solana, however, does not offer
mutability for assets, unlike Libra. Like Libra, Token Metadata v1.1.0 offers
