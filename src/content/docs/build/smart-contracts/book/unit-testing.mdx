...
Unit tests for a Move package can be run with the `libra2 move test` command. See [package](/build/smart-contracts/book/packages) for more info.
...
```shellscript filename="Terminal"
$ libra2 move init --name TestExample
```
...
```toml
[dependencies]
MoveStdlib = { git = "https://github.com/libra2org/libra2-framework.git", subdir="libra2-move/framework/move-stdlib", rev = "main", addr_subst = { "std" = "0x1" } }
```
...
You can then run these tests with the `libra2 move test` command:
...
+-------------------------+
Please use `libra2 move coverage -h` for more detailed source or bytecode test coverage of this package
```

Then by running `libra2 move coverage`, we can get more detailed coverage information. These can be found using the help flag:

```shellscript filename="Terminal"
$ libra2 move coverage -h
```
