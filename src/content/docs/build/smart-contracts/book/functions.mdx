...
A function value is evaluated by providing the corresponding number of parameters, similar as when calling a named function. During evaluation, the function value is _consumed_. Hence if the value needs to be evaluated multiple times, it's type must have the `copy` ability:
