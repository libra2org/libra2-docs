...
# Git dependency
MoveStdlib = { git = "https://github.com/libra2org/libra2-framework", subdir="move-stdlib", rev = "mainnet" }
...
Note, both `libra2 move prove` and `libra2 move test` commands, currently, do not support bytecode as dependencies.
...
## Overriding the Standard Libra2ries

When working with third-party packages, you might encounter issues where different versions of the Move and Libra2 standard library packages are referenced.
  ...
  Unable to resolve packages for package 'C':
    While resolving dependency 'B' in package 'C':
      Unable to resolve package dependency 'B':
        While resolving dependency 'Libra2Framework' in package 'B':
          Unable to resolve package dependency 'Libra2Framework':
            Conflicting dependencies found: package 'Libra2Framework' conflicts with 'Libra2Framework'
```
...
You can apply the override to commands like `libra2 move compile`, `libra2 move run`, and others. Here is the syntax:
