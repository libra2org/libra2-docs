...
Move code (e.g., Move modules) on the Libra2 blockchain can be upgraded. This
...
The Libra2 blockchain natively supports different _upgrade policies_, which allow
...
Move code upgrades on the Libra2 blockchain happen at the [Move package](/build/smart-contracts/book/packages)
...
<Aside type="note">
  Libra2 checks compatibility at the time a Move package is published via an Libra2 transaction. This transaction will abort if deemed incompatible.
</Aside>
...
[Libra2 CLI](/build/cli/working-with-move-contracts). For an example,
see the [Your First Move Module](/build/guides/first-move-module) tutorial.

## Upgrade policies

There are two different upgrade policies currently supported by Libra2:
...
- If you have a dependency to a `compatible` package, it is highly
  ...
  an upgrade; a vote or similar has to be taken. This is the case for the Libra2
  framework.

## Programmatic upgrade

In general, Libra2 offers, via the Move module `libra2_framework::code`,
...
The Libra2 framework itself, including all the on-chain administration logic, is
...
see [Libra2 Governance](/network/blockchain/governance).
