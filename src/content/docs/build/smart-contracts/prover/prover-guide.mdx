...
The Move Prover is invoked via the [Libra2 CLI](/build/cli). In order
...
- Prove the sources of the package in a specific directly:
  ```shellscript filename="Terminal"
  libra2 move prove --package-dir <path>
  ```
...
section of Use Libra2 CLI.

### Target filtering

By default, the `libra2 move prove` command verifies all files of a package.
...
> verify all modules, verifying them in a single `libra2 move prove` run will be
> significantly faster than sequentially.

### Prover options

The Move Prover has a number of options (such as the filter option above) that
you pass with an invocation of: `libra2 move prove <options>`. The most commonly
used option is the `-t` (`--trace`) option that causes the Move Prover to
produce richer diagnosis when it encounters errors:

```shellscript filename="Terminal"
libra2 move prove -f coin -t
```

To see the list of all command line options, run: `libra2 move prove --help`
