...
This new way of indexing uses the [Transaction Stream Service](/build/indexer/txn-stream). You can either use the [Labs-Hosted Transaction Stream Service](/build/indexer/txn-stream/libra2-hosted-txn-stream) or [run your own instance of Transaction Stream Service](/build/indexer/txn-stream/self-hosted).

## 1. Clone the repo

```shellscript filename="Terminal"
# SSH
git clone git@github.com:libra2-core/libra2-indexer-processors.git

# HTTPS
git clone https://github.com/libra2org/libra2-indexer-processors.git
```

Navigate to the directory for the service:

```shellscript filename="Terminal"
cd libra2-indexer-processors
cd rust/processor
```
...
Migrate your custom schemas by copying over each of your custom migrations to the [`migrations`](https://github.com/libra2org/libra2-indexer-processors/tree/main/rust/processor/src/db/postgres/migrations) folder.

### b. Migrate custom processors code

Migrate the code by copying over your custom processors to the [`processors`](https://github.com/libra2org/libra2-indexer-processors/tree/main/rust/processor) folder and any relevant custom models to the [`models`](https://github.com/libra2org/libra2-indexer-processors/tree/main/rust/processor/src/db/common/models) folder. Integrate the custom processors with the rest of the code by adding them to the following Rust code files.

[`mod.rs`](https://github.com/libra2org/libra2-indexer-processors/blob/main/rust/processor/src/processors/mod.rs)
...
[`worker.rs`](https://github.com/libra2org/libra2-indexer-processors/blob/main/rust/processor/src/worker.rs)
