...
Use this method if you want to develop your custom indexer for the Libra2 ledger data.

<Aside type="note">
  When should you use the custom indexer?

  Currently Libra2-provided indexing service (see above) supports the following core Move modules:
...
In the below detailed description, an example of indexing and querying for the coin balances is used. You can see this in the [`coin_processor.rs`](https://github.com/libra2org/libra2-core/blob/main/crates/indexer/src/processors/coin_processor.rs).
...
Make sure you clone the Libra2-core repo by running `git clone https://github.com/libra2org/libra2-core.git` and then `cd` into `libra2-core/tree/main/crates/indexer` directory. Then proceed as below.

a. The first step is to create a new Diesel migration. This will generate a new folder under [migrations](https://github.com/libra2org/libra2-core/tree/main/crates/indexer/migrations) with `up.sql` and `down.sql`
...
See the [full source for `up.sql` and `down.sql`](https://github.com/libra2org/libra2-core/tree/main/crates/indexer/migrations/2022-10-04-073529_add_coin_tables).

c. Run the migration. We suggest running it multiple times with `redo` to ensure that both `up.sql` and `down.sql` are implemented correctly. This will also modify the [`schema.rs`](https://github.com/libra2org/libra2-core/blob/main/crates/indexer/src/schema.rs) file.
...
See the full code in [coin\_balances.rs](https://github.com/libra2org/libra2-core/blob/main/crates/indexer/src/models/coin_models/coin_balances.rs).
...
<Aside type="note">
  See [coin\_processor.rs](https://github.com/libra2org/libra2-core/blob/main/crates/indexer/src/processors/coin_processor.rs) for a relatively straightforward example. You can search for `coin_balances` in the page for the specific code snippet related to coin balances.
</Aside>
...
1. To start with, make sure to add the new processor in the Rust code files: [`mod.rs`](https://github.com/libra2org/libra2-core/blob/main/crates/indexer/src/processors/mod.rs) and [`runtime.rs`](https://github.com/libra2org/libra2-core/blob/main/crates/indexer/src/runtime.rs). See below:

[**mod.rs**](https://github.com/libra2org/libra2-core/blob/main/crates/indexer/src/processors/mod.rs)
...
[**runtime.rs**](https://github.com/libra2org/libra2-core/blob/main/crates/indexer/src/runtime.rs)
...
Test by starting an Libra2 fullnode by running the below command. You will see many logs in the terminal output, so use the `grep` filter to see only indexer log output, as shown below:

```shellscript filename="Terminal"
cargo run -p libra2-node --features "indexer" --release -- -f ./fullnode_coin.yaml | grep -E "_processor"
```
