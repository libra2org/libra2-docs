...
Use this method if you want to develop your custom indexer for the Libra ledger data.

<Aside type="note">
  When should you use the custom indexer?

  Currently Libra-provided indexing service (see above) supports the following core Move modules:
...
Make sure you clone the Libra-core repo by running `git clone https://github.com/libra2org/libra2-core.git` and then `cd` into `libra2-core/tree/main/crates/indexer` directory. Then proceed as below.
...
Test by starting an Libra fullnode by running the below command. You will see many logs in the terminal output, so use the `grep` filter to see only indexer log output, as shown below:
