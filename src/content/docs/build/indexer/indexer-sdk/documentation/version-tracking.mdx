...
The `VersionTrackerStep` is a [common step in the SDK](https://github.com/libra2org/libra2-indexer-processors-v2/blob/main/processor/src/processors/events/events_processor.rs#L125) method as other steps.  Upon a successfully processed batch, the `VersionTrackerStep` will [call](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/sdk/src/common_steps/version_tracker_step.rs#L57) the trait implementation of `save_processor_status()`.
...
If you're writing to Postgres, you can use the SDK's Postgres implementation [here](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/sdk/src/postgres/utils/checkpoint.rs#L66).
It is possible to checkpoint progress in different ways by using enums.
The SDK's Postgres implementation inserts using a simple [`processor_status` model](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/sdk/src/postgres/models/processor_status.rs).
...
[Here is an example](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/sdk/src/postgres/utils/checkpoint.rs#L118) of a `get_starting_version()` method that returns the latest processed version saved.
...
[Add an additional field](https://github.com/libra2org/libra2-indexer-processors-v2/blob/main/processor/src/config/processor_mode.rs#L29) on your `IndexerProcessorConfig` for a `BackfillConfig`.
...
Add the `backfill_config` section to `server_config` in your yaml file to set `backfill_alias`. [Example](https://github.com/libra2org/libra2-indexer-processors-v2/blob/main/processor/example-backfill-config.yaml)
...
Expand on this [implementation](https://github.com/libra2org/libra2-indexer-processors-v2/blob/main/processor/src/db/backfill_processor_status.rs).
...
Add a [statement](https://github.com/libra2org/libra2-indexer-processors-v2/blob/main/processor/src/processors/processor_status_saver.rs#L190) in your `get_starting_version` method to query the `backfill_processor_status_table` when the `BackfillConfig` field is present in `IndexerProcessorConfig` .
