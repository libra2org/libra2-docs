...
For example, if you're using the `IndexerProcessorConfig` from [`aptos-indexer-processor-example`](https://github.com/libra2org/aptos-indexer-processor-example/blob/main/aptos-indexer-processor-example/src/config/indexer_processor_config.rs), a basic `config.yaml` would look like this:
...
`TransactionStreamConfig` also supports more optional fields to modify the connection to [Transaction Stream](/build/indexer/txn-stream), which you can learn more about [here](https://github.com/libra2org/aptos-indexer-processor-sdk/tree/main/aptos-indexer-processors-sdk/transaction-stream).
