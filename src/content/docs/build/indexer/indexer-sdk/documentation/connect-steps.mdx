...
1. After you've instantiated your steps, you need to convert them into [`RunnableStep`](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/sdk/src/traits/runnable_step.rs#L6).
   ...
   The SDK provides a helper function [`.into_runnable_step`](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/sdk/src/traits/into_runnable_step.rs#L13) to convert a step into a `RunnableStep`.
2. Setup your first step with [`ProcessorBuilder::new_with_inputless_first_step`](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/sdk/src/builder/processor_builder.rs#L222).
   In almost all cases, the first step should be a `TransactionStreamStep`. {/* <!-- Add link --> */}
3. Connect the previous step to the next step using [`.connect_to`](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/sdk/src/builder/processor_builder.rs#L303).
...
5. To close off the `ProcessorBuilder`, use [`.end_and_return_output_receiver`](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/sdk/src/builder/processor_builder.rs#L400).
   This returns an [`InstrumentedAsyncReceiver`](https://github.com/libra2org/libra2-indexer-processor-sdk/blob/main/libra2-indexer-processors-sdk/instrumented-channel/src/lib.rs#L88) which you can use to process the output of the last step in the graph.
...
Here's a [full example](https://github.com/libra2org/libra2-indexer-processor-example/blob/main/libra2-indexer-processor-example/src/processors/events/events_processor.rs#L75) from `libra2-indexer-processor-example`.
