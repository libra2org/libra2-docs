...
The Libra2 core processors represent (1) getting a stream of transactions from [Transaction Stream](/build/indexer/txn-stream), (2) extracting the data, (3) writing to a database, and (4) tracking the progress, each as separate steps.
...
1. Implement the `Processable` trait. This trait defines several important details about the step: the input and output types, the processing logic, and the run type (either `AsyncStepRunType` or `PollableAsyncStepRunType`).
   ...
   <Aside type="note">
     ...
     You can see an example of how we use [`rayon.par_iter`](https://docs.rs/rayon/latest/rayon/#basic-usage-and-the-rayon-prelude) to parallelize the processing [here](https://github.com/libra2org/libra2-indexer-processor-example/blob/main/libra2-indexer-processor-example/src/processors/events/events_extractor.rs#L30).
   </Aside>
...
The SDK comes with a set of [common steps](https://github.com/libra2org/libra2-indexer-processor-sdk/tree/main/libra2-indexer-processors-sdk/sdk/src/common_steps) that you can use to build your processor.

1. `TransactionStreamStep` provides a stream of Libra2 transactions to the processor. Read more about it [here](/build/indexer/indexer-sdk/documentation/steps/transaction-stream).
