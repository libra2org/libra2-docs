...
For basic cases, you can copy the [`IndexerProcessorConfig` from the `aptos-indexer-processor-example`](https://github.com/libra2org/aptos-indexer-processor-example/blob/main/aptos-indexer-processor-example/src/config/indexer_processor_config.rs) repository and modify it to fit your needs.
...
You can see a basic example of a `ProcessorConfig` [here](https://github.com/libra2org/aptos-indexer-processor-example/blob/main/aptos-indexer-processor-example/src/config/processor_config.rs).
An example of a more complex setup that includes multiple processors and configurations is [`aptos-indexer-processors`](https://github.com/libra2org/aptos-indexer-processors-v2/blob/main/processor/src/config/processor_config.rs#L84).
...
Next, your processor needs to implement the [`ProcessorTrait`](https://github.com/libra2org/aptos-indexer-processor-sdk/blob/a56b641a6aaca60092fcc9bbd98252f3cd703299/aptos-indexer-processors-sdk/sdk/src/traits/processor_trait.rs#L4).
...
You can see a full example of a processor that indexes raw Libra events in [`aptos-indexer-processor-example`](https://github.com/libra2org/aptos-indexer-processor-example/blob/main/aptos-indexer-processor-example/src/processors/events/events_processor.rs).
As a reference, you can also see all of the processors that make up the [Indexer API](/build/indexer/indexer-api) in [`aptos-indexer-processors`](https://github.com/libra2org/aptos-indexer-processors-v2/tree/main/processor/src/processors).

## How to define `main.rs`

You may copy the [`main.rs`](https://github.com/libra2org/aptos-indexer-processor-example/blob/main/aptos-indexer-processor-example/src/main.rs) file from the `aptos-indexer-processor-example`.
