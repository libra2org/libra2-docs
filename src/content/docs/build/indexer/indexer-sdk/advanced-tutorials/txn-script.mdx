...
1. Clone the [libra2-core](https://github.com/libra2org/libra2-core) repository:
   - Navigate to the `libra2-core/ecosystem/indexer-grpc/indexer-transaction-generator` directory.
...
<Steps>
  1. Set up move\_fixtures folder
     ...
     1. Location:
        The `move_fixtures` folder should be created in the `libra2-core/ecosystem/indexer-grpc/indexer-transaction-generator/imported_transactions` directory. This is the folder where Move scripts and their configurations for test transactions will be stored.

        <Aside type="note"> **Note:** Do not create the `move_fixtures` folder in your processor repository. All Move-related files should reside in the `libra2-core` repository under the specified directory. </Aside>
     2. Steps to set up the folder:
        - if starting fresh, remove all existing files and projects in the `move_fixtures` folder in the libra2-core repo
  ...
  2. Create Your Move Project and Write your Move Script
     ...
     Create your Move project and write a module to output the scenario that you would like to test in your processor. You can refer to an example [here](https://github.com/libra2org/libra2-core/tree/main/ecosystem/indexer-grpc/indexer-transaction-generator/imported_transactions/move_fixtures).
  ...
  3. Set Up Test Accounts
     ...
     3. Update [`libra2-core/ecosystem/indexer-grpc/indexer-transaction-generator/imported_transactions/testing_accounts.yaml`](https://github.com/libra2org/libra2-core/blob/main/ecosystem/indexer-grpc/indexer-transaction-generator/imported_transactions/testing_accounts.yaml) with your accounts.
  ...
  4. Create a Configuration File
     ...
     1. Create a configuration file in the `move_fixtures` [directory](https://github.com/libra2org/libra2-core/blob/main/ecosystem/indexer-grpc/indexer-transaction-generator/imported_transactions/move_fixtures). Name the configuration file according to the test scenario it corresponds to.
  ...
  5. Generate JSON Files and Rust File
     ...
     ```shellscript
         cd ~/libra2-core/ecosystem/indexer-grpc/indexer-transaction-generator
     ...
     3. Output the generated JSON files to the designated folder (`~/libra2-core/ecosystem/indexer-grpc/indexer-test-transactions/src/json_transactions`).
...
[Reference mod.rs](https://github.com/libra2org/libra2-indexer-processor-example/blob/main/test-transactions-example/src/json_transactions/mod.rs).
...
[Reference lib.rs](https://github.com/libra2org/libra2-indexer-processor-example/blob/main/test-transactions-example/src/lib.rs).
...
[Example](https://github.com/libra2org/libra2-indexer-processors-v2/blob/main/integration-tests/Cargo.toml#L19).
...
[Example](https://github.com/libra2org/libra2-indexer-processors-v2/blob/main/integration-tests/src/sdk_tests/events_processor_tests.rs)
