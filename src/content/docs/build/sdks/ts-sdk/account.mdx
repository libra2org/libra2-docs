...
<Aside type="note">
  Following [AIP-55](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-55.md) the SDK supports `Legacy` and `Unified` authentications. `Legacy` includes `ED25519` and `MultiED25519` and `Unified` includes `SingleSender` and `MultiSender` authenticators.
</Aside>
...
const transaction = await libra2.fundAccount({
