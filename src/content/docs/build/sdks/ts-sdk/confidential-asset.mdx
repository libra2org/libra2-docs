...
You can use `confidentialCoin` property of `Libra` client to interact with `CA`
...
Generate `android` and `ios` bindings [here](https://github.com/libra2org/confidential-asset-wasm-bindings) and integrate in your app as you please.
...
For the native apps, you could generate `android` and `ios` bindings [here](https://github.com/libra2org/confidential-asset-wasm-bindings) to use instead of WASM.

***

Now we are ready to go. Let's define Libra client:
...
Or use [`pepper`](/build/guides/libra2-keyless/how-keyless-works) from [Keyless Account](/build/guides/libra2-keyless)
...
And to do so - use `libra2.confidentialAsset.rolloverPendingBalance`.
...
To cover [normalization](#normalization) & `rollover` simultaneously, you could use `libra2.confidentialAsset.safeRolloverPendingCB`.
...
Get your balance and finally call the `libra2.confidentialAsset.normalizeUserBalance` method:
...
For [transfer](/build/sdks/ts-sdk/confidential-asset#confidential-transfer) you need to know the recipient's encryption key and `libra2` account address
...
To do [key rotation](/build/sdks/ts-sdk/confidential-asset#rotate-encryption-key), you need to create a new decryption key and use `libra2.confidentialAsset.rotateCBKey`
