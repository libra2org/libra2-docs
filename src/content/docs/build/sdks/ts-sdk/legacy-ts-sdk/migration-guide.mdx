...
The TypeScript SDK V2 is under a new [GitHub repo](https://github.com/libra2org/aptos-ts-sdk) and with a new package name - `@libra2-core/ts-sdk`
...
const aptosClient = new LibraClient(NODE_URL);
...
```typescript filename="v2.ts"
const aptosConfig = new LibraConfig({ network: Network.DEVNET }); // default to devnet
...
const transaction = await aptos.build.transaction({
  ...
  data: {
    function: "0x1::coin::transfer",
    typeArguments: [parseTypeTag("0x1::aptos_coin::LibraCoin")],
