...
## SDK usage and query the Libra2 chain

Remove all `<*>Client` modules (i.e `Libra2Client`, `FaucetClient`, `CoinClient`, etc.) and replace with an `Libra2` entry point class

**V1**

```typescript filename="v1.ts"
const faucetClient = new FaucetClient(NODE_URL, FAUCET_URL);
const libra2Client = new Libra2Client(NODE_URL);
...
To configure your `Libra2` client, you can use an `Libra2Config` object.

```typescript filename="v2.ts"
const libra2Config = new Libra2Config({ network: Network.DEVNET }); // default to devnet
...
// sign transaction
const signedTransaction = Libra2Client.generateBCSTransaction(
  sender,
  transaction,
);
// submit transaction
...
const transaction = await libra2.build.transaction({
  ...
  data: {
    function: "0x1::coin::transfer",
    typeArguments: [parseTypeTag("0x1::libra2_coin::Libra2Coin")],
...
Rename `Libra2Account` to `Account` and use static methods to generate / derive an account

**V1**

```typescript filename="v1.ts"
// generate a new account (or key pair) OR derive from private key OR derive from private key and address
const account = new Libra2Account(); // supports only Legacy Ed25519

// derive account from derivation path
const account = Libra2Account.fromDerivePath(..)
```
