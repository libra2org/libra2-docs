...
<Steps>
  ...
  4. Set up the Libra2 client

     You can use the `Libra2` object to handle everything that requires a connection to the Libra2 network. A connection is established as soon as you create the object.

     ```typescript filename="quickstart.ts"
     import { Account, Libra2, Libra2Config, Network } from "@libra2-core/ts-sdk";

     // Specify which network to connect to via Libra2Config
     ...
     <Aside type="note">
       (Advanced) If you need to connect to a specific node, you can set that in the `Libra2Config` by specifying the `fullnode` url. Ex. `fullnode: http://localhost:8080/v1`.
     </Aside>

  5. Fetch data from on-chain

     You can use the `Libra2` client to fetch all sorts of data from on-chain such as information about the network itself or account-specific information.
  ...
  6. Send Transactions
     ...
     2. `data` - The information needed for Libra2 to identify what transaction to execute.
...
```typescript filename="quickstart.ts"
/**
 * This example shows how to use the Libra2 client to create accounts, fund them, and transfer between them.
 */
...
async function example() {
  ...
  // Waits for Libra2 to verify and execute the transaction
...
1. Connecting to the network using the `Libra2` client.
2. Creating an account.
3. Looking up data from on-chain using client helper functions like [`libra2.getAccountModules`](https://libra2-core.github.io/libra2-ts-sdk/@libra2-core/ts-sdk-1.35.0/classes/Libra2.html#getAccountModules).
4. Signing and submitting a transaction to the network.
5. Waiting for Libra2 to execute the transaction.
...
<Aside type="note">
  ...
  Note that most helper functions are listed on the [`Libra2` client object](https://libra2-core.github.io/libra2-ts-sdk/@libra2-core/ts-sdk-1.35.0/classes/Libra2.html).
</Aside>
