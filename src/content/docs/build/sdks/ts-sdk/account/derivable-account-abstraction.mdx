...
[Derivable Account Abstraction (DAA)](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-113.md)
...
```move
module libra2_experimental::test_derivable_account_abstraction_ed25519_hex {
    use std::error;
...
<Steps>
  ...
  3. 3. Fund the DAA to create it on chain
     ...
     ```typescript
     await libra2.fundAccount({ accountAddress: daa.accountAddress, amount: 1000000 });
     ```
  ...
  4. 4. Create a recipient account and transfer APT to it
     ...
     const pendingTxn = await libra2.transaction.signAndSubmitTransaction({
       signer: daa,
       transaction: await libra2.transferCoinTransaction({
     ...
     const response = await libra2.waitForTransaction({ transactionHash: pendingTxn.hash });
     ```
</Steps>
