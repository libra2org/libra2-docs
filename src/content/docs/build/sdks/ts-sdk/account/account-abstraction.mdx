...
Account Abstraction (AA) on Libra2 **enables custom transaction authentication logic through Move modules**, allowing accounts to define their own rules beyond native cryptographic schemes. **Note:** This is currently only live on testnet as of July 17, 2025.
...
<Steps>
  1. 1. Deploy Authentication Module
     ...
     To deploy the module, you can use the following commands from the [Libra2 CLI](/build/cli). We assume that you already have set up a workspace with `libra2 init` and
  ...
  2. 2. Setup your Environment
     ...
     const libra2 = new Libra2(new Libra2Config({ network: Network.DEVNET }));
       ...
       export default function useEnableAbstraction() {
         ...
         return {
           enableAbstraction: async () => {
             ...
             // Note: The Libra2 client must be defined somewhere in the application.
             const transaction = libra2.abstraction.enableAccountAbstractionTransaction({
