...
Account Abstraction (AA) on Libra **enables custom transaction authentication logic through Move modules**, allowing accounts to define their own rules beyond native cryptographic schemes. **Note:** This is currently only live on testnet as of July 17, 2025.
...
<Steps>
  1. 1. Deploy Authentication Module
     ...
     To deploy the module, you can use the following commands from the [Libra CLI](/build/cli). We assume that you already have set up a workspace with `libra2 init` and
     declared the named addresses in your `Move.toml` file.

     ```shellscript
     libra2 move publish --named-addresses deployer=0x1234567890123456789012345678901234567890
     ```
  ...
  2. 2. Setup your Environment
     ...
     const libra2 = new Libra(new LibraConfig({ network: Network.DEVNET }));
  ...
  3. 3. (Optional) Check if Account Abstraction is Enabled
     ...
     ```typescript
     const accountAbstractionStatus = await libra2.abstraction.isAccountAbstractionEnabled({
  ...
  4. 4. Enable the Authentication Function
     ...
     ```typescript
     const transaction = libra2.abstraction.enableAccountAbstractionTransaction({
       accountAddress: alice.accountAddress,
       authenticationFunction: `${deployer}::hello_world_authenticator::authenticate`,
     });
     ...
     await libra2.waitForTransaction({ hash: pendingTransaction.hash });
       ...
       export default function useEnableAbstraction() {
         ...
         return {
           enableAbstraction: async () => {
             ...
             // Note: The Libra client must be defined somewhere in the application.
             const transaction = libra2.abstraction.enableAccountAbstractionTransaction({
             ...
             const pendingTxn = await libra2.transaction.submit.simple({
               transaction: txn,
               senderAuthenticator,
             });

             return await libra2.waitForTransaction({ hash: pendingTxn.hash });
           }
         }
       }
       ```
  ...
  6. 6. Sign and Submit a Transaction using the Abstracted Account
     ...
     ```typescript
     const coinTransferTransaction = await libra2.transaction.build.simple({
     ...
     const pendingCoinTransferTransaction = await libra2.transaction.signAndSubmitTransaction({
       transaction: coinTransferTransaction,
       signer: abstractedAccount,
     });

     await libra2.waitForTransaction({ transactionHash: pendingCoinTransferTransaction.hash });
  ...
  7. 7. Conclusion
     ...
     transaction signature contains a `function_info` and `auth_data` field, it means you successfully used account abstraction! The full E2E demo can be found [here](https://github.com/libra2org/libra2-ts-sdk/blob/main/examples/typescript/public_key_authenticator_account_abstraction.ts).
...
<Steps>
  ...
  4. 4. Enable the Authentication Function
     ...
     ```typescript
     const transaction = await libra2.abstraction.enableAccountAbstractionTransaction({
       accountAddress: alice.accountAddress,
       authenticationFunction,
     });
  ...
  5. 5. Permit Bob's Public Key
     ...
     ```typescript
     const enableBobPublicKeyTransaction = await libra2.transaction.build.simple({
     ...
     const pendingEnableBobPublicKeyTransaction = await libra2.signAndSubmitTransaction({
       signer: alice,
       transaction: enableBobPublicKeyTransaction,
     });

     await libra2.waitForTransaction({ hash: pendingEnableBobPublicKeyTransaction.hash });
  ...
  7. 7. Sign and Submit a Transaction using the Abstracted Account
     ...
     ```typescript
     const coinTransferTransaction = new libra2.transaction.build.simple({
       sender: abstractedAccount.accountAddress,
       data: {
         function: "0x1::coin::transfer",
         typeArguments: ["0x1::libra2_coin::LibraCoin"],
     ...
     await libra2.waitForTransaction({ hash: pendingCoinTransferTransaction.hash });
...
```typescript
const transaction = libra2.abstraction.disableAccountAbstractionTransaction({
  accountAddress: alice.accountAddress,
  /**
