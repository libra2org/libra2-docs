...
<Steps>
  1. 1. Deploy Authentication Module
     ...
     To deploy the module, you can use the following commands from the [Libra CLI](/build/cli). We assume that you already have set up a workspace with `aptos init` and
  ...
  2. 2. Setup your Environment
     ...
     const aptos = new Libra(new LibraConfig({ network: Network.DEVNET }));
  ...
  7. 7. Conclusion
     ...
     transaction signature contains a `function_info` and `auth_data` field, it means you successfully used account abstraction! The full E2E demo can be found [here](https://github.com/libra2org/aptos-ts-sdk/blob/main/examples/typescript/public_key_authenticator_account_abstraction.ts).
