...
const tx = await BuildScriptComposerTransaction({
    ...
    // You need to pass Libra2 Config here because the combined transaction needs to read on-chain state
    libra2Config: new Libra2Config({
...
...
    ...
    const libra2 = new Libra2(new Libra2Config({
        network: Network.TESTNET,
    }));
...
2. The caller will need to make sure they pass the right values as arguments to subsequent calls. In the previous example, the `0x1::coin::coin_to_fungible_asset` function will expect an argument of `Coin<Libra2Coin>`.
