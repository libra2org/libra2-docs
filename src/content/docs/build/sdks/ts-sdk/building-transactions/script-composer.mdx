...
<Aside type="tip">
  ...
  https://github.com/libra2org/script-composer-sdk

  Of course! You can use it as an npm package! Please install this package:

  https://www.npmjs.com/package/@libra2-core/script-composer-sdk
</Aside>
...
<Aside type="caution">
  ...
  [https://www.npmjs.com/package/@libra2-core/ts-sdk/v/1.39.0](https://www.npmjs.com/package/@libra2-core/ts-sdk/v/1.39.0)
</Aside>
...
const tx = await BuildScriptComposerTransaction({
    ...
    // You need to pass Libra Config here because the combined transaction needs to read on-chain state
    libra2Config: new LibraConfig({
...
After combining the transaction, we can use interfaces like sign transaction / simulate transaction / submit transaction from `@libra2-core/ts-sdk`:
...
...
    ...
    const libra2 = new Libra(new LibraConfig({
        network: Network.TESTNET,
    }));

    const simulate_result = await libra2.transaction.simulate.simple({
        transaction: tx,
    })
...
1. nodejs: https://github.com/libra2org/script-composer-sdk/tree/main/examples/nodejs
2. nextjs: https://github.com/libra2org/script-composer-sdk/tree/main/examples/nextjs-project
3. react: https://github.com/libra2org/script-composer-sdk/tree/main/examples/react-project
...
2. The caller will need to make sure they pass the right values as arguments to subsequent calls. In the previous example, the `0x1::coin::coin_to_fungible_asset` function will expect an argument of `Coin<LibraCoin>`.

This implements [AIP-102](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-102.md)
