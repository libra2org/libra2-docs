...
Behind the scenes, the Libra2 SDK has two formats for transaction parameters:

1. **Simple** - This represents transaction parameters using primitive types like strings, integers, etc.
2. **Binary Canonical Serialization (BCS)** - This is the format the Libra2 chain recognizes, with specific types (ex. Instead of an integer, it uses types like `U64` or `U128`)
