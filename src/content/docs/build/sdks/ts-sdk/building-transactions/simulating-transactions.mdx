...
The `signerPublicKey` parameter in `libra2.transaction.simulate.simple` is used to verify the signer’s authentication key during transaction simulation. This parameter is optional, and simulation will bypass checking the authentication key if omitted. For example below:
...
For the complete source code, see the [Multisig V2 Example](https://github.com/libra2org/libra2-ts-sdk/blob/main/examples/typescript-esm/multisig_v2.ts).
