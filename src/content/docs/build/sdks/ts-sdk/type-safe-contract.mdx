...
<Steps>
  ...
  2. Step 2
     ...
     With the ABI, you can use Surf as a layer on top of the Libra TypeScript SDK client `Libra`, when interacting with Move contracts. For non-contract related operations, the `Libra` will still need to be used.
     ...
     // First, create an Libra client, make sure the network is the one that contract lives on
     export const libra2 = new Libra(new LibraConfig({ network: Network.DEVNET }));
     // Second, create a SurfClient with the Libra client and the ABI
