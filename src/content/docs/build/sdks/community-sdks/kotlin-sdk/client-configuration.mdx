...
Whilst **Klibra2** offers a **consistent interface** for interacting with Libra
...
it to the `LibraSettings` object when creating an `Libra` client as shown below:

```kotlin
val clientConfig = ClientConfig(followRedirects = false, retryOnServerErrors = 3)
val client = Libra(LibraConfig(LibraSettings(clientConfig = clientConfig)))
```
