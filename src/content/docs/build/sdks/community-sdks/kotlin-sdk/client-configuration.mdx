...
Whilst **Klibra2** offers a **consistent interface** for interacting with Libra2
...
it to the `Libra2Settings` object when creating an `Libra2` client as shown below:

```kotlin
val clientConfig = ClientConfig(followRedirects = false, retryOnServerErrors = 3)
val client = Libra2(Libra2Config(Libra2Settings(clientConfig = clientConfig)))
```
