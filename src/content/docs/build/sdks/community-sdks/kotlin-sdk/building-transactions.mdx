...
Klibra2 boasts an expressive and type-safe DSL-style API for building and sending
transactions on-chain. This guide will walk you through the process of building
and sending transactions using Klibra2.
...
<Steps>
  ...
  2) Build the Transaction
     ...
     Klibra2 provides a `buildTransaction.simple` method to build a transaction. You can specify
  ...
  4) Submit the Transaction
     ...
     <Aside type="note" emoji="ℹ️">
       ...
       ```kotlin
       val executedTransaction = libra2.signAndSubmitTransaction(
             signer = aliceAccount,
...
fun main() = runBlocking {
  val libra2 = Libra2(Libra2Config(Libra2Settings(network = Network.DEVNET)))
  ...
  libra2.fundAccount(alice.accountAddress, FUNDING_AMOUNT).expect("Failed to fund Alice's account")
  libra2.fundAccount(bob.accountAddress, FUNDING_AMOUNT).expect("Failed to fund Bob's account")

  println("Created accounts on chain")
  println("Alice's balance: ${libra2.getAccountAPTAmount(alice.accountAddress)}")
  println("Bob's balance: ${libra2.getAccountAPTAmount(bob.accountAddress)}")
  println("=============================================")
  println(
  ...
  val txn =
    libra2.buildTransaction.simple(
      sender = alice.accountAddress,
      data =
        entryFunctionData {
          function = "0x1::coin::transfer"
          typeArguments = typeArguments { +TypeTagStruct("0x1::libra2_coin::Libra2Coin") }
          functionArguments = functionArguments {
            +bob.accountAddress
  ...
  val commitedTransaction = libra2.signAndSubmitTransaction(alice, txn)

  val executedTransaction =
    libra2.waitForTransaction(
  ...
  val aliceNewBalance =
    libra2.getAccountAPTAmount(alice.accountAddress).expect("Alice's account does not exist")
  val bobNewBalance =
    libra2.getAccountAPTAmount(bob.accountAddress).expect("Bob's account does not exist")
