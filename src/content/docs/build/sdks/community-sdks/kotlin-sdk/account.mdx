...
**Klibra2** provides a few ways to generate account credentials, both _legacy_ and
`SingleKeyAccount`s. You can either generate a new account or derive
an account from a private key.

## Legacy Account

**Klibra2** offers a straightforward method to generate a legacy account using the
...
```kotlin
val fundedAccount = libra2.fundAccount(aliceAccount.accountAddress, FUNDING_AMOUNT)
```
