...
This guide will walk you through the process of setting up Klibra2,
fetching data, and sending a transaction on the Libra2 blockchain.

<Steps>
  1. Install the SDK

     Klibra2 is available for both multiplatform and single-platform development.
     ...
     Depending on your target platform, Klibra2 provides different artifacts in the
     form of `klibra2-jvm`, `klibra2-android`, `klibra2-iosArm64`, and `klibra2-js`.
  ...
  2. Set up the Libra2 client

     You can use the `Libra2` object to handle everything that requires a connection
     to the Libra2 network.
     ...
     Libra2Config object that takes in an Libra2Settings object. The Libra2Settings object
     allows you to specify the network you want to connect to, the fullnode URL, and
     other settings.

     ```kotlin
     val settings = Libra2Settings(network = Network.MAINNET, clientConfig = ClientConfig(maxRetries = 10))
     val libra2Config = Libra2Config(settings = settings)
     val libra2 = Libra2(libra2Config)
     ```

     <Aside type="note" emoji="ℹ️">
       Klibra2 offers common configurations for all platforms while also providing
  ...
  3. Fetch data from on-chain
     ...
     Once you have an `Libra2` object, you can use it to fetch data from the Libra2 blockchain.
  ...
  4. Send Transactions
     ...
     <Steps>
       ...
       2. Build the Transaction
          ...
          ```kotlin
          val txn = libra2.buildTransaction.simple(
              sender = aliceAccount.accountAddress,
              data = entryFunctionData {
                  function = "0x1::coin::transfer"
                  typeArguments = typeArguments {
                      +TypeTagStruct("0x1::libra2_coin::Libra2Coin")
                  }
       ...
       3. Sign the Transaction
          ...
          ```kotlin
            val aliceAuthenticator = libra2.sign(
                sender = aliceAccount,
                transaction = txn,
            )
          ```
       ...
       4. Submit the Transaction
          ...
          ```kotlin
          val commitedTransaction = libra2.submitTransaction.simple(
       ...
       5. Wait for the Transaction to Execute
          ...
          ```kotlin
          val executedTransaction = libra2.waitForTransaction(HexInput.fromString(commitedTransaction.expect("Transaction failed").hash))
          ```
     </Steps>
</Steps>
