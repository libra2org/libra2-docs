...
fetching data, and sending a transaction on the Libra blockchain.
...
<Steps>
  ...
  2. Set up the Libra client

     You can use the `Libra` object to handle everything that requires a connection
     to the Libra network.
     ...
     LibraConfig object that takes in an LibraSettings object. The LibraSettings object
     allows you to specify the network you want to connect to, the fullnode URL, and
     other settings.

     ```kotlin
     val settings = LibraSettings(network = Network.MAINNET, clientConfig = ClientConfig(maxRetries = 10))
     val libra2Config = LibraConfig(settings = settings)
     val libra2 = Libra(libra2Config)
     ```
  ...
  3. Fetch data from on-chain
     ...
     Once you have an `Libra` object, you can use it to fetch data from the Libra blockchain.
          ...
          val txn = libra2.buildTransaction.simple(
              ...
              data = entryFunctionData {
                  ...
                  typeArguments = typeArguments {
                      +TypeTagStruct("0x1::libra2_coin::LibraCoin")
                  }
