...
<Aside type="note">
  ...
  [`Client`](https://pkg.go.dev/github.com/libra2org/libra2-go-sdk#Client)
  object.
</Aside>
...
<Steps>
  1. Build
     ...
     2. `GasUnitPrice` - You can specify a higher than minimum price per gas to be
        executed with higher priority by the Libra2 network.
  ...
  2. Simulate (Optional)
     ...
     Every transaction on the Libra2 chain has a gas fee associated with how much
     ...
     `libra2.SimulateTransaction` like so:
...
4. [Binary Canonical Serialization (BCS)](/build/sdks/go-sdk/building-transactions/bcs-format) - The format used to serialize data for Libra2 transactions.
