...
2. (Optional) Install the plugins for any “Legacy Standard Compatible” Wallets you want to support from [this list](https://github.com/aptos-labs/aptos-wallet-adapter/blob/main/README.md#supported-wallet-packages).
    <Aside type="note">
...
3. In `App.tsx` or it’s equivalent, import the Aptos Wallet Adapter and any legacy Wallet plugins.
...
4. Initialize the `AptosWalletAdapterProvider`.
...
6. Import `useWallet` in files where you want to access data from the `Provider`.
