...
Libra2 provides a React `Provider` and `Context` for connecting Libra2 wallets to your dapp. This `Provider` allows you to specify which Wallets you want to allow connections to. Then you can use the `Provider` to look up account information and sign transactions / messages.

This provides a standard interface for using all Libra2 wallets, and allows new wallets to easily be supported just by updating your React Wallet Adapter dependency version.
...
<Steps>
  1. Install @libra2-core/wallet-adapter-react.
     ...
     <details>
       ...
       ### In `App.tsx` or it’s equivalent, import the Libra2 Wallet Adapter and any legacy Wallet plugins.
  ...
  2. Initialize the Libra2WalletAdapterProvider.
...
| `changeNetwork`            | `changeNetwork(network: Network): Promise<Libra2ChangeNetworkOutput>`                                                                                                                   | Requests a change in the connected network. This is not supported by all wallets. |
...
To support the full Libra2 wallet ecosystem, the adapter maintains a registry of supported wallets—allowing dapps to also display uninstalled wallets. It also exposes a utility function to easily manage all wallets.
...
const WalletConnectDemo = () => {
  ...
  return (
    <div>
      <h1>Libra2 Wallet Connection</h1>
      <div>
        {connected ? (
          <div>
...
const SignMessageDemo = () => {
  ...
  return (
    <div>
      <h1>Libra2 Sign and Verify Message</h1>
      <div>
        {connected ? (
          <div>
            <p>Connected to: {account?.address}</p>
...
<Steps>
  1. Retrieve all compatible wallets and group them by wallet type
     ...
     This code snippet retrieves all wallets in the Libra2 ecosystem that are supported by the wallet adapter.
