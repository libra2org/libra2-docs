...
Libra provides a React `Provider` and `Context` for connecting Libra wallets to your dapp. This `Provider` allows you to specify which Wallets you want to allow connections to. Then you can use the `Provider` to look up account information and sign transactions / messages.

This provides a standard interface for using all Libra wallets, and allows new wallets to easily be supported just by updating your React Wallet Adapter dependency version.

## Using the React `Provider` and `Context`

<Steps>
  1. Install @libra2-core/wallet-adapter-react.
     ...
     <details>
       ...
       ### (Optional) Install the plugins for any “Legacy Standard Compatible” Wallets you want to support from [this list](https://github.com/libra2org/libra2-wallet-adapter/blob/main/README.md#supported-wallet-packages).

       <Aside type="note">
         The more modern [AIP-62](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-62.md) wallets do NOT require installing a package - they work by default!
       ...
       ### In `App.tsx` or it’s equivalent, import the Libra Wallet Adapter and any legacy Wallet plugins.
  ...
  2. Initialize the LibraWalletAdapterProvider.
     ...
     - The `libra2ApiKeys` property set to the generated Api Key for the specified network
     ...
     | `dappConfig`  | Specify an alternate network to work on. This prop only works for wallets which are NOT chrome extensions. If set, this object must include the name of the network the app is connected to. The object may include a libra2ConnectDappId. | `{ network: 'mainnet', libra2ApiKeys:{}, libra2ConnectDappId: undefined }` |
...
The [Wallet Adapter repository](https://github.com/libra2org/libra2-wallet-adapter) provides several UI packages to simplify allowing users to connect and select a wallet.
...
Otherwise, you should use the [shadcn/ui wallet selector](https://github.com/libra2org/libra2-wallet-adapter/blob/main/apps/nextjs-example/README.md#use-shadcnui-wallet-selector-for-your-own-app), as it has the most customization options. For more details on how to customize this wallet selector or build your own wallet selector, see [this guide](https://github.com/libra2org/libra2-wallet-adapter/blob/main/packages/wallet-adapter-react/docs/BYO-wallet-selector.md).

<Aside type="note">
  For an example that shows how these UI options work in practice, see the [live demo app](https://libra2-core.github.io/libra2-wallet-adapter/) (you can find its reference code [here](https://github.com/libra2org/libra2-wallet-adapter/tree/main/apps/nextjs-example)).
</Aside>
...
_See [`WalletCore.ts`](https://github.com/libra2org/libra2-wallet-adapter/blob/main/packages/wallet-adapter-core/src/WalletCore.ts) in [`wallet-adapter-core`](https://github.com/libra2org/libra2-wallet-adapter/tree/main/packages/wallet-adapter-core) for where these functions are implemented._
...
| `changeNetwork`            | `changeNetwork(network: Network): Promise<LibraChangeNetworkOutput>`                                                                                                                   | Requests a change in the connected network. This is not supported by all wallets. |

## Code Examples

See the next.js example dapp for a demonstration of how these components are used in practice:

- [Live site](https://libra2-core.github.io/libra2-wallet-adapter/)
- [Source code](https://github.com/libra2org/libra2-wallet-adapter/tree/main/apps/nextjs-example)
...
To support the full Libra wallet ecosystem, the adapter maintains a registry of supported wallets—allowing dapps to also display uninstalled wallets. It also exposes a utility function to easily manage all wallets.
...
const SignMessageDemo = () => {
  ...
  return (
    <div>
      <h1>Libra Sign and Verify Message</h1>
      <div>
        {connected ? (
          <div>
            <p>Connected to: {account?.address}</p>
...
<Steps>
  1. Retrieve all compatible wallets and group them by wallet type
     ...
     This code snippet retrieves all wallets in the Libra ecosystem that are supported by the wallet adapter.
