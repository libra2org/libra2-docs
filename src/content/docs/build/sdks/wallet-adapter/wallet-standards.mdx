...
In order to ensure interoperability across Libra2 wallets, the following is
required:
...
coming from other chains. To support both of these use cases, the Libra2 wallet
...
### Creating an Libra2 account

Libra2 account creation can be supported across wallets in the following manner:
...
- See the [Libra2 TypeScript SDK's implementation for the derive path](https://github.com/libra2org/libra2-ts-sdk/blob/main/src/account/Account.ts#L181-L202)
...
- See the [Libra2 TypeScript SDK's implementation for the derive path](https://github.com/libra2org/libra2-core/blob/1bc5fd1f5eeaebd2ef291ac741c0f5d6f75ddaef/ecosystem/typescript/sdk/src/libra2_account.ts#L49-L69)
...
[Following AIP-62](https://github.com/libra2-foundation/AIPs/blob/main/aips/aip-62.md), the Wallet standard defines an API for wallet and application interactions.

### Wallet Interface Standard

A wallet must implement a [Libra2Wallet interface](https://github.com/libra2org/wallet-standard/blob/main/src/wallet.ts) with the wallet provider info and features:
...
A wallet must implement a [Libra2WalletAccount interface](https://github.com/libra2org/wallet-standard/blob/main/src/account.ts) that represents the accounts that have been authorized by the dapp.
...
A wallet is considered a valid Libra2 wallet if it implements the standard [required features](https://github.com/libra2org/wallet-standard/blob/main/src/detect.ts#L16).

A wallet must throw a [Libra2WalletError](https://github.com/libra2org/wallet-standard/blob/main/src/errors.ts). The standard requires to support `Unauthorized` and `InternalError` but a wallet can throw a custom `Libra2WalletError` error
...
if (error) {
  throw new Libra2WalletError(-32000, "Invalid Input");
}
```
...
<Aside type="note">
  For a dapp to easily integrate with a wallet, it encouraged to use the [Libra2 Wallet Adapter Standard](/build/sdks/sdks/wallet-adapter).
</Aside>

If for some reason, a dapp decides to implement a custom wallet integration:

A dapp uses the [getLibra2Wallets()](https://github.com/libra2org/wallet-standard/blob/main/src/detect.ts#L40) function which gets all the Libra2 standard compatible wallets.
...
const removeRegisterListener = on("register", function () {
  // The dapp can add new libra2 wallets to its own state context as they are registered
...
keys has been [implemented](https://github.com/libra2org/libra2-core/pull/2972), but SDK integration is in progress.
