...
In order to ensure interoperability across Libra wallets, the following is
required:
...
coming from other chains. To support both of these use cases, the Libra wallet
...
### Creating an Libra account

Libra account creation can be supported across wallets in the following manner:
...
- See the [Libra TypeScript SDK's implementation for the derive path](https://github.com/libra2org/libra2-ts-sdk/blob/main/src/account/Account.ts#L181-L202)
...
- See the [Libra TypeScript SDK's implementation for the derive path](https://github.com/libra2org/libra2-core/blob/1bc5fd1f5eeaebd2ef291ac741c0f5d6f75ddaef/ecosystem/typescript/sdk/src/libra2_account.ts#L49-L69)
...
A wallet must implement a [LibraWallet interface](https://github.com/libra2org/wallet-standard/blob/main/src/wallet.ts) with the wallet provider info and features:
...
A wallet must implement a [LibraWalletAccount interface](https://github.com/libra2org/wallet-standard/blob/main/src/account.ts) that represents the accounts that have been authorized by the dapp.
...
A wallet is considered a valid Libra wallet if it implements the standard [required features](https://github.com/libra2org/wallet-standard/blob/main/src/detect.ts#L16).

A wallet must throw a [LibraWalletError](https://github.com/libra2org/wallet-standard/blob/main/src/errors.ts). The standard requires to support `Unauthorized` and `InternalError` but a wallet can throw a custom `LibraWalletError` error
...
if (error) {
  throw new LibraWalletError(-32000, "Invalid Input");
}
```
...
<Aside type="note">
  For a dapp to easily integrate with a wallet, it encouraged to use the [Libra Wallet Adapter Standard](/build/sdks/sdks/wallet-adapter).
</Aside>

If for some reason, a dapp decides to implement a custom wallet integration:

A dapp uses the [getLibraWallets()](https://github.com/libra2org/wallet-standard/blob/main/src/detect.ts#L40) function which gets all the Libra standard compatible wallets.
