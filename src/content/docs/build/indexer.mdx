...
We have several offerings for getting indexed data from the Libra blockchain.
...
The Libra Indexer is an API you can use to get:

1. Aggregate data (ex. How many NFTs exist?)
2. Historical data (ex. What transactions has this account submitted?)
3. Data that is hard to get from the simpler [Libra Node API](/build/apis/fullnode-rest-api) (ex. What account owns a token named "ExampleToken"?).
...
<CardGrid>
  ...
  <LinkCard href="/build/indexer/indexer-api/ans-lookup" title="Get Libra Name" description="Retrieve the Libra name associated with an account (via the ANS)." />
...
<CardGrid>
  <LinkCard href="/build/indexer/txn-stream/libra2-hosted-txn-stream" title="Libra-Hosted Transaction Stream Service" description="Get access to the Libra-hosted Transaction Stream Service" />
</CardGrid>
