...
This tutorial introduces a practical use case that combines Libra2 framework
...
publishing and Libra2 framework multisig account before moving on to the
...
To create a fungible asset controlled by an Libra2 framework multisig account
...
prerequisite. The Libra2 team provides an example code in `libra2-core` repo.
...
<Aside type="note">
  This example uses the Libra2 devnet, which has historically been reset each
  Thursday.
...
to the console on Libra2 explorer.
...
who will co-own an Libra2 framework multisig account.
...
### Create an Libra2 framework multisig account with a managed fungible asset

Next, let owner1 call the `initialize()` function defined
in `multisig_managed_coin.move`, which first create an Libra2 framework multisig
...
This tutorial shows an e2e flow of using Libra2 framework multisig account to
