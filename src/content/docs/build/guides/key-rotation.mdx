...
Libra2 Move accounts have a public address, an authentication key, a public key,
...
The Libra2 account model facilitates the unique ability to rotate an account's
...
`libra2` CLI does not currently support unproven key rotations.
...
## Key rotation with the Libra2 CLI
...
<Steps>
  ...
  3. Initialize a profile
     ...
     <details>
       ...
       ---
       Libra2 CLI is now set up for account 0xaaa5131b4d3fcef8d33ee465c4ee65727e36039f283455be87b1164200572e5b as profile test-profile-1!  Run `libra2 --help` for more information about commands
       {
         "Result": "Success"
       }
       ```
     ...
     ```shellscript filename="Terminal"
     libra2 config show-profiles --profile test-profile-1
     ```
     ...
     ```shellscript filename="Terminal"
     libra2 config show-private-key --profile test-profile-1
     ```
  ...
  4. Look up address
     ...
     you can look up the account address using `libra2 account lookup-address`:
  ...
  8. Compare profiles
     ...
     ```shellscript filename="Terminal"
     libra2 config show-profiles --profile test-profile-1
     libra2 config show-profiles --profile test-profile-2
     ```
  ...
  11. Attempt invalid rotation (new key already mapped)
      ...
      <details>
        ...
        ---
        Libra2 CLI is now set up for account 0xccc79d46b2963cb87f2ff32c51eb6c6361e8aa108d334d3183c3016389542958 as profile test-profile-3!  Run `libra2 --help` for more information about commands
        {
          "Result": "Success"
        }
        ```
  ...
  13. Clean up
      ...
      libra2 config delete-profile --profile test-profile-1
      libra2 config delete-profile --profile test-profile-2
      libra2 config delete-profile --profile test-profile-3
      libra2 config delete-profile --profile test-profile-4
      ```
...
[here](https://github.com/libra2org/libra2-ts-sdk/blob/main/examples/typescript/rotate_key.ts).
...
```shellscript filename="Terminal"
cd ~/libra2-core/ecosystem/typescript/sdk/examples/typescript-esm
...
View the full example for this code
[here](https://github.com/libra2org/libra2-python-sdk/blob/main/examples/rotate_key.py).
...
```shellscript filename="Terminal"
cd libra2-core/ecosystem/python/sdk
...
[`account::rotate_authentication_key`]: https://github.com/libra2org/libra2-core/blob/acb6c891cd42a63b3af96561a1aca164b800c7ee/libra2-move/framework/libra2-framework/sources/account.move#L326

[`account::rotate_authentication_key_call`]: https://github.com/libra2org/libra2-core/blob/acb6c891cd42a63b3af96561a1aca164b800c7ee/libra2-move/framework/libra2-framework/sources/account.move#L294

[`account::RotationProofChallenge`]: https://github.com/libra2org/libra2-core/blob/acb6c891cd42a63b3af96561a1aca164b800c7ee/libra2-move/framework/libra2-framework/sources/account.move#L79
...
[`account::set_originating_address`]: https://github.com/alnoki/libra2-core/blob/5ba4a8d1344b0bb6e22665525a96e787b9a44e55/libra2-move/framework/libra2-framework/sources/account.move#L528
