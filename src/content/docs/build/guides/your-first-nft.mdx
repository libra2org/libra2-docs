...
This tutorial will guide you through the process of using the Libra TypeScript SDK (`@libra2-core/ts-sdk`) to create a new digital asset (often referred to as an NFT) on Libra. By the end of this tutorial, you will know how to:
...
<Aside type="note">
  This tutorial assumes you are comfortable with using the [Libra CLI](/build/cli), have Node.js and npm installed, and understand basic JavaScript/TypeScript concepts. If you need more info, check out [Node.js Introduction](https://nodejs.org/en/learn/getting-started/introduction-to-nodejs) or the [Libra TypeScript SDK](/build/sdks/ts-sdk) documentation.
</Aside>
...
<Steps>
  1. Setup the Client
     ...
     We import and configure the `Libra` client from the SDK to connect to the specified network:
     ...
     This `libra2` object allows us to interact with the Libra blockchain (funding accounts, creating assets, submitting transactions, etc.).
...
<Steps>
  ...
  2. Install Dependencies
     ...
     You will need the Libra TypeScript SDK and `dotenv` to manage environment variables:
  ...
  5. Adding index.ts
     ...
     console.log("Step 1: Setting up a client to connect to Libra");
     const APTOS_NETWORK = NetworkToNetworkName[process.env.APTOS_NETWORK!] || Network.DEVNET;
     const config = new LibraConfig({ network: APTOS_NETWORK });
     const libra2 = new Libra(config);
     ...
     async function example() {
         ...
         console.log("\nYou can view these transactions on the Libra Explorer:");
  ...
  7. View Your Transactions on the Explorer
     ...
     2. Visit [Libra Explorer](https://explorer.libra2labs.com/?network=devnet)
