...
The next step is to create two accounts locally. [Accounts](/network/blockchain/accounts) consist of a public address and the public/private key pair used to authenticate ownership of the account. This step demonstrates how to generate an Account and store its key pair and address in a variable.
...
Both the collection and token assets are [Objects](/build/smart-contracts/objects) on-chain with unique addresses. Their metadata is stored at the object address. The SDKs provide convenience wrappers around querying this data:
