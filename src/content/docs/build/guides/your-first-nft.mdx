...
This tutorial will guide you through the process of using the Libra2 TypeScript SDK (`@libra2-core/ts-sdk`) to create a new digital asset (often referred to as an NFT) on Libra2. By the end of this tutorial, you will know how to:
...
<Aside type="note">
  This tutorial assumes you are comfortable with using the [Libra2 CLI](/build/cli), have Node.js and npm installed, and understand basic JavaScript/TypeScript concepts. If you need more info, check out [Node.js Introduction](https://nodejs.org/en/learn/getting-started/introduction-to-nodejs) or the [Libra2 TypeScript SDK](/build/sdks/ts-sdk) documentation.
</Aside>
...
<Steps>
  1. Setup the Client
     ...
     We import and configure the `Libra2` client from the SDK to connect to the specified network:
     ...
     This `libra2` object allows us to interact with the Libra2 blockchain (funding accounts, creating assets, submitting transactions, etc.).
  ...
  2. Create and Fund Accounts
     ...
     await libra2.fundAccount({ accountAddress: alice.accountAddress, amount: INITIAL_BALANCE });
     await libra2.fundAccount({ accountAddress: bob.accountAddress, amount: INITIAL_BALANCE });
     ```
  ...
  3. Create a Collection
     ...
     ```tsx filename="index.ts"
     const createCollectionTransaction = await libra2.createCollectionTransaction({
       creator: alice,
     ...
     await libra2.waitForTransaction({ transactionHash: committedTxn.hash });
     ```
  ...
  4. Mint a Digital Asset
     ...
     ```tsx filename="index.ts"
     const mintTokenTransaction = await libra2.mintDigitalAssetTransaction({
       creator: alice,
       collection: "Example Collection",
       description: "This is an example digital asset.",
       name: "Example Asset",
       uri: "https://docs.libra2.org/asset.png",
     });

     const mintTxn = await libra2.signAndSubmitTransaction({
       signer: alice,
       transaction: mintTokenTransaction,
     });
     await libra2.waitForTransaction({ transactionHash: mintTxn.hash });
     ```
  ...
  5. Transfer the Digital Asset
     ...
     ```tsx filename="index.ts"
     const aliceDigitalAssets = await libra2.getOwnedDigitalAssets({ ownerAddress: alice.accountAddress });
     const digitalAssetAddress = aliceDigitalAssets[0].token_data_id;

     const transferTransaction = await libra2.transferDigitalAssetTransaction({
       sender: alice,
       digitalAssetAddress,
       recipient: bob.accountAddress,
     });

     const transferTxn = await libra2.signAndSubmitTransaction({
       signer: alice,
       transaction: transferTransaction,
     });
     await libra2.waitForTransaction({ transactionHash: transferTxn.hash });
     ```
  ...
  6. Verify the Balances
     ...
     ```tsx filename="index.ts"
     const aliceDigitalAssetsAfter = await libra2.getOwnedDigitalAssets({ ownerAddress: alice.accountAddress });
     const bobDigitalAssetsAfter = await libra2.getOwnedDigitalAssets({ ownerAddress: bob.accountAddress });
...
<Steps>
  1. Set up Your Project
     ...
     mkdir libra2-digital-asset-tutorial
     cd libra2-digital-asset-tutorial
     npm init -y
     ```
  ...
  2. Install Dependencies
     ...
     You will need the Libra2 TypeScript SDK and `dotenv` to manage environment variables:

     ```shellscript filename="Terminal"
     npm install @libra2-core/ts-sdk dotenv
  ...
  5. Adding index.ts
     ...
     console.log("Step 1: Setting up a client to connect to Libra2");
     const APTOS_NETWORK = NetworkToNetworkName[process.env.APTOS_NETWORK!] || Network.DEVNET;
     const config = new Libra2Config({ network: APTOS_NETWORK });
     const libra2 = new Libra2(config);
     ...
     async function example() {
         ...
         const collectionURI = "docs.libra2.org";
         ...
         const tokenURI = "docs.libra2.org/asset";
         ...
         const aliceDigitalAssets = await libra2.getOwnedDigitalAssets({
         ...
         const aliceDigitalAssetsAfter = await libra2.getOwnedDigitalAssets({
             ownerAddress: alice.accountAddress,
         });
         const bobDigitalAssetsAfter = await libra2.getOwnedDigitalAssets({
         ...
         console.log("\nYou can view these transactions on the Libra2 Explorer:");
         console.log("https://explorer.libra2labs.com/?network=devnet");
     }
  ...
  7. View Your Transactions on the Explorer
     ...
     You can view these transactions on the Libra2 Explorer:
     https://explorer.libra2labs.com/?network=devnet
     ```

     You can view these transactions on the Libra2 Explorer:

     1. Copy the transaction hash from your console
     2. Visit [Libra2 Explorer](https://explorer.libra2labs.com/?network=devnet)
...
- [libra2 ts-sdk docs](/build/sdks/ts-sdk)
