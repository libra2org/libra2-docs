---
title: "Your First Libra Multisig (Python SDK)"
---
...
1. Set up a development environment for Libra
...
<Aside type="note">
  If you're coming from Ethereum/Solidity, note that Libra handles multisig accounts differently. Libra implements [multisig directly at the protocol level](/network/blockchain/accounts), allowing accounts to require multiple signatures without deploying additional smart contracts.

  ![Multisig Diagram](~/images/multisig_chart.svg)

  We're interfacing with Libra using the [Libra Python SDK](/build/sdks/python-sdk).
</Aside>

Conceptually, a multisig (multi-signature) account works like a bank vault requiring multiple key holders to authorize access. In Libra, this is implemented with digital signatures rather than physical keys, with each authorized signer providing their cryptographic approval.
...
<Steps>
  ...
  7. Install Libra SDK
     ...
     - Downloads the Libra SDK package from PyPI (Python Package Index)
...
<Steps>
  ...
  2. Add base code
     ...
     This code imports all the necessary modules from the Libra SDK. The `libra2_sdk.account` module provides essential functionality for managing accounts and signatures, while `libra2_sdk.transactions` gives us the tools to create and submit blockchain transactions.

     <Aside type="note">
       You can get free test tokens on Libra Devnet or Testnet by using the `FaucetClient`.
     </Aside>
</Steps>
...
<Steps>
  1. Create key holder accounts
     ...
     The `Account.generate()` function creates a new Libra account with a fresh keypair. Each account will have its own private key (for signing) and public key (for verification). In our multisig setup, these accounts represent the key holders who will have authorization to sign transactions, similar to how each bank vault key holder would have their own unique physical key.
  ...
  3. Run the script
     ...
     <Aside type="note">
       ...
       The Libra account model facilitates the unique ability to rotate an account's private key. Since an account's address is the _initial_ authentication key, the ability to sign for an account can be transferred to another private key without changing its public address.
     </Aside>
</Steps>
...
<Steps>
  ...
  3. Run the script
     ...
     You should see output showing your multisig account's public key type and its unique address on the Libra blockchain. For example:
...
<Steps>
  1. Add funding code
     ...
     The `fund_account()` function requests test tokens from the Libra faucet to let us experiment without using real APT. We fund all accounts simultaneously rather than one at a time by first initializing them as `[name]_fund` and then awaiting the async function call that gathers them: `asyncio.gather()`.
...
<Steps>
  ...
  4. Run the script
     ...
     <Aside type="note">
       You can verify transaction on Libra Explorer:

       - Go to [Libra Explorer](https://explorer.libra2labs.com/)
...
You've completed the basics of Libra multisig - creating a "vault" (multisig account), adding "key holders" (signers), and making a simple transfer that requires multiple approvals. But just like modern banking, there's much more we can do:

### Vanity Addresses

Like having a custom bank account number, Libra lets you create "vanity" addresses that start with specific characters. Imagine being able to choose a memorable account number like "0xdd..." for your company "Digital Dynamics"!

### Account Rotation

Banks let you update your security credentials without changing your account number. Similarly, Libra multisig accounts can "rotate" their authentication keys while keeping the same address - perfect for updating security without disrupting existing payment setups.

### Governance & Smart Contracts

Just as banks have complex approval systems for large corporate accounts, Libra multisig can interact with smart contracts and governance systems. Imagine setting up automated rules like:
...
<Aside type="note">
  Let us know what excites you most about multisig on Libra! Join our community channels to share your ideas and experiences.
</Aside>
...
3. Explore [account abstraction in Libra](/network/blockchain/accounts).
4. Join the [Libra Discord](https://discord.gg/libra2labs) for developer support.
