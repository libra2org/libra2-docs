---
title: "Your First Libra2 Multisig (Python SDK)"
---
...
1. Set up a development environment for Libra2
...
<Aside type="note">
  If you're coming from Ethereum/Solidity, note that Libra2 handles multisig accounts differently. Libra2 implements [multisig directly at the protocol level](/network/blockchain/accounts), allowing accounts to require multiple signatures without deploying additional smart contracts.

  ![Multisig Diagram](~/images/multisig_chart.svg)

  We're interfacing with Libra2 using the [Libra2 Python SDK](/build/sdks/python-sdk).
</Aside>

Conceptually, a multisig (multi-signature) account works like a bank vault requiring multiple key holders to authorize access. In Libra2, this is implemented with digital signatures rather than physical keys, with each authorized signer providing their cryptographic approval.
...
<Steps>
  ...
  7. Install Libra2 SDK
     ...
     - Downloads the Libra2 SDK package from PyPI (Python Package Index)
     - Installs it inside your `venv` folder
     - Creates files in `venv/lib/python3.x/site-packages/libra2_sdk`
...
<Steps>
  ...
  2. Add base code
     ...
     This code imports all the necessary modules from the Libra2 SDK. The `libra2_sdk.account` module provides essential functionality for managing accounts and signatures, while `libra2_sdk.transactions` gives us the tools to create and submit blockchain transactions.

     <Aside type="note">
       You can get free test tokens on Libra2 Devnet or Testnet by using the `FaucetClient`.
     </Aside>
</Steps>
...
<Steps>
  1. Create key holder accounts
     ...
     The `Account.generate()` function creates a new Libra2 account with a fresh keypair. Each account will have its own private key (for signing) and public key (for verification). In our multisig setup, these accounts represent the key holders who will have authorization to sign transactions, similar to how each bank vault key holder would have their own unique physical key.
  ...
  3. Run the script
     ...
     <Aside type="note">
       ...
       The Libra2 account model facilitates the unique ability to rotate an account's private key. Since an account's address is the _initial_ authentication key, the ability to sign for an account can be transferred to another private key without changing its public address.
     </Aside>
</Steps>
...
<Steps>
  ...
  3. Run the script
     ...
     You should see output showing your multisig account's public key type and its unique address on the Libra2 blockchain. For example:
...
<Steps>
  1. Add funding code
     ...
     The `fund_account()` function requests test tokens from the Libra2 faucet to let us experiment without using real APT. We fund all accounts simultaneously rather than one at a time by first initializing them as `[name]_fund` and then awaiting the async function call that gathers them: `asyncio.gather()`.
...
<Steps>
  ...
  4. Run the script
     ...
     <Aside type="note">
       You can verify transaction on Libra2 Explorer:

       - Go to [Libra2 Explorer](https://explorer.libra2labs.com/)
       - Make sure Explorer is set to Devnet (check the top right corner)
         ![Switching to Devnet network in Libra2 Explorer](~/images/screenshots/explorer_devnet.png)
...
You've completed the basics of Libra2 multisig - creating a "vault" (multisig account), adding "key holders" (signers), and making a simple transfer that requires multiple approvals. But just like modern banking, there's much more we can do:

### Vanity Addresses

Like having a custom bank account number, Libra2 lets you create "vanity" addresses that start with specific characters. Imagine being able to choose a memorable account number like "0xdd..." for your company "Digital Dynamics"!

### Account Rotation

Banks let you update your security credentials without changing your account number. Similarly, Libra2 multisig accounts can "rotate" their authentication keys while keeping the same address - perfect for updating security without disrupting existing payment setups.

### Governance & Smart Contracts

Just as banks have complex approval systems for large corporate accounts, Libra2 multisig can interact with smart contracts and governance systems. Imagine setting up automated rules like:
...
<Aside type="note">
  Let us know what excites you most about multisig on Libra2! Join our community channels to share your ideas and experiences.
</Aside>

## Next Steps

1. Review the [complete code example](https://github.com/libra2org/libra2-python-sdk/blob/main/examples/multisig.py) which include all the Advanced Features (see above).
2. Learn about [multisig governance in this tutorial](/build/cli/working-with-move-contracts/multi-signature-tutorial).
3. Explore [account abstraction in Libra2](/network/blockchain/accounts).
4. Join the [Libra2 Discord](https://discord.gg/libra2labs) for developer support.
