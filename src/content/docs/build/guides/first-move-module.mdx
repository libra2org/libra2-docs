...
The Libra blockchain allows developers to write Turing complete smart contracts (called “modules”) with the secure-by-design Move language. Smart contracts enable users to send money with the blockchain, but also write arbitrary code, even games! It all starts with the Libra CLI creating an account which will store the deployed (”published”) Move module.

This tutorial will help you understand Move Modules by guiding you through setting up a minimal Libra environment, then how to compile, test, publish and interact with Move modules on the Libra Blockchain. You will learn how to:

1. Setup your environment, install the CLI
2. Create a devnet account and fund it
3. Compile and test a Move module
4. Publish (or "deploy") a Move module to the Libra blockchain
5. Interact with the module
6. Keep building with Libra (next steps)
...
Changes to the blockchain are called “transactions”, and they require an account to pay the network fee (”gas fee”). We will need to create an account with some APT to pay that fee and own the published contract. In order to do that, we will need to use the Libra CLI.

<Steps>
  1. Install the Libra CLI

     [Install the Libra CLI](/build/cli) (if you haven't already).
  ...
  6. Initialize your account
     ...
     <Aside type="note">
       As we are configuring your Libra CLI for this folder, notice that this setup follows the logic of the blockchain itself:
       ...
       3. How do I pay for "gas"? (For devnet, testnet, and local networks, the Libra CLI will helpfully fund this account with Libra Coin, APT).
     ...
     <Aside type="note">
       What you might not have noticed is that the Libra CLI has created a new hidden folder `.libra2/` with a `.gitignore` and `config.yaml` which contains the account information, including private key, public key, and account address.
...
<Steps>
  ...
  7. Verify your balance
     ...
     Click the "Coins" tab to see that you have 1 APT of the Libra Coin. This will allow you to publish and interact with smart contracts on the libra2 devnet.
...
After successfully compiling and testing your module, you can publish it to the Libra blockchain. This process deploys your code so that it's accessible on-chain.
...
<Steps>
  ...
  3. View your balance
     ...
     Click the "Coins" tab to see that you now have less than 1 APT of the Libra Coin.
...
Now that your module is published, you can interact with it through the Libra CLI:
...
4. Join the [Libra Discord](https://discord.gg/libra2labs) to connect with other developers.
