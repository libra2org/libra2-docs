...
<Steps>
  ...
  6. Initialize your account
     ...
     <Aside type="note">
       What you might not have noticed is that the Libra CLI has created a new hidden folder `.aptos/` with a `.gitignore` and `config.yaml` which contains the account information, including private key, public key, and account address.
...
<Steps>
  ...
  2. Open the Libra Explorer
     ...
     Go to the [Libra Explorer](https://explorer.aptoslabs.com/?network=devnet).
  ...
  7. Verify your balance
     ...
     Click the "Coins" tab to see that you have 1 APT of the Libra Coin. This will allow you to publish and interact with smart contracts on the aptos devnet.
...
4. Join the [Libra Discord](https://discord.gg/aptoslabs) to connect with other developers.
