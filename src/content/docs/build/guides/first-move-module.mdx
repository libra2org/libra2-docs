...
The Libra2 blockchain allows developers to write Turing complete smart contracts (called “modules”) with the secure-by-design Move language. Smart contracts enable users to send money with the blockchain, but also write arbitrary code, even games! It all starts with the Libra2 CLI creating an account which will store the deployed (”published”) Move module.

This tutorial will help you understand Move Modules by guiding you through setting up a minimal Libra2 environment, then how to compile, test, publish and interact with Move modules on the Libra2 Blockchain. You will learn how to:

1. Setup your environment, install the CLI
2. Create a devnet account and fund it
3. Compile and test a Move module
4. Publish (or "deploy") a Move module to the Libra2 blockchain
5. Interact with the module
6. Keep building with Libra2 (next steps)
...
Changes to the blockchain are called “transactions”, and they require an account to pay the network fee (”gas fee”). We will need to create an account with some APT to pay that fee and own the published contract. In order to do that, we will need to use the Libra2 CLI.

<Steps>
  1. Install the Libra2 CLI

     [Install the Libra2 CLI](/build/cli) (if you haven't already).
  ...
  3. Verify the installation
     ...
     Run `libra2 --version` to verify you have it installed.

     ```shellscript filename="Terminal"
     libra2 --version
     ```

     You should see a response like `libra2 4.6.1`.
  ...
  6. Initialize your account
     ...
     Run `libra2 init` and press 'enter' for each step of setup to create a test account on `devnet`.

     <Aside type="note">
       As we are configuring your Libra2 CLI for this folder, notice that this setup follows the logic of the blockchain itself:
       ...
       3. How do I pay for "gas"? (For devnet, testnet, and local networks, the Libra2 CLI will helpfully fund this account with Libra2 Coin, APT).
     ...
     <Aside type="note">
       What you might not have noticed is that the Libra2 CLI has created a new hidden folder `.libra2/` with a `.gitignore` and `config.yaml` which contains the account information, including private key, public key, and account address.
...
<Steps>
  ...
  2. Open the Libra2 Explorer

     Go to the [Libra2 Explorer](https://explorer.libra2labs.com/?network=devnet).
  ...
  7. Verify your balance
     ...
     Click the "Coins" tab to see that you have 1 APT of the Libra2 Coin. This will allow you to publish and interact with smart contracts on the libra2 devnet.
...
<Steps>
  1. Initialize the project
     ...
     Initialize a new move project with `libra2 move init --name my_first_module`
  ...
  3. Compile the module
     ...
     Compile the Move module we just created with `libra2 move compile --named-addresses my_first_module=default`
...
<Steps>
  ...
  3. Run the tests
     ...
     Now run the tests with `libra2 move test --named-addresses my_first_module=default`
...
After successfully compiling and testing your module, you can publish it to the Libra2 blockchain. This process deploys your code so that it's accessible on-chain.

<Steps>
  1. Publish the module

     Publish your module with `libra2 move publish --named-addresses my_first_module=default`
...
<Steps>
  ...
  3. View your balance
     ...
     Click the "Coins" tab to see that you now have less than 1 APT of the Libra2 Coin.
...
Now that your module is published, you can interact with it through the Libra2 CLI:
...
4. Join the [Libra2 Discord](https://discord.gg/libra2labs) to connect with other developers.
...
- [REST API specification](/network/nodes/libra2-api-spec)
