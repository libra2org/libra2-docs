...
The `set_message` function modifies the `hello_blockchain` `MessageHolder` resource. A resource is a data structure that is stored in [global storage](/build/smart-contracts/book/structs-and-resources.mdx#storing-resources-in-global-storage). The resource can be read by querying the following REST API:
...
Each successful call to `set_message` after the first call results in the `MessageChange` event being emitted. `MessageChange` is a [Module Event](/network/blockchain/events.mdx#module-events). Module Events for a given account can be accessed via the [GraphQL API](/build/indexer).
