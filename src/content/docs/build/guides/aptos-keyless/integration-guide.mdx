...
4. 1. Present the user with a "Sign In with [IdP]" button on the UI
...
5. 2. Handle the callback by parsing the token and create a Keyless account for the user
...
6. 3. Store the KeylessAccount in local storage (Optional)
...
7. 4. Submit transactions to the Aptos blockchain
