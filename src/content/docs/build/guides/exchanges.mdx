...
This describes how to integrate Libra and Libra assets into an exchange.  It provides
generic information for tracking balances, transferring assets, and testing the integration.

## Overview

This document will guide you through the following tasks to integrate with Libra:
...
It's suggested that you run your own [full node](/network/nodes/full-node) to interact with the Libra blockchain.
...
There is additionally, Libra Name Service (ANS) for friendly .apt names.  For more information about addresses
and Libra Names, see our page on [Accounts](/network/blockchain/accounts).
...
Libra provides two standards for fungible tokens, similar to ERC-20 tokens on Ethereum:

- An earlier [Coin standard](/build/smart-contracts/libra2-coin) used by assets on Libra.
...
<Aside type="note">
  APT, the native token of Libra, is a migrated coin.  This means it can be used
...
represented as a struct name e.g. `0x1::libra2_coin::LibraCoin` for `APT`.
...
<Aside type="note">
  APT, the native token of Libra, is a migrated coin.  Please use the `libra2_account::transfer` functions
...
To test balance checks, you can check the balance for the account `0x5` for the asset `0x1::libra2_coin::LibraCoin`.
...
Libra uses a BFT consensus algorithm, so transactions are finalized immediately
