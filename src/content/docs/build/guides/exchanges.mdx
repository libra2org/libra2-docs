...
This describes how to integrate Libra2 and Libra2 assets into an exchange.  It provides
...
It's suggested that you run your own [full node](/network/nodes/full-node) to interact with the Libra2 blockchain.
...
There is additionally, Libra2 Name Service (ANS) for friendly .apt names.  For more information about addresses
and Libra2 Names, see our page on [Accounts](/network/blockchain/accounts).
...
be created by simply calling `0x1::libra2_account::transfer` with a zero amount to the account you want to create.  Optionally,
`0x1::libra2_account::create_account` can be used to create an account with a zero balance.

```typescript filename="example.ts"
import { Libra2, Ed25519Account, Ed25519PrivateKey } from "@libra2-core/ts-sdk";

const libra2 = new Libra2();
...
Libra2 provides two standards for fungible tokens, similar to ERC-20 tokens on Ethereum:

- An earlier [Coin standard](/build/smart-contracts/libra2-coin) used by assets on Libra2.
...
<Aside type="note">
  APT, the native token of Libra2, is a migrated coin.  This means it can be used
...
represented as a struct name e.g. `0x1::libra2_coin::Libra2Coin` for `APT`.

All coins are stored in an account resource called `0x1::coin::CoinStore<CoinType>`.  Coins must be registered
prior to using the `CoinStore`, but if using the proper functions e.g. `0x1::libra2_account::transfer`
or `0x1::libra2_account::transfer_coins<CoinType>`, this will be done automatically.
...
More info can be found at: [Coin Standard](/build/smart-contracts/libra2-coin)
...
Besides SDK, you can also directly use libra2 node's [balance API endpoint](/build/apis/fullnode-rest-api#tag/accounts/GET/accounts/\{address}/balance/\{asset_type}) to get the balance of a migrated coin or fungible asset.
...
Here is an [example of a coin transfer](https://explorer.libra2labs.com/txn/1747361321?network=mainnet).
The coin transfer can be tracked as an individual transaction
[here](https://fullnode.mainnet.libra2labs.com/v1/transactions/by_version/1747361321)
from the REST API.
...
An example: https://api.mainnet.libra2labs.com/v1/transactions/by\_version/1750174030
...
Here is an example of a migrated coin with APT: https://api.mainnet.libra2labs.com/v1/transactions/by\_version/1642580695
...
<Aside type="note">
  APT, the native token of Libra2, is a migrated coin.  Please use the `libra2_account::transfer` functions
  to transfer APT tokens.
</Aside>

We suggest you use `0x1::libra2_account::transfer_coins<CoinType>(receiver address, amount)` for transferring coins.  It will
...
- [`0x1::libra2_account::transfer_coins<CoinType>(receiver address, amount)`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/libra2_account.move#L108-L112) - Transfer a coin to another account.
- [`0x1::libra2_account::batch_transfer_coins<CoinType>(receiver addresses, amounts)`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/libra2_account.move#L93-L106) - Transfer a coin to multiple accounts.
- [`0x1::libra2_account::transfer(receiver address, amount)`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/libra2_account.move#L74-L91) - Transfer specifically APT to another account.
...
<Aside type="caution">
  ...
  `0x1::libra2_account::create_account(account address)` to create the account
  before the user can
  submit transactions.
</Aside>
...
To test balance checks, you can check the balance for the account `0x5` for the asset `0x1::libra2_coin::Libra2Coin`.
...
| -------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Tether USD           | USDt         | [0x357b0b74bc833e95a115ad22604854d6b0fca151cecd94111770e5d6ffc9dc2b](https://explorer.libra2labs.com/fungible_asset/0x357b0b74bc833e95a115ad22604854d6b0fca151cecd94111770e5d6ffc9dc2b?network=mainnet)               | [Libra2 Foundation](https://libra2foundation.org/currents/global-finance-moves-faster-on-libra2) |
...
Libra2 uses a BFT consensus algorithm, so transactions are finalized immediately
