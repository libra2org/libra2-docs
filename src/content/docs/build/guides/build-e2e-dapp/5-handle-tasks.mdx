...
In the fifth and final chapter of the tutorial on [building an end-to-end dapp on Libra2](/build/guides/build-e2e-dapp), you will add functionality to the app so the user interface is able to handle cases where an account has created a list.
...
const fetchList = async () => {
  ...
  try {
    ...
    while (counter <= taskCounter) {
      ...
      const task = await libra2Client().getTableItem<Task>({handle:tableHandle, data:tableItem});
      tasks.push(task);
      counter++;
    }
...
while (counter <= taskCounter) {
  ...
  const task = await libra2Client().getTableItem(tableHandle, tableItem);
  tasks.push(task);
  counter++;
}
```
...
```tsx filename="App.tsx"
  const onTaskAdded = async () => {
    ...
    try {
      ...
      await libra2Client().waitForTransaction({ transactionHash: response.hash });
...
You have now learned how to build a dapp on Libra2 from end to end. Congratulations! Tell your friends. :-)
