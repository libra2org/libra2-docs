...
Take a look at the new `Move.toml` file. You should see your package information and an `LibraFramework` dependency. The `LibraFramework` dependency points to the `libra2-core/aptos-move/framework/aptos-framework` GitHub repo main branch.
...
6. You can now head to the [Libra Explorer](https://explorer.aptoslabs.com/) link and view the transaction details. You can also see the module published on chain by looking for the object address.
