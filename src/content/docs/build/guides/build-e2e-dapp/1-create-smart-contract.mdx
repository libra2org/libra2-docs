...
This is the first chapter of the tutorial on [building an end-to-end dapp on Libra2](/build/guides/build-e2e-dapp). If you haven’t done it, review that introduction, and ensure your environment meets the [prerequisites](/build/guides/build-e2e-dapp#prerequisites) listed there.
...
Take a look at the new `Move.toml` file. You should see your package information and an `Libra2Framework` dependency. The `Libra2Framework` dependency points to the `libra2-core/libra2-move/framework/libra2-framework` GitHub repo main branch.
...
<Aside type="note">
  You just created a new account on the Libra2 (dev) network! Yay! You can see it by going to the Libra2 Explorer Devnet network view, pasting the `VITE_MODULE_PUBLISHER_ACCOUNT_ADDRESS` value from your `.env` file into the search field, and clicking on the dropdown option!
</Aside>
...
<Aside type="note">
  Creating an event is not mandatory yet useful if dapps/users want to monitor data, such as how many people create a new task, using the [Libra2 Indexer](/build/indexer).
</Aside>
...
6. You can now head to the [Libra2 Explorer](https://explorer.libra2labs.com/) link and view the transaction details. You can also see the module published on chain by looking for the object address.
