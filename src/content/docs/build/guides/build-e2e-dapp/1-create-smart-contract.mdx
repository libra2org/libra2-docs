...
This is the first chapter of the tutorial on [building an end-to-end dapp on Libra](/build/guides/build-e2e-dapp). If you haven’t done it, review that introduction, and ensure your environment meets the [prerequisites](/build/guides/build-e2e-dapp#prerequisites) listed there.
...
Take a look at the new `Move.toml` file. You should see your package information and an `LibraFramework` dependency. The `LibraFramework` dependency points to the `libra2-core/libra2-move/framework/libra2-framework` GitHub repo main branch.
...
<Aside type="note">
  You just created a new account on the Libra (dev) network! Yay! You can see it by going to the Libra Explorer Devnet network view, pasting the `VITE_MODULE_PUBLISHER_ACCOUNT_ADDRESS` value from your `.env` file into the search field, and clicking on the dropdown option!
</Aside>
...
`create-libra2-dapp` comes with premade scripts to easily run `move` commands, like `compile`, `test` and `publish`.
...
<Aside type="note">
  Creating an event is not mandatory yet useful if dapps/users want to monitor data, such as how many people create a new task, using the [Libra Indexer](/build/indexer).
</Aside>
...
UPDATING GIT DEPENDENCY https://github.com/libra2org/libra2-core.git
...
```move filename="todolist.move"
use std::signer;
use libra2_std::table::{Self, Table}; // This one we already have, need to modify it
```
...
- Finally, we emit the `task_created` event that holds the new Task data. `emit_event` is an `libra2-framework` function that accepts a reference to the event handle and a message. In our case, we are passing the function a reference (using the sign &) to the account’s `TodoListresource` `set_task_event` property as the first argument and a second message argument which is the new Task we just created. Remember, we have a `set_task_event` property in our `TodoList` struct.
...
6. Run the `libra2 move test` command. If all goes right, we should see a success message like:
...
```shellscript filename="Terminal"
Transaction submitted: https://explorer.libra2labs.com/txn/0x68dadf24b9ec29b9c32bd78836d20032de615bbef5f10db580228577f7ca945a?network=devnet
...
6. You can now head to the [Libra Explorer](https://explorer.libra2labs.com/) link and view the transaction details. You can also see the module published on chain by looking for the object address.
