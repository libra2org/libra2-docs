...
In the third chapter of the tutorial on [building an end-to-end dapp on Libra2](/build/guides/build-e2e-dapp), you will be learning to fetch data from chain.
...
To fetch data from chain, we can use the [Libra2 TypeScript SDK](/build/sdks/ts-sdk). The SDK provides classes and functions for us to easily interact and query the Libra2 chain.
...
6. Import `libra2Client` using `import { libra2Client } from "./utils/libra2Client";`. This is a client `create-libra2-dapp` created for us to interact with the chain.
...
const fetchList = async () => {
  ...
  try {
    const todoListResource = await libra2Client().getAccountResource(
      {
...
The `libra2Client().getAccountResource()` expects an _account address_ that holds the resource we are looking for and a string representation of an on-chain _Move struct type_.
