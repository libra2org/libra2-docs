...
If you provide blockchain services to your customers and wish to add the Libra2
blockchain to your platform, then this guide is for you. This system integrators
guide will walk you through all you need to integrate the Libra2 blockchain into
your platform.
...
Libra2 has multiple SDKs across many different languages and platforms, please
...
an entity on the Libra2 blockchain that can send transactions. Each account is
...
On Libra2, accounts must be created on-chain prior to any blockchain operations
involving that account. The Libra2 framework supports implicitly creating
accounts when transferring Libra2 coin via [`libra2_account::transfer`](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/libra2_account.move#L18)
...
- A [resource containing Libra2 Coin](https://github.com/libra2org/libra2-core/blob/60751b5ed44984178c7163933da3d1b18ad80388/libra2-move/framework/libra2-framework/sources/coin.move#L50)
...
Libra2 [transactions](/network/blockchain/txns-states) are encoded
...
Libra2 blockchain, and the amount of gas the sender is willing to pay to execute
the transaction.
...
Most basic operations on the Libra2 blockchain should be available via entry
...
To facilitate evaluation of transactions as well as gas estimation, Libra2
...
Most integrations into the Libra2 blockchain benefit from a holistic and
comprehensive overview of the current and historical state of the blockchain.
Libra2 provides historical transactions, state, and events, all the result of
...
to `false`. This is default behavior in Mainnet. In the near future, Libra2 will
...
creation, each user has the resource `CoinStore<0x1::libra2_coin::Libra2Coin>`
or `CoinStore<Libra2Coin>`, for short. Within this resource is the Libra2
coin: `Coin<Libra2Coin>`.
...
In Libra2, each transaction is committed as a distinct version to the
...
Libra2 provides clear and canonical events for all withdraw and deposit of
...
<Aside type="note">
  ...
  sufficient. `0x1::libra2_coin::Libra2Coin`, however, requires
  considering `gas_used` for each transaction sent from the given account
  since it represents gas in Libra2. To reduce unnecessary overhead, extracting
