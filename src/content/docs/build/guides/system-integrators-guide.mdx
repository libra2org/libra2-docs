...
If you provide blockchain services to your customers and wish to add the Libra
blockchain to your platform, then this guide is for you. This system integrators
guide will walk you through all you need to integrate the Libra blockchain into
your platform.
...
Libra has multiple SDKs across many different languages and platforms, please
...
an entity on the Libra blockchain that can send transactions. Each account is
...
On Libra, accounts must be created on-chain prior to any blockchain operations
involving that account. The Libra framework supports implicitly creating
accounts when transferring Libra coin via [`libra2_account::transfer`](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/libra2_account.move#L18)
or explicitly via [`libra2_account::create_account`](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/libra2_account.move#L13).

At creation, an [Libra account](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/account.move#L23) contains:

- A [resource containing Libra Coin](https://github.com/libra2org/libra2-core/blob/60751b5ed44984178c7163933da3d1b18ad80388/libra2-move/framework/libra2-framework/sources/coin.move#L50)
...
Libra [transactions](/network/blockchain/txns-states) are encoded
...
Libra blockchain, and the amount of gas the sender is willing to pay to execute
the transaction.
...
Libra supports two methods for constructing transactions:

- Using the Libra client libraries to generate native BCS transactions.
...
must specify `Content-Type: application/x.libra2.signed_transaction+bcs` in the
HTTP headers. This will return a transaction submission result that, if
successful, contains a transaction hash in
the `hash` [field](https://github.com/libra2org/libra2-core/blob/9b85d41ed8ef4a61a9cd64f9de511654fcc02024/ecosystem/python/sdk/libra2_sdk/client.py#L138).
...
points out many of those entry functions, such as `libra2_account::transfer`
and `libra2_account::create_account`.

Most basic operations on the Libra blockchain should be available via entry
...
example [Move script](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/scripts/two_by_two_transfer)
that uses a MultiAgent transaction to extract funds from two accounts and
deposit them into two other accounts. This is
a [Python example](https://github.com/libra2org/libra2-python-sdk/blob/main/examples/transfer_two_by_two.py)
that uses the bytecode generated by compiling that script.

### Status of a transaction

Obtain transaction status by querying the
API [`/transactions/by_hash/{hash}`](https://api.devnet.libra2labs.com/v1/spec#/operations/get_transaction_by_hash)
...
To facilitate evaluation of transactions as well as gas estimation, Libra
...
to [`/transactions/simulate`](https://api.devnet.libra2labs.com/v1/spec#/operations/simulate_transaction).

Both
the [Typescript SDK](https://github.com/libra2org/libra2-ts-sdk/blob/main/src/api/transactionSubmission/simulate.ts)
and [Python SDK](https://github.com/libra2org/libra2-python-sdk/blob/main/examples/simulate_transfer_coin.py)
...
Most integrations into the Libra blockchain benefit from a holistic and
comprehensive overview of the current and historical state of the blockchain.
Libra provides historical transactions, state, and events, all the result of
...
the [`enable_ledger_pruner`](https://github.com/libra2org/libra2-core/blob/cf0bc2e4031a843cdc0c04e70b3f7cd92666afcf/config/src/config/storage_config.rs#L141)
to `false`. This is default behavior in Mainnet. In the near future, Libra will
...
- [Transactions for an account](https://api.devnet.libra2labs.com/v1/spec#/operations/get_account_transactions)
- [Transaction by version](https://api.devnet.libra2labs.com/v1/spec#/operations/get_transaction_by_version)
- [Events by event handle](https://api.devnet.libra2labs.com/v1/spec#/operations/get_events_by_event_handle)

## Exchanging and tracking fungible assets

Libra has a
standard [Fungible Asset](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/fungible_asset.move).
...
the [`primary_fungible_store::transfer`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/primary_fungible_store.move#L142)
function.
...
Libra has a standard
[Coin type](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/coin.move).
...
creation, each user has the resource `CoinStore<0x1::libra2_coin::LibraCoin>`
or `CoinStore<LibraCoin>`, for short. Within this resource is the Libra
coin: `Coin<LibraCoin>`.

### Transferring coins between users

Coins, including APT, can be transferred between users via
the [`libra2_account::transfer_coins`](https://github.com/libra2org/libra2-core/blob/d1610e1bb5214689a37a9cab59cf9254e8eb2be1/libra2-move/framework/libra2-framework/sources/libra2_account.move#L92)
function for all coins
and [`libra2_account::transfer`](https://github.com/libra2org/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/libra2_account.move#L18)
for Libra coins.
...
In Libra, each transaction is committed as a distinct version to the
...
Libra provides clear and canonical events for all withdraw and deposit of
...
<Aside type="note">
  ...
  sufficient. `0x1::libra2_coin::LibraCoin`, however, requires
  considering `gas_used` for each transaction sent from the given account
  since it represents gas in Libra. To reduce unnecessary overhead, extracting
...
3. A gas fee `2200` of `0x1::libra2_coin::LibraCoin` from the sending account `0x810026ca8291dd88b5b30a1d3ca2edd683d33d06c4a7f7c451d96f6d47bc5e8b`
...
1. The `gas_used` field must be multiplied times the `gas_unit_price`. In this
   ...
   withdrawn is `2200`.
2. Gas is always: `0x1::libra2_coin::LibraCoin`
