...
In Libra, transactions are mapped back to an account in terms of the entity that
signs or authorizes that transaction and provides an account-based sequence
number. When the Libra network receives a new transaction, several rules are
...
account is not enough. Instead, Libra supports the concept of worker accounts
