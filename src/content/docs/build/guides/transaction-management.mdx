...
on the Libra2 blockchain.

## Background

In Libra2, transactions are mapped back to an account in terms of the entity that
signs or authorizes that transaction and provides an account-based sequence
number. When the Libra2 network receives a new transaction, several rules are
...
account is not enough. Instead, Libra2 supports the concept of worker accounts
