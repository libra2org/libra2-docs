...
This tutorial introduces how you can compile, deploy, and mint your own coin (as defined [here](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/coin.move)), named [MoonCoin](https://github.com/libra2org/libra2-core/tree/main/libra2-move/move-examples/moon_coin).
...
[Install the precompiled binary for the Libra2 CLI](/build/cli).
...
<Tabs>
  <TabItem label="TypeScript">
    Clone the `libra2-ts-sdk` repo and build it:

    ```shellscript filename="Terminal"
    git clone https://github.com/libra2org/libra2-ts-sdk.git
    cd libra2-ts-sdk
    pnpm install
    pnpm build
    ```
    ...
    Run the TypeScript [`your_coin`](https://github.com/libra2org/libra2-ts-sdk/blob/main/examples/typescript/your_coin.ts) example:
  ...
  <TabItem label="Python">
    Clone the `libra2-core` repo:

    ```shellscript filename="Terminal"
    git clone https://github.com/libra2org/libra2-core
    ```
    ...
    Run the Python [`your_coin`](https://github.com/libra2org/libra2-python-sdk/blob/main/examples/your_coin.py) example:

    ```shellscript filename="Terminal"
    poetry run python -m examples.your_coin ~/libra2-core/libra2-move/move-examples/moon_coin
    ```
    ...
    ```shellscript filename="Terminal"
    cd ~/libra2-core/libra2-move/move-examples/moon_coin
    ```

    Next, build the package using the CLI:

    ```shellscript filename="Terminal"
    libra2 move compile --named-addresses MoonCoin=0x5e603a89cf690d7134cf2f24fdb16ba90c4f5686333721c12e835fb6c76bc7ba --save-metadata
    ```
...
These are read by the example and published to the Libra2 blockchain:

<Tabs>
  <TabItem label="TypeScript">
    In the TypeScript example, we use `libra2 move build-publish-payload` command to compile and build the module.
    ...
    export function compilePackage(
      ...
      const compileCommand = `libra2 move build-publish-payload --json-output-file ${outputFile} --package-dir ${packageDir} --named-addresses ${addressArg} --assume-yes`;
    ...
    const transaction = await libra2.publishPackageTransaction({
    ...
    await libra2.waitForTransaction({ transactionHash: pendingTransaction.hash });
    ```
  </TabItem>
...
<Aside type="note">
  ...
  [`ManagedCoin`](https://github.com/libra2org/libra2-core/blob/f81ccb01f00227f9c0f36856fead4879f185a9f6/libra2-move/framework/libra2-framework/sources/managed_coin.move#L1) is a simple coin management framework for coins directly managed by users. It provides convenience wrappers around `mint` and `burn`.
</Aside>
...
module MoonCoin::moon_coin {
    ...
    fun init_module(sender: &signer) {
        libra2_framework::managed_coin::initialize<MoonCoin>(
            sender,
            b"Moon Coin",
            b"MOON",
            6,
...
Once a coin type has been published to the Libra2 blockchain, the entity that published that coin type can initialize it:
...
Libra2 provides several building blocks to support coin transfers:
...
- `libra2_account::transfer_coins<CoinType>`: Transfer coins of specific CoinType to a receiver.
