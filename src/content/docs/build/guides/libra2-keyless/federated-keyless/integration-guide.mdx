...
<Steps>
  ...
  2. Step 2. Register the JSON Web Key Set (JWKS) on-chain
     ...
     ```tsx
     import {Libra2} from '@libra2-core/ts-sdk'; // Requires version v1.29.1 or later

     const libra2 = new Libra2(new Libra2Config({ network: Network.DEVNET })); // Configure your network here
     const alice = // Derive your Libra2 account here
  ...
  3. Step 3. Follow the Libra2 Keyless integration guide

     Now that you have registered the JWKS, you can follow the Libra2 Keyless integration guide starting from step 2.  Be sure to set the `jwkAddress` to the address of the account you used to register the JWKS when deriving the `KeylessAccount`.

     [Libra2 Keyless Integration Guide - Step 2](/build/guides/libra2-keyless/integration-guide#step-2-install-the-libra2-typescript-sdk)
</Steps>
