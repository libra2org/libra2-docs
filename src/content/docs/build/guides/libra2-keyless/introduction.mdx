...
Keyless accounts represent a pivotal advancement within the Libra ecosystem, revolutionizing the way users onboard and interact with decentralized applications (dApps). Libra Keyless allows users to gain ownership of a **self-custodial** Libra blockchain account from their existing OpenID Connect (OIDC) account(s) (e.g., Sign in with Google; Sign in with Apple), rather than from a traditional secret key or mnemonic. In a nutshell, with Libra Keyless, a user’s blockchain account is their OIDC account. Over time, Keyless will evolve to support many IdPs who support the OIDC standard, but we will begin with support for the providers listed [here](/build/guides/libra2-keyless/oidc-support).
...
There are two ways to interact with Keyless accounts in the Libra ecosystem. Developers are able to either 1) integrate the Libra Keyless SDK directly into their dApp or 2) integrate a wallet, like Libra Connect, that supports Keyless account creation. This documentation will focus on case #1 and more details on #2 can be found [here](https://libra2connect.app/docs/). Please note that a direct integration of the Keyless SDK will result in user accounts being domain specific to your dApp whereas the use of a wallet integration will allow your users to carry their accounts to any application that supports that wallet.

Note: the Libra Keyless SDK and Libra Connect are representative examples of the aforementioned product experience, but developers in our ecosystem are building alternatives, like a Keyless Unity SDK and alternative wallet products with Keyless integration.

## Libra Keyless Benefits
...
2. **Enhanced dApp user experience**: Ability to transact on the Libra blockchain without needing to navigate away from the application experience to download a wallet.
