...
- **Ephemeral Key Pair:** a temporary public/private key pair that is used to sign transactions for an Libra Keyless account. The public key and its expiration date are committed in the JWT token via the `nonce` field.
...
**If I use the Libra Keyless SDK, can my user’s use their accounts across other dApps?**
